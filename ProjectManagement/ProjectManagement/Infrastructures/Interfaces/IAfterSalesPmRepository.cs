...
namespace ProjectManagement.Infrastructures.Interfaces
{
    interface IAfterSalesPmRepository
    {
      ...
      //  string SaveFocForAftersalesPm(VmAftersalesPmFoc model);
        ...
        SwQcHeadAssignsFromPm GetAllVersionNameForPm1(long swVerNo, long proId);
        string SaveAftersalesPmHarwareIssue(long ids, string projectName, string swVersionNo, string versionName, string moduleName, string issueFrequency, string issueType, string issueDetails, string testingPath, string resultFound, string data1, string issueOccRatio, string numberOfHSsChecked, string remarks, string numberOfHSsReturn);
        List<AftersalesPm_HwIssueVerificationModel> GetHwIssueVerificationList();
        List<AftersalesPm_HwIssueVerificationModel> GetPerHwIssue(long issueId);
        List<AftersalesPm_HwIssuePlaceRequirement> GetPlaceRequirementDetails(long issueId);
        string SaveForwardData(long ids, string forwardReason);
        string SaveAcceptedData(long ids);
        string SaveClosedIssueData(long ids);
        List<AftersalesPm_HwIssueVerificationLogModel> GetLogDetailsPerIssue(long issueId);
        string SaveReportAcceptedData(long ids);
        string SaveReportForwardData(long ids, string forwardReason);
        List<AftersalesPm_HwIssueReportShareModel> GetReportShareDetailsForPerModel(long issueId);
        #endregion
