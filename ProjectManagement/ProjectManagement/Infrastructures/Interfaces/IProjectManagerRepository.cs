...
namespace ProjectManagement.Infrastructures.Interfaces
{
    public interface IProjectManagerRepository
    {
       ...
       // bool SavePmMonthlyIncentive(VmPmIncentivePolicy model);
        ...
        #endregion
        #region UX Team
        List<PmQcAssignModel> GetPmAssignedProjectForUx();
        List<UxDetailsInfoModel> GetUxOptions(string swQcHeadIds);
        List<UxStandardModel> GetUxStandard(string swQcHeadIds);
        List<UxStandardModel> GetUxStandardDetails(string swQcHeadIds);
        string SaveUserExperienceData(VmUserExperienceTest models);
        string GetUxInforForMail(long swQcIds);
        string UserExperienceAttachmentDelete(long swQcIds);
        #endregion
