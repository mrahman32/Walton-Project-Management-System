...
namespace ProjectManagement.Infrastructures.Interfaces
{
    public interface ICommercialRepository
    {
        ...
        List<VmIncentivePolicy> GetIncentiveReport2(string monId, long yearIds);
        ...
        List<LcOpeningPermissionModel> GetLcOpeningPermWithInitiationDone();
        List<LcOpeningPermissionOtherProductModel> GetLcOpeningOtherPermWithInitiationDone();
        bool CheckDupePo(string oraclePoNo);
        bool CheckDupePoOtherProduct(string oraclePoNo);
        ...
        List<LcOpeningPermissionOtherProductModel> GetLcOpeningPermissionOtherProdWithinDateRange(DateTime from,
            DateTime to);
        LcOpeningPermissionDetailModel SaveOrUpdateLcOpeningPermissionDetail(LcOpeningPermissionDetailModel model);
        List<LcOpeningPermissionDetailModel> GetLcPermissionDetailByPermId(long permissionId);
        LcOpeningPermissionDetailModel GetLcPermissionDetailById(long id);
        #endregion
        ...
        void SaveBtrcSpec(ForeignModelInfoFromBtrcModel model);
        List<ForeignModelInfoFromBtrcModel> GetInfoFromBtrc();
        List<FobPriceUpdateLog> GetFobPriceUpdateLogByProjectId(long projectId);
        List<ProjectForecastingInfoModel> GetProjectForecastingList();
        List<ProjectForecastingInfoModel> GetProjectForecastingSum();
        #region Forecast
        List<ProjectMasterModel> GetAllProjectName();
        string SaveProjectForecastingData(List<ProjectForecastingInfoModel> results);
        List<ProjectForecastingInfoModel> GetProjectInfoForForecasting(string proIds);
        string UpdateProjectForecastingData(List<ProjectForecastingInfoModel> results);
        string DeleteProjectForecastingData(List<ProjectForecastingInfoModel> results);
        #endregion

        #region Commercial Incentive policy 2022
        string SaveSpareArrangementAndSupport(Cm_SpareArrangementAndSupportModel models);
        List<Cm_SpareArrangementAndSupportModel> GetSpareArrangementAndSupport();
        string SaveMaterialReorderAndSupport(Cm_MaterialReorderAndSupportModel models);
        List<Cm_MaterialReorderAndSupportModel> GetMaterialReorderAndSupport();
        string SaveReportAnalysis(Cm_ReportAnalysisModel models); 
        List<Cm_ReportAnalysisModel> GetReportAnalysis();
        string SaveNegotiationsAndDevelopment(Cm_NegotiationsAndDevelopmentModel models);
        List<Cm_NegotiationsAndDevelopmentModel> GetNegotiationsAndDevelopment();
        string SavePenalties(Cm_PenaltiesModel models);
        List<Cm_PenaltiesModel> GetPenalties();
        List<Cm_SpareArrangementAndSupportModel> GetSpareArrangementAndSupportIncentive(string months, string years);
        List<Cm_MaterialReorderAndSupportModel> GetMaterialReorderAndSupportIncentive(string months, string years);
        List<Cm_ReportAnalysisModel> GetReportAnalysisIncentive(string months, string years);
        List<Cm_NegotiationsAndDevelopmentModel> GetNegotiationsIncentive(string months, string years);
        List<Cm_PenaltiesModel> GetPenaltiesIncentive(string months, string years);
        List<Cm_MaterialShipmentAndConsumableItemsModel> GetMaterialShipmentAndConsumableItems(string months, string years);
        List<Cm_TotalPoLcAndTtModel> GetPoLcAnTt(string months, string years);
        List<Cm_UserInfosForNewPolicyModel> GetCmUserListForNewPolicy(string months, string years);
        string SaveCommercialMonthlyIncentiveNew(string month, string monNum, string year, string totalAmount, List<Cm_UserInfosForNewPolicyModel> results21,List<Cm_MaterialShipmentAndConsumableItemsModel> results2);
        List<Cm_UserInfosForNewPolicyModel> CheckIncentiveEntry(string months, string years);
        List<Cm_TotalPoLcAndTtModel> GetTotalPoLcAndTt(string months, string years);
        #endregion
