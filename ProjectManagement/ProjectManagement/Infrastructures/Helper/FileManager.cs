        ...
        public string GetFile(string path)
        {
            ...
            catch (Exception)
            {
                ...
                catch (Exception)
                {
                    ...
                    {
                        const string webServerFilePath4 = @"../../Content/UploadImage";
                        fileName = Path.GetFileName(path);
                        try
                        {
                            if (path != null)
                            {
                                if (fileName == null) return null;
                                var webServerFileName = Path.Combine(HttpContext.Current.Server.MapPath(webServerFilePath4),
                                    fileName);
                                if (webServerFileName != "" && !File.Exists(webServerFileName))
                                {
                                    File.Copy(path, webServerFileName, true);
                            ...
                            return webServerFilePath4 + "/" + fileName;
                        }
                        catch (Exception e)
                        {
                            return e.Message;
