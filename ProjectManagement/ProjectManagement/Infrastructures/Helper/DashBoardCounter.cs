...
//                                                  ) AS SwotAnalysisPending");
                var query = string.Format(@"select 
                                                  ...
                                                  (
                                                   select count(*) from ProjectMasters pm  where pm.ProjectStatus in ('NEW') AND CeoApprovalBy IS NOT NULL AND BiApprovalBy IS NOT NULL
                                                  ) AS InitialApprovalPending,
                                                  (
...
//                                                  ) AS SwotAnalysisPending");
                var query = string.Format(@"select 
                                                  ...
                                                  (
                                                   select count(*) from ProjectMasters pm  where pm.ProjectStatus in ('NEW') AND CeoApprovalBy IS NULL AND BiApprovalBy IS NOT NULL
                                                  ) AS InitialApprovalPending,
                                                  (
                                                   select count(*) from ProjectMasters pm  where pm.ProjectStatus in ('REJECTED') AND CeoApprovalBy IS NULL
                                                  ) AS Rejected,
