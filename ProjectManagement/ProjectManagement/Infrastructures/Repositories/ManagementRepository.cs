...
");
  ...
  group by x.ProjectMasterId,x.ProjectName, x.AddedDate", projectId);
        ...
        #region bidding
        public List<Bid_CreateBidModel> GetManagementApprovalPendingBid()
        {
            var query = _dbContext.Database.SqlQuery<Bid_CreateBidModel>(@"SELECT top 100 cb.*,cu1.UserFullName,cu2.UserFullName as AdminApproveByName,cu3.UserFullName as ManagementApproveByName
            FROM [CellPhoneProject].[dbo].[Bid_CreateBid] cb
            left join CellPhoneProject.dbo.CmnUsers cu1 on cu1.CmnUserId=cb.Added
            left join CellPhoneProject.dbo.CmnUsers cu2 on cu2.CmnUserId=cb.AdminApproveBy
            left join CellPhoneProject.dbo.CmnUsers cu3 on cu3.CmnUserId=cb.ManagementApproveBy
            where cb.Status='ADMINAPPROVED' and  AdminApproveDate is not null and ManagementApproveDate is null order by Id desc").ToList();
            return query;
        }
        public List<Bid_SparePartsDetailsModel> GetBidderListWithBid(string bidId)
        {
            long bids;
            long.TryParse(bidId, out bids);

            var query = _dbContext.Database.SqlQuery<Bid_SparePartsDetailsModel>(@"select *,AddedDate as BidCreationDate from [CellPhoneProject].[dbo].[Bid_SparePartsDetails]
            where BidId={0} and IsActive=1", bids).ToList();

            return query;
        }
        public List<Bid_SparePartsDetailsModel> GetBidderListWithBidPrice(string bidId)
        {
            long ids;
            long.TryParse(bidId, out ids);

           var query = _dbContext.Database.SqlQuery<Bid_SparePartsDetailsModel>(@"            
                
            select bdd.BidId,cu.UserFullName,sum(SalableQty)SalableQty,sum(Per_Unit_Price)Per_Unit_Price,sum(Bidder_Bid_Qty)Bidder_Bid_Qty,
            sum(BidderPerUnitPrice) BidderPerUnitPrice,
            cast(cast(bdd.AddedDate as varchar(10)) as date) as BiddingDate,bdd.Added,
            case when  (select top 1 bdd1.ApproveStatus from [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bdd1 
            where bdd1.ApproveStatus=bdd.ApproveStatus and  bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added) is null then 'NOT APPROVED' else
			(select top 1 bdd1.ApproveStatus from [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bdd1 
            where bdd1.ApproveStatus=bdd.ApproveStatus and  bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added) end
			as ApproveStatus,bb.Category

            FROM [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bdd
            inner join [CellPhoneProject].[dbo].Bid_CreateBid bb on bb.Id=bdd.BidId
            left join CellPhoneProject.dbo.CmnUsers cu on cu.CmnUserId=bdd.Added
            where bdd.IsActive=1 and bdd.BidId={0}
            and cast(cast(bdd.AddedDate as varchar(10)) as date) in (select top 1 cast(cast(bdd1.AddedDate as varchar(10)) as date) 
            from  [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bdd1
            where bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added order by bdd1.Added asc)
            group by bdd.BidId,cu.UserFullName,cast(cast(bdd.AddedDate as varchar(10)) as date),bdd.Added,bdd.ApproveStatus,bb.Category", ids).ToList();
            return query;
        }
        public List<Bid_HandsetDetailsModel> GetDetailsForHandset1(string bidId)
        {
            long bids;
            long.TryParse(bidId, out bids);

            var query = _dbContext.Database.SqlQuery<Bid_HandsetDetailsModel>(@"select *,AddedDate as BidCreationDate from [CellPhoneProject].[dbo].[Bid_HandsetDetails]
            where BidId={0} and IsActive=1", bids).ToList();

            return query;
        }
        public List<Bid_HandsetDetailsModel> GetDetailsForHandsetPersonWise(string bidId)
        {
            long ids;
            long.TryParse(bidId, out ids);
            
            var query = _dbContext.Database.SqlQuery<Bid_HandsetDetailsModel>(@"                        
            select bdd.BidId,cu.UserFullName,sum(Qty)Qty,sum(Per_Unit_Price)Per_Unit_Price,
            sum(Bidder_Bid_Qty)Bidder_Bid_Qty, sum(BidderPerUnitPrice) BidderPerUnitPrice,
            cast(cast(bdd.AddedDate as varchar(10)) as date) as BiddingDate,bdd.Added,
            case when (select top 1 bdd1.ApproveStatus from [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders bdd1 
            where bdd1.ApproveStatus=bdd.ApproveStatus and  bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added) IS NULL THEN 'NOT APPROVED' else
            (select top 1 bdd1.ApproveStatus from [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders bdd1 
            where bdd1.ApproveStatus=bdd.ApproveStatus and  bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added) END
            as ApproveStatus,bb.Category

            FROM [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders bdd
            inner join [CellPhoneProject].[dbo].Bid_CreateBid bb on bb.Id=bdd.BidId
            left join CellPhoneProject.dbo.CmnUsers cu on cu.CmnUserId=bdd.Added
            where bdd.IsActive=1 and bdd.BidId={0}
            and cast(cast(bdd.AddedDate as varchar(10)) as date)  in (select top 1 cast(cast(bdd1.AddedDate as varchar(10)) as date)  from  [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders bdd1
            where bdd1.BidId=bdd.BidId and bdd1.Added=bdd.Added order by bdd1.Added asc)
            group by bdd.BidId,cu.UserFullName,cast(cast(bdd.AddedDate as varchar(10)) as date),bdd.Added,bdd.ApproveStatus,bb.Category", ids).ToList();
        ...
        public List<Bid_CreateBidModel> GetParticularBid(string bidId)
        {
            ...
            var query = _dbContext.Database.SqlQuery<Bid_CreateBidModel>(@"select * from CellPhoneProject.[dbo].[Bid_CreateBid] 
            where IsActive=1 and Id={0} ", ids).ToList();

            var query2 = _dbContext.Database.SqlQuery<Bid_BidderAttachmentModel>(@"
                select * from [CellPhoneProject].[dbo].[Bid_BidderAttachment] where BidId={0} and Added={1}", ids, userId).ToList();
            ...
            if (query2 != null)
            {
                foreach (var qq2 in query2)
                {
                    foreach (var qq1 in query)
                    {
                        if (qq1.Id == qq2.BidId)
                        {
                            qq1.BidderAttachment = qq2.BidderAttachment;
                            break;
                        }
                    }
        ...
        public List<Bid_SparePartsDetailsModel> GetHeaderPriceDetailsForPerItems(string spareIds)
        {
            long ids;
            long.TryParse(spareIds, out ids);
            var query = _dbContext.Database.SqlQuery<Bid_SparePartsDetailsModel>(@"select bidder.*,cu.UserFullName,cu.EmployeeCode from [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bidder 
              left join CellPhoneProject.dbo.CmnUsers cu on cu.CmnUserId=bidder.Added
              where SpareIds={0} and bidder.IsActive=1 ", ids).ToList();
            return query;
        }

        public List<Bid_SparePartsDetailsModel> GetBiddersPerItems(string bidId, string addedId)
        {
            long ids;
            long.TryParse(bidId, out ids);

            long userIds;
            long.TryParse(addedId, out userIds);

            var query = _dbContext.Database.SqlQuery<Bid_SparePartsDetailsModel>(@"  
               select bdd.Id,spp.Id as SpareIds,spp.BidId,spp.Model,spp.ItemName,spp.ItemCode,spp.SalableQty,spp.SQA_Comment,spp.Per_Unit_Price,spp.Admin_Remarks,spp.Lock,spp.AddedDate as BidCreationDate,
              bdd.Bidder_Bid_Qty,bdd.BidderPerUnitPrice,bdd.BidderRemarks,bdd.ApproveStatus,bdd.AddedDate as BiddingDate
              from  [CellPhoneProject].[dbo].Bid_SparePartsDetails spp
              inner join  [CellPhoneProject].[dbo].[Bid_SparePartsDetailsForBidders] bdd on bdd.SpareIds=spp.Id 
              where bdd.BidId={0} and bdd.Added={1} ", ids, userIds).ToList();

            return query;
        }
        public List<Bid_HandsetDetailsModel> GetPerItemListForHandset(string spareIds)
        {
            long ids;
            long.TryParse(spareIds, out ids);
            var query = _dbContext.Database.SqlQuery<Bid_HandsetDetailsModel>(@"
            select hdd.Id,bi.Model,bi.ItemCode,bi.Color,bi.IMEI1,bi.Qty,bi.IMEI2,bi.QC_Functional,bi.QC_Aesthetical,bi.Per_Unit_Price,bi.Admin_Remarks,bi.Lock,
            cu.UserFullName,cu.EmployeeCode,hdd.Bidder_Bid_Qty,hdd.BidderPerUnitPrice,hdd.BidderRemarks,hdd.ApproveStatus,hdd.AddedDate,hdd.Added
            from [CellPhoneProject].[dbo].Bid_HandsetDetails bi 
            left join [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders hdd on hdd.HandsetIds=bi.Id
            left join CellPhoneProject.dbo.CmnUsers cu on cu.CmnUserId=hdd.Added
            where Bi.Id={0} and bi.IsActive=1 and hdd.IsActive=1", ids).ToList();
            return query;
        }

        public List<Bid_HandsetDetailsModel> GetBidderListForHandset(string spareIds, string addedId)
        {
            long ids;
            long.TryParse(spareIds, out ids);
            ...
            var query = _dbContext.Database.SqlQuery<Bid_HandsetDetailsModel>(@"  
                select bdd.Id,spp.Id as HandsetIds,spp.BidId,spp.Model,spp.ItemCode,spp.Color,spp.IMEI1,spp.IMEI2,spp.QC_Aesthetical,spp.QC_Functional,spp.Qty,spp.Per_Unit_Price,spp.Admin_Remarks,spp.Lock,spp.AddedDate as BidCreationDate,
                bdd.Bidder_Bid_Qty,bdd.BidderPerUnitPrice,bdd.BidderRemarks,bdd.ApproveStatus,bdd.AddedDate as BiddingDate
                from  [CellPhoneProject].[dbo].Bid_HandsetDetails spp
                inner join  [CellPhoneProject].[dbo].Bid_HandsetDetailsForBidders bdd on bdd.HandsetIds=spp.Id 
                where bdd.BidId={0} and bdd.Added={1} ", ids, userIds).ToList();

            return query;
        }

        public string MngmtApproveSpareBidderDetails(List<Bid_SparePartsDetailsModel> results)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);
            ...
            long ids = Convert.ToInt64(results[0].Id);
            var query = _dbContext.Bid_CreateBid.FirstOrDefault(i => i.Id == ids);
          
            //main table
            query.Status = results[0].ApproveStatus;
            query.ManagementApproveBy = userId;
            query.ManagementApproveDate = DateTime.Now; 
            query.Updated = userId;
            query.UpdatedDate = DateTime.Now;
            //bidderIds = Convert.ToInt64(query.Added);
            _dbContext.Bid_CreateBid.AddOrUpdate(query);
            _dbContext.SaveChanges();

            #region mail Handset
            var bidStart = _dbContext.Bid_CreateBid.FirstOrDefault(i => i.Id == ids && i.IsActive == true);
            var bidderDetails = _dbContext.CmnUsers.FirstOrDefault(i => i.CmnUserId == bidderIds);

            List<string> ids3 = new List<string>();
            List<string> ids4 = new List<string>();
            var wpmsUsers = _dbContext.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);

            long add = Convert.ToInt64(results[0].Added);

            var bidApprovedUsers = _dbContext.CmnUsers.FirstOrDefault(i => i.CmnUserId == add);

            var wpmsUsersBidder = _dbContext.CmnUsers.Where(i => i.RoleName == "MM" || i.RoleName == "BIDDER" || i.RoleName == "HEADBIDDER" && i.IsActive == true).ToList();

            string body = string.Empty;

            if (ids != 0 && query.Status == "MANAGEMENTAPPROVED")
            {
                //bidStart.Status = "CLOSED";
                //bidStart.Status = "MANAGEMENTAPPROVED";
                //bidStart.ManagementApproveDate = DateTime.Now;
                //bidStart.ManagementApproveBy = userId;
                //bidStart.Updated = userId;
                //bidStart.UpdatedDate = DateTime.Now;
                //_dbContext.Bid_CreateBid.AddOrUpdate(bidStart);
                //_dbContext.SaveChanges();

                body += "Thank you for your participation in the bidding. We are glad to inform you that the following bid <b>“Bidding Name:</b> ” " + bidStart.BidName + ", and <b>“Reference Number: ”</b>" + bidStart.Id + "  has been awarded to <b>“ID: ”</b>" + bidApprovedUsers.EmployeeCode + ". This bid has been approved by both Bidding Admin and Management.<br/><br/>";

                body += "Hence <b>“ID: ” </b>" + bidApprovedUsers.EmployeeCode + " is requested to contact with <b>" + bidStart.Category + "</b> department to collect the products and take the necessary actions to complete the process.<br/><br/>";

                body += "<b>Best Regards,</b><br/>";
                body +="Management of Walton Mobile<br/>";
              

                var mailSendFromPms = new MailSendFromPms();
                ids3.Add(wpmsUsers.Email);
                foreach (var wpp in wpmsUsersBidder)
                {
                    ids4.Add(wpp.Email);
                }
                ids4.Add(bidApprovedUsers.Email);

                mailSendFromPms.SendMail2(ids4, ids3, "Bid approval greetings from Management.", body);
            }
            #endregion
        ...
        public List<tblLocationTrackerMobileNumber> GetAllLocationTrackerEmployee(string network)
        {
            string query = string.Format(@" Select * from [RBSYNERGY].[dbo].[tblLocationTrackerMobileNumber] where [IsActive]=1 AND Network='{0}' ", network);
            var allEmployee = _dbContext.Database.SqlQuery<tblLocationTrackerMobileNumber>(query).ToList();
            return allEmployee;
        }

        public List<LocationTrackerViewModel> GetAllLocationTrackReport(string network, string fromDate, string todate, string ddlPhn)
        {
            try
            {
                string[] bpphn;
                ...
                if (!string.IsNullOrWhiteSpace(ddlPhn))
                {
                    ...
                    z = z.TrimEnd(',');
                    whereExt = "AND tltm.EmployeeId in (" + z + ")";
                }

                _dbContext.Database.CommandTimeout = 6000;
                string query = string.Format(@" Select tltm.EmployeeId,tltm.Name,tltm.Network,tltm.Mobile,tlt.Lat,tlt.Lon,tlt.Address,LocationDate 
                                                from RBSYNERGY.dbo.tblLocationTracker tlt
                                                join RBSYNERGY.dbo.tblLocationTrackerMobileNumber tltm on tlt.MobileNumber=tltm.Mobile
                                                where tltm.Network='{0}' and 
                                                CONVERT(date,tlt.LocationDate) between '{1}' and '{2}' {3} ", network, fromDate, todate, whereExt);
                var data = _dbContext.Database.SqlQuery<LocationTrackerViewModel>(query).ToList();
                return data;
            }
            catch (Exception e)
            {
