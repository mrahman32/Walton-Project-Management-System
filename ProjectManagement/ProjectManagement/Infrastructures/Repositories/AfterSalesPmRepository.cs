...
Issues,Status,cast(AssignedTo as bigint) as AssignedTo,RaiseDate,AssignRemarks,AssignedPerson,EmployeeCode,Solution,SolutionGivenBy,SolutionDate
        ...
        #region Aftersales Hardware Issue
        public SwQcHeadAssignsFromPm GetAllVersionNameForPm1(long swVerNo, long proId)
        {
            var proNames = (from pm in _dbeEntities.ProjectMasters where pm.ProjectMasterId == proId select pm.ProjectName).FirstOrDefault();

            var getSwQcInchargeTestCounts = new SwQcHeadAssignsFromPm();
            getSwQcInchargeTestCounts = _dbeEntities.Database.SqlQuery<SwQcHeadAssignsFromPm>(@"select * from SwQcHeadAssignsFromPm
                where ProjectName={1} and SoftwareVersionNo={0} and TestPhaseID !=5 and  TestPhaseID !=10", swVerNo, proNames).FirstOrDefault();
            return getSwQcInchargeTestCounts;
        }

        public string SaveAftersalesPmHarwareIssue(long ids, string projectName, string swVersionNo, string versionName,
            string moduleName, string issueFrequency, string issueType, string issueDetails, string testingPath,
            string resultFound, string data1, string issueOccRatio, string numberOfHSsChecked, string remarks,
            string numberOfHSsReturn)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var aftersalesPm = new AftersalesPm_HwIssueVerification();
            aftersalesPm.ProjectMasterId = ids;
            aftersalesPm.ModelName = projectName;
            aftersalesPm.SoftwareVersionNo = Convert.ToString(swVersionNo);
            aftersalesPm.SoftwareVersionName = versionName;
            aftersalesPm.Module = moduleName;
            aftersalesPm.IssueFrequency = issueFrequency;
            aftersalesPm.IssueType = issueDetails;
            aftersalesPm.IssueDetails = issueDetails;
            aftersalesPm.TestingPath = testingPath;
            aftersalesPm.ResultFound = resultFound;
            aftersalesPm.Data = data1;
            aftersalesPm.IssueOccRatio = issueOccRatio;
            if (numberOfHSsChecked != "")
            {
                aftersalesPm.NumberOfHSsChecked = Convert.ToInt32(numberOfHSsChecked);
            }
            else
            {
                aftersalesPm.NumberOfHSsChecked = 0;
            }

            aftersalesPm.Remarks = remarks;
            if (numberOfHSsReturn != "")
            {
                aftersalesPm.NumberOfHSsReturn = Convert.ToInt32(numberOfHSsReturn);
            }
            else
            {
                aftersalesPm.NumberOfHSsReturn = 0;
            }

            aftersalesPm.IsActive = true;
            aftersalesPm.Status = "ASSIGNED";
            aftersalesPm.Added = userId;
            aftersalesPm.AddedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssueVerification.Add(aftersalesPm);
            _dbeEntities.SaveChanges();


            long idss = aftersalesPm.Id;

            if (idss > 0)
            {
                var logModel = new AftersalesPm_HwIssueVerificationLog();
                logModel.MainId = idss;
                logModel.ProjectMasterId = aftersalesPm.ProjectMasterId;
                logModel.ModelName = aftersalesPm.ModelName;
                logModel.SoftwareVersionNo = aftersalesPm.SoftwareVersionNo;
                logModel.SoftwareVersionName = aftersalesPm.SoftwareVersionName;
                logModel.Module = aftersalesPm.Module;
                logModel.IssueFrequency = aftersalesPm.IssueFrequency;
                logModel.IssueType = aftersalesPm.IssueType;
                logModel.IssueDetails = aftersalesPm.IssueDetails;

                logModel.TestingPath = aftersalesPm.TestingPath;
                logModel.ResultFound = aftersalesPm.ResultFound;
                logModel.Data = aftersalesPm.Data;
                logModel.IssueOccRatio = aftersalesPm.IssueOccRatio;
                logModel.NumberOfHSsChecked = aftersalesPm.NumberOfHSsChecked;
                logModel.Remarks = aftersalesPm.Remarks;
                logModel.NumberOfHSsReturn = aftersalesPm.NumberOfHSsReturn;
                logModel.Status = aftersalesPm.Status;
                logModel.IsActive = aftersalesPm.IsActive;
                logModel.Added = aftersalesPm.Added;
                logModel.AddedDate = aftersalesPm.AddedDate;
                logModel.Updated = aftersalesPm.Updated;
                logModel.UpdatedDate = aftersalesPm.UpdatedDate;
                logModel.LogAdded = userId;
                logModel.LogAddedDate = DateTime.Now;

                _dbeEntities.AftersalesPm_HwIssueVerificationLog.Add(logModel);
                _dbeEntities.SaveChanges();
            }
            _dbeEntities.SaveChanges();
            ...
            if (aftersalesPm.Id > 0)
            {
                List<long> ids3 = new List<long>();
                List<long> ids4 = new List<long>();
                var wpmsUsers = _dbeEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);
                // var wpmsUsers2 = _dbeEntities.CmnUsers.FirstOrDefault(i => i.RoleName == "HWHEAD" && i.RoleName=="HW" && i.IsActive == true);
                var wpmsUsers2 = (from map in _dbeEntities.CmnUsers
                                  where (map.RoleName == "HWHEAD" || map.RoleName == "HW") && map.IsActive == true
                                  select map).ToList();
                //var wpmsUsers3 = _dbeEntities.CmnUsers.FirstOrDefault(i => i.RoleName == "ASPMHEAD" && i.IsActive == true);
                ...
                //ids4.Add(userId);
                //ids4.Add(wpmsUsers3.CmnUserId);
                var wpmsUsers3 = (from map in _dbeEntities.CmnUsers
                                  where (map.RoleName == "ASPMHEAD" || map.RoleName == "ASPM") && map.IsActive == true
                                  select map).ToList();

                ids3.Add(userId);
                foreach (var cmnUser in wpmsUsers2)
                {
                    ids3.Add(cmnUser.CmnUserId);
                }
                ids4.Add(userId);
                foreach (var cmnUser in wpmsUsers3)
                {
                    ids4.Add(cmnUser.CmnUserId);
                }

                string body = string.Empty;

                body += "This is to inform you that a new market issue has been raised by  <b>" + wpmsUsers.UserFullName
                    + " </b> for Verification purpose. Please place requirement within 3 days. Issue details are given below : ";

                body += "<br/><br/> <b>Model Name :</b>" + projectName + "<br/><b>Module :</b>" + moduleName + "<br/><b>Issue Details :</b>" + issueDetails +
                     "<br/><b>Issue Type :</b>" + issueType + "<br/><b>Result :</b>" + resultFound + "<br/><b>Remarks :</b>" + remarks +
                    "<br/><b>Status :</b>" + aftersalesPm.Status + "<br/><br/><br/>";

                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(ids3, ids4, "Market Issue.", body);
        ...
        public List<AftersalesPm_HwIssueVerificationModel> GetHwIssueVerificationList()
        {
            var query = _dbeEntities.Database.SqlQuery<AftersalesPm_HwIssueVerificationModel>(@"SELECT afi.*, pm.SupplierName
                FROM [CellPhoneProject].[dbo].[AftersalesPm_HwIssueVerification] afi
                left join [CellPhoneProject].[dbo].ProjectMasters pm on pm.ProjectMasterId=afi.ProjectMasterId
                where afi.IsActive=1 order by afi.Id desc").ToList();

            return query;
        }

        public List<AftersalesPm_HwIssueVerificationModel> GetPerHwIssue(long issueId)
        {
            var query = _dbeEntities.Database.SqlQuery<AftersalesPm_HwIssueVerificationModel>(@"SELECT * 
             FROM [CellPhoneProject].[dbo].[AftersalesPm_HwIssueVerification] afi  where afi.IsActive=1
             and Id={0}", issueId).ToList();

            return query;
        }

        public List<AftersalesPm_HwIssuePlaceRequirement> GetPlaceRequirementDetails(long issueId)
        {
            var query = _dbeEntities.Database.SqlQuery<AftersalesPm_HwIssuePlaceRequirement>(@"SELECT * 
             FROM [CellPhoneProject].[dbo].[AftersalesPm_HwIssuePlaceRequirement] afi  where  MainId={0} order by Id desc", issueId).ToList();

            return query;
        }

        public string SaveForwardData(long ids, string forwardReason)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var query1 =
            (from mm in _dbeEntities.AftersalesPm_HwIssuePlaceRequirement where mm.MainId == ids select mm).OrderByDescending(mm => mm.Id).FirstOrDefault();

            query1.ForwardReason = forwardReason;
            query1.ReqStatus = "RequirementForwarded";
            query1.Updated = userId;
            query1.UpdatedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssuePlaceRequirement.AddOrUpdate(query1);
            _dbeEntities.SaveChanges();

            //main table
            var query =
                (from mm in _dbeEntities.AftersalesPm_HwIssueVerification where mm.Id == ids select mm).FirstOrDefault();

            query.Status = "RequirementForwarded";
            query.Updated = userId;
            query.UpdatedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssueVerification.AddOrUpdate(query);
            _dbeEntities.SaveChanges();

            //log table

            MapperConfiguration configuration = new MapperConfiguration(config => config.CreateMap<AftersalesPm_HwIssueVerification, AftersalesPm_HwIssueVerificationLog>());
            IMapper mapper = configuration.CreateMapper();
            var destination = mapper.Map<AftersalesPm_HwIssueVerification, AftersalesPm_HwIssueVerificationLog>(query);

            Mapper.CreateMap<AftersalesPm_HwIssueVerification, AftersalesPm_HwIssueVerificationLog>();

            destination.MainId = query.Id;
            destination.LogAdded = userId;
            destination.LogAddedDate = DateTime.Now;

            _dbeEntities.AftersalesPm_HwIssueVerificationLog.Add(destination);
            _dbeEntities.SaveChanges();
            ...
            if (ids > 0)
            {
                ...
                List<long> ids4 = new List<long>();
                var wpmsUsers = _dbeEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);
                //var wpmsUsers2 = _dbeEntities.CmnUsers.FirstOrDefault(i => i.RoleName == "HWHEAD" && i.IsActive == true);
                var wpmsUsers2 = (from map in _dbeEntities.CmnUsers
                                  where (map.RoleName == "HWHEAD" || map.RoleName == "HW") && map.IsActive == true
                                  select map).ToList();
                var wpmsUsers3 = (from map in _dbeEntities.CmnUsers
                ...
                body += "This is to inform you that requirement again forwarded by  <b>" + wpmsUsers.UserFullName
                    + "</b>. Issue details are given below : ";

                body += "<br/><br/> <b>Model Name :</b>" + query.ModelName + "<br/><b>Module :</b>" + query.Module + "<br/><b>Issue Details :</b>" + query.IssueDetails +
                     "<br/><b>Issue Type :</b>" + query.IssueType + "<br/><b>Result :</b>" + query.ResultFound + "<br/><b>Remarks :</b>" + query.Remarks +
                    "<br/><b>Status :</b>" + query.Status + "<br/><b>Forward Reason :</b>" + query1.ForwardReason + "<br/><br/><br/>";

                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(ids3, ids4, "Requirement forwarded by ASPM.", body);
        ...
        public string SaveAcceptedData(long ids)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);
            ...
            query1.ReqStatus = "RequirementAccepted";
            query1.Updated = userId;
            query1.UpdatedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssuePlaceRequirement.AddOrUpdate(query1);
            _dbeEntities.SaveChanges();
            ...
            query.Status = "RequirementAccepted";
            query.Updated = userId;
            query.UpdatedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssueVerification.AddOrUpdate(query);
            _dbeEntities.SaveChanges();
            ...
            if (ids > 0)
            {
                ...
                body += "This is to inform you that placed requirement has been accepted by  <b>" + wpmsUsers.UserFullName
                    + "</b>. Now you can close this issue. Issue details are given below : ";

                body += "<br/><br/> <b>Model Name :</b>" + query.ModelName + "<br/><b>Module :</b>" + query.Module + "<br/><b>Issue Details :</b>" + query.IssueDetails +
                     "<br/><b>Issue Type :</b>" + query.IssueType + "<br/><b>Result :</b>" + query.ResultFound + "<br/><b>Remarks :</b>" + query.Remarks +
                    "<br/><b>Status :</b>" + query.Status + "<br/><br/><br/>";

                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(ids3, ids4, "Requirement accepted by Aftersales Pm.", body);
        ...
        public string SaveClosedIssueData(long ids)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);
            ...
            if (query.Status == "RequirementAccepted")
            {
                query.Status = "RequirementClosed";
            }
            else
            {
                query.Status = "CLOSED";
            ...
            if (ids > 0)
            {
                ...
                {
                    body += "This is to inform you that issue has been closed by  <b>" + wpmsUsers.UserFullName
                    + "</b>. Issue details are given below : ";
                    ...
                    var mailSendFromPms = new MailSendFromPms();
                    mailSendFromPms.SendMail(ids3, ids4, "Issue has been closed by Aftersales Pm.", body);
                }
                else if (query.Status == "RequirementClosed")
                {
                    body += "This is to inform you that issue requirement has been closed by  <b>" + wpmsUsers.UserFullName
                          + "</b>. Issue details are given below : ";
                    ...
                    var mailSendFromPms = new MailSendFromPms();
                    mailSendFromPms.SendMail(ids3, ids4, "Issue Requirement has been closed by Aftersales Pm.", body);
                }
            }
            #endregion
        ...
        public List<AftersalesPm_HwIssueVerificationLogModel> GetLogDetailsPerIssue(long issueId)
        {
            var query = _dbeEntities.Database.SqlQuery<AftersalesPm_HwIssueVerificationLogModel>(@"select Id,MainId,Status,LogAddedDate as StartDate,lead(LogAddedDate,1,null) over (order by Id) EndDate,
            case when datediff(day,LogAddedDate,lead(LogAddedDate,1,null) over (order by Id)) is null then 0 else 
            datediff(day,LogAddedDate,lead(LogAddedDate,1,null) over (order by Id)) end as DateDiffs
            from [CellPhoneProject].[dbo].[AftersalesPm_HwIssueVerificationLog]  afl where afl.MainId={0}
            order by afl.Id asc
            ", issueId).ToList();
        ...
        public string SaveReportAcceptedData(long ids)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var query1 =
            (from mm in _dbeEntities.AftersalesPm_HwIssueReportShare where mm.MainId == ids select mm).OrderByDescending(mm => mm.Id).FirstOrDefault();

            query1.Status = "ReportAccepted";
            query1.Updated = userId;
            query1.UpdatedDate = DateTime.Now;
            _dbeEntities.AftersalesPm_HwIssueReportShare.AddOrUpdate(query1);
            _dbeEntities.SaveChanges();
            ...
            if (ids > 0)
            {
                ...
                body += "This is to inform you that shared report has been accept by  <b>" + wpmsUsers.UserFullName
                    + "</b>. Issue details are given below : ";

                body += "<br/><br/> <b>Model Name :</b>" + query.ModelName + "<br/><b>Module :</b>" + query.Module + "<br/><b>Issue Details :</b>" + query.IssueDetails +
                     "<br/><b>Issue Type :</b>" + query.IssueType + "<br/><b>Result :</b>" + query.ResultFound + "<br/><b>Remarks :</b>" + query.Remarks +
                    "<br/><b>Status :</b>" + "<br/><br/><br/>";

                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(ids3, ids4, "Shared report has been accepted by Aftersales Pm.", body);
        ...
        public string SaveReportForwardData(long ids, string forwardReason)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);
            ...
            if (ids > 0)
            {
                ...
                body += "This is to inform you that shared report has been forwarded by  <b>" + wpmsUsers.UserFullName
                    + "</b> for further checking. Issue details are given below : ";
                ...
                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(ids3, ids4, "Shared report has been forwarded by Aftersales Pm for further checking.", body);
        ...
        public List<AftersalesPm_HwIssueReportShareModel> GetReportShareDetailsForPerModel(long issueId)
        {
            var query = _dbeEntities.Database.SqlQuery<AftersalesPm_HwIssueReportShareModel>(@"SELECT * 
             FROM [CellPhoneProject].[dbo].[AftersalesPm_HwIssueReportShare] afi  where  MainId={0} order by Id desc", issueId).ToList();
