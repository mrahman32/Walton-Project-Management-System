...
namespace ProjectManagement.Infrastructures.Repositories
{
    public class CommonRepository : ICommonRepository
    {
        ...
        public List<ProjectMasterModel> GetProjectMasterModels()
        {
            var query =
                string.Format(
                    @"select * from ProjectMasters where ProjectStatus<>'DISCARDED' and ProjectStatus<>'REJECTED'");
            var v = _dbEntities.Database.SqlQuery<ProjectMasterModel>(query).ToList();
            return v;
        } 
        ...
        public List<SupplierKpiPerformanceModel> GetSupplierKpiPerformanceByProjectId(long? projectId)
        {
            ...
            var supplierName =
                _dbrbsyEntities.tblSupplierModelInfoes.Where(x => x.Model == variantName)
                    ...
                    .FirstOrDefault();
            var query = string.Format(@"select Model,supplierName,TotalReceive
,CONVERT(DECIMAL(4,2),(convert(decimal(10,0),TotalReceive)/convert(decimal(10,0),TotalActivation)*100)) as TotalReceivePercentage
,TotalUniqueServiceReceive,TotalActivation
from
(select 
sm.Model
,si.SupplierName
,Count(*) TotalReceive
,(Select sum(singleIme) 
...
,(select COUNT(*) 
   ...
   Group by sm.Model,si.supplierName) A", supplierName);
        ...
        public List<ProjectImage> GetProjectImages(long? projectId)
        {
            ...
            foreach (var img in v)
            {
                ...
                img.ImagePath = manager.GetPoFeedbackFile(img.ImagePath);
            }
            return v;
        } 
...
(select top 1 n.added from Notification n where n.message like '%{1}%' and n.ProjectMasterId={0})  as ForwardForFinalApproval,pm.FinalApprovalDate,
        ...
        public List<WSMTHandset> GetWSMTModels()
        {
            ...
            string username = "walton";
            string password = "walton@@##";
            string databaseName = "wsmt";
...
poqd.*
,(select ProjectName from CellPhoneProject.dbo.ProjectMasters where ProjectMasterId=poqd.ProjectMasterId) ProjectName
,(select OrderNuber from CellPhoneProject.dbo.ProjectMasters where ProjectMasterId=poqd.ProjectMasterId) OrderNumber
,(select OrderQuantity from CellPhoneProject.dbo.ProjectPurchaseOrderForms where ProjectMasterId=poqd.ProjectMasterId) TotalOrderQuantity
,(select UserFullName from CellPhoneProject.dbo.CmnUsers where CmnUserId=poqd.AddedBy) AddedByName
,(select UserFullName from CellPhoneProject.dbo.CmnUsers where CmnUserId=poqd.UpdatedBy) UpdatedByName
,(select UserFullName from CellPhoneProject.dbo.CmnUsers where CmnUserId=poqd.VariantClosingBy) VariantClosingByName
from CellPhoneProject.dbo.ProjectOrderQuantityDetails poqd");
            var d = _dbEntities.Database.SqlQuery<ProjectOrderQuantityDetailModel>(query).ToList();
            return d;
        }
        ...
        public ProjectOrderQuantityDetailModel SaveUpdateProjectVariantInOrderQuantityDetail(ProjectOrderQuantityDetailModel model)
        {
            ...
            model.VariantClosingByName =
                _dbEntities.CmnUsers.Where(x => x.CmnUserId == v.VariantClosingBy)
                    ...
                    .FirstOrDefault();
            //log data
            Mapper.CreateMap<ProjectOrderQuantityDetailModel, ProjectOrderQuantityDetailLog>();
            var log = Mapper.Map<ProjectOrderQuantityDetailLog>(model);
            log.OrderQuantityDetailsId = v.Id;
            log.LogAddedBy = v.UpdatedBy??v.AddedBy;
            log.LogAddedDate = DateTime.Now;
            _dbEntities.ProjectOrderQuantityDetailLogs.Add(log);
            _dbEntities.SaveChanges();
            return model;
        }
        ...
        public bool DeleteVariant(long id)
        {
            try
            {
                var v = _dbEntities.ProjectOrderQuantityDetails.FirstOrDefault(x => x.Id == id);
                _dbEntities.ProjectOrderQuantityDetails.Remove(v);
                _dbEntities.SaveChanges();
                return true;
            }
            catch (Exception e)
...
order by pm.OrderNuber",
        ...
        public void SaveLcOpeningApprovalDetail(LcOpeningApprovalDetail mod)
        {
            var rolDesc =
                _dbEntities.CmnRoles.Where(x => x.RoleName == mod.Role).Select(x => x.RoleDescription).FirstOrDefault();
            mod.RoleDescription = rolDesc;
            _dbEntities.LcOpeningApprovalDetails.Add(mod);
            _dbEntities.SaveChanges();
        }

        public void SaveLcOpeningApprovalOtherProductDetail(LcOpeningApprovalOtherProductDetail mod)
        {
            var rolDesc =
                _dbEntities.CmnRoles.Where(x => x.RoleName == mod.Role).Select(x => x.RoleDescription).FirstOrDefault();
            mod.RoleDescription = rolDesc; 
            _dbEntities.LcOpeningApprovalOtherProductDetails.Add(mod);
            _dbEntities.SaveChanges();
        }
        ...
        public List<LcOpeningPermissionModel> RelevantModelPickPendingProjectsForLc()
        {
            string query =
                string.Format(
                    @"SELECT ProjectMasterId,CONCAT(Model,' (Order ',OrderNo,')') as Model FROM [CellPhoneProject].[dbo].[LCOpeningPermission] where ApprovedBy is null and IsActive=1 group by ProjectMasterId,Model,OrderNo");
        ...
        public List<MaterialWastageMasterModel> GetMetarialWastageList()
        {
            List<MaterialWastageMasterModel> models = (from materialWastageMaster in _dbEntities.MaterialWastageMasters
                                                       ...
                                                       join dcooUser in _dbEntities.CmnUsers on materialWastageMaster.DeputyCooId equals dcooUser.CmnUserId into dcoo
                                                       from dcooUser in dcoo.DefaultIfEmpty()
                                                       ...
                                                       select new MaterialWastageMasterModel
                                                       {
                                                           ...
                                                           IsDeputyCooApproved = materialWastageMaster.IsDeputyCooApproved,
                                                           DeputyCooApprovername = dcooUser.UserFullName,
        ...
        public List<MaterialWastageMasterModel> GetPendingApprovals(int approvalStage, string identityName)
        {
            long id = Convert.ToInt64(identityName);
            var sessionUser = HttpContext.Current.User;
            CmnUser user = _dbEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == id);
            //bool isApproved = _dbEntities.MaterialWastageRecommendations.Any(
            //            i => i.AddedBy == user.CmnUserId && i.RecommendationType == "APPROVED");
            if (approvalStage == 1)
            {
                ...
                if (user != null)
                {
                    if (sessionUser.IsInRole("WASSHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("assembly"));
                        if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WACCHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("charger"));

                         if (type != null) types.Add(type.Id);
                    }
                    if ( sessionUser.IsInRole("WSMTHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("smt"));
                         if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WCHAHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("charger"));
                    ...
                    if (sessionUser.IsInRole("WBATHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("battery"));
                         if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WHOUHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("housing"));
                         if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WEARHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("earphone"));
                         if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WUSBHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("usb"));
                         if (type != null) types.Add(type.Id);
                    }
                }
                ...
                List<MaterialWastageMasterModel> models = (from materialWastageMaster in _dbEntities.MaterialWastageMasters
                                                           ...
                                                           select new MaterialWastageMasterModel
                                                           {
                                                               ...
                                                               SpecialApproverName = specialUser.UserFullName,
                                                               YearNumber = materialWastageMaster.YearNumber,
                                                               IsEligble = materialWastageMaster.MaterialWastageDetails.Any(i => types.Contains((long)i.BOMTypeId) )
                                                           }
                    ).ToList();


                List<MaterialWastageMasterModel> filteredList = new List<MaterialWastageMasterModel>();
                foreach (var itr in models)
                {
                    bool isApproved =
                        _dbEntities.MaterialWastageRecommendations.Any(
                            i => i.MaterialWastageMasterId == itr.Id && i.AddedBy == id);
                    if (!isApproved)
                    {
                        filteredList.Add(itr);
                    }
                }
            ...
            if (approvalStage == 4)
            {
                ...
                List<MaterialWastageMasterModel> models = (from materialWastageMaster in _dbEntities.MaterialWastageMasters
                                                           ...
                                                           from addedUser in addedU.DefaultIfEmpty()
                                                           //join projectOrderQuantityDetail in _dbEntities.ProjectOrderQuantityDetails on materialWastageMaster.ProjectOrderQuantityDetailsId equals projectOrderQuantityDetail.Id
                                                           where materialWastageMaster.IsInchargeApproved == true &&
                                                          (materialWastageMaster.IsDeputyCooApproved == true || materialWastageMaster.IsCooApproved == true)
                                                          && (materialWastageMaster.IsManagementApproved == false || materialWastageMaster.IsManagementApproved == null)
        ...
        EditMaterialWastageViewModel ICommonRepository.GetMaterialWastageById(long id)
        {
            var userId = Convert.ToInt64(HttpContext.Current.User.Identity.Name);
            var sessionUser = HttpContext.Current.User;
            CmnUser user = _dbEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);
            List<long> types = new List<long>();
            if (user != null)
            {
                ...
                if (user != null)
                {
                    ...
                    if (sessionUser.IsInRole("WLENSHEAD"))
                    {
                        MaterialWastageItemType type =
                            _dbEntities.MaterialWastageItemTypes.FirstOrDefault(
                                i => i.ItemType.ToLower().Contains("lens"));
                        if (type != null) types.Add(type.Id);
                    }
                    if (sessionUser.IsInRole("WBATHEAD"))
                    {
            ...
            var model = new EditMaterialWastageViewModel();
            _dbEntities.Configuration.LazyLoadingEnabled = true;
            _dbEntities.Database.CommandTimeout = 600;
            ...
            List<EditMaterialDetail> editMaterialDetails = new List<EditMaterialDetail>();

            List<long?> materialTypeIds = master.MaterialWastageDetails.Select(i => i.BOMTypeId).Distinct().ToList();

            foreach (var materialTypeId in types.Any() ? materialTypeIds.Where(i => types.Contains((long) i)).ToList() : materialTypeIds)
            {
                var editMaterial = new EditMaterialDetail();
                var details = master.MaterialWastageDetails.Where(i => i.BOMTypeId == materialTypeId && (i.IsDeleted == null || i.IsDeleted == false)).ToList();

                if (details.Any())
                {
                    editMaterial.MaterialWastageDetails = details;
                    editMaterial.MaterialTypeId = Convert.ToInt64(materialTypeId);
                    editMaterial.MasterId = master.Id;
                    var materialWastageItemType = _dbEntities.MaterialWastageItemTypes.FirstOrDefault(i => i.Id == materialTypeId);
                    if (materialWastageItemType != null)
                        editMaterial.MaterialTypeName = materialWastageItemType.ItemType;

                    editMaterial.Average1 = Math.Round(master.MaterialWastageDetails.Sum(i => i.ActualAssemblyWastage_TotalLot) / master.MaterialWastageDetails.Count(i => i.ActualAssemblyWastage_TotalLot >= 0), 2);

                    editMaterial.Average2 = Math.Round(master.MaterialWastageDetails.Sum(i => i.ActualRepairWastage_TotalLot) / master.MaterialWastageDetails.Count(i => i.ActualRepairWastage_TotalLot >= 0), 2);

                    editMaterial.Average3 = Math.Round(master.MaterialWastageDetails.Sum(i => i.ActualWastageOfTotalLot) / master.MaterialWastageDetails.Count(i => i.ActualWastageOfTotalLot >= 0), 2);

                    editMaterial.Average4 = Math.Round(master.MaterialWastageDetails.Sum(i => i.NetAdjustment) / master.MaterialWastageDetails.Count(i => i.NetAdjustment >= 0), 2);


                    editMaterialDetails.Add(editMaterial);
                }
            }
            model.EditMaterialDetails = editMaterialDetails;
        ...
        public ResponseModel RecommendMaterialWastage(long id, bool isRecom, string recomMsg, bool isApproved, string approvedMsg, IPrincipal user, int recommenderType)
        {
            try
            {
                ...
                var sessionUser = HttpContext.Current.User;
                CmnUser cmnUser = _dbEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);
                ...
                if (cmnUser != null)
                {
                    ...
                    else if (sessionUser.IsInRole("WASCOO") || sessionUser.IsInRole("WASDCOO") || sessionUser.IsInRole("MM"))
                    {
                        if (master != null)
                        {
                            List<long?> typeIdList = master.MaterialWastageDetails
                                .Where(i => i.MaterialWastageMasterId == id && i.IsDeleted == null && i.BOMTypeId != null)
                                .Select(i => i.BOMTypeId).ToList();
                            typeIdList = typeIdList.Distinct().ToList();
                            types.AddRange(typeIdList.Select(tp => Convert.ToInt64(tp)));
                        }
                    }
                }
                ...
                if (master != null)
                {
                    ...
                    else if (isApproved)
                    {
                        switch (recommenderType)
                        {
                            case 1:
                                Thread.Sleep(300);
                                int materialTypeCount = master.MaterialWastageDetails.Where(i=>i.MaterialWastageMasterId == id && i.IsDeleted==null).Select(i => i.BOMTypeId).Distinct().Count();
                                int inchCount = master.MaterialWastageRecommendations.Count(
                                        i => i.UserType == "INCHARGE" && i.RecommendationType == "APPROVED" && i.MaterialWastageMasterId == id);

                                if (materialTypeCount > inchCount && materialTypeCount == inchCount + types.Count)
                                {
                ...
                if (recommendation != null)
                {
                    ...
                    {
                        recommendation.MaterialTypeId = type;
                        var newWastageRecommendation = new MaterialWastageRecommendation
                        {
                            AddedBy = recommendation.AddedBy,
                            AddedDate = recommendation.AddedDate,
                            MaterialTypeId = type,
                            MaterialWastageMasterId = recommendation.MaterialWastageMasterId,
                            RecommendationType = recommendation.RecommendationType,
                            RecommendedBy = recommendation.RecommendedBy,
                            Remarks = recommendation.Remarks,
                            UserType = recommendation.UserType
                        };
                        _dbEntities.MaterialWastageRecommendations.Add(newWastageRecommendation);
                    }
...
//                ", projectName, orders).ToList();
                query = _dbEntities.Database.SqlQuery<WpmsAllProjectDetailsModel>(@"
                 select ProjectMasterId,ProjectName,ProjectModel,Orders,IsActiveOrder,ProjectCreationDate,SupplierName,ProjectStatus,ProjectManagerName,MarketClearanceDate,IsActiveOrder,
                OrderQuantity,SupplierTrustLevel,ProjectType,OsName,OsVersion,DisplaySize,DisplayName,ProcessorName,ProcessorClock,Chipset,FrontCamera,FrontCam,
                BackCam,BackCamera,Ram,Rom,Battery,SimSlotNumber,ProjectNameForScreening,PcbaFinalVendor,PcbaVendorName,DisplayResulution,Color,BatteryType,BatteryRating,SecondGen,
                case when FlashLight=1 then 'Yes' when FlashLight=0 then 'No' end as FlashLight,BatteryCoverLogoType,SpecialSensor,case when Gps=1 then 'Yes' when Gps=0 then 'No' end as Gps,
                case when Otg=1 then 'Yes' when Otg=0 then 'No' end as Otg,case when HallSensor=1 then 'Yes' when HallSensor=0 then 'No' end as HallSensor,
                case when Gyroscope=1 then 'Yes' when Gyroscope=0 then 'No' end as Gyroscope,case when Compass=1 then 'Yes' when Compass=0 then 'No' end as Compass,
                case when Lsensor=1 then 'Yes' when Lsensor=0 then 'No' end as Lsensor,case when Psensor=1 then 'Yes' when Psensor=0 then 'No' end as Psensor,
                case when Gsensor=1 then 'Yes' when Gsensor=0 then 'No' end as Gsensor,ChipsetCore,ChipsetBit,ChipsetFrequency,ChipsetName,CpuName,FrontCamBsi,FrontCamSensor,PoDate,InitialApprovalPendings
                from 
                (
	                    ...
	                    pm.BatteryRating,pm.SecondGen,pm.FlashLight,pm.BatteryCoverLogoType,pm.SpecialSensor,pm.Gps,pm.Otg,pm.HallSensor,pm.Gyroscope,pm.Compass,
	                    pm.Lsensor,pm.Psensor,pm.Gsensor,pm.ChipsetCore,pm.ChipsetBit,pm.ChipsetFrequency,pm.ChipsetName,pm.CpuName,pm.FrontCamBsi,pm.FrontCamSensor,
	                    case when cast(ppo.PoDate as varchar(50)) is null then 'PO Pending' else cast(ppo.PoDate as varchar(50)) end as PoDate, 
                        
                        case when pm.ProjectStatus='NEW' and pm.BiApprovalBy is null and pm.InitialApprovalBy is null and pm.PsApprovalBy is null and pm.CeoApprovalBy is null then 'Action Not taken By BI' 
						when pm.ProjectStatus='NEW' and pm.BiApprovalBy is not null and pm.PsApprovalBy is null then 'Action Not taken By Management Coordinator'
						when pm.ProjectStatus='NEW' and pm.CeoApprovalBy is null and pm.PsApprovalBy is not null and pm.InitialApprovalBy is null  then 'Action Not taken By CBO'
	                    when pm.ProjectStatus='NEW' and pm.InitialApprovalBy is null and pm.CeoApprovalBy is not null and pm.PsApprovalBy is not null then 'Action Not taken By MD' 
	                    when pm.ProjectStatus='SWOTPENDING' and pm.BiApprovalBy is null and pm.PsApprovalBy is null and pm.InitialApprovalBy is null and pm.CeoApprovalBy is null then 'SWOT analysis Pending'
	                    else '----' end as InitialApprovalPendings
	                    ...
	                    pm.BatteryRating,pm.SecondGen,pm.FlashLight,pm.BatteryCoverLogoType,pm.SpecialSensor,pm.Gps,pm.Otg,pm.HallSensor,pm.Gyroscope,pm.Compass,
	                    pm.Lsensor,pm.Psensor,pm.Gsensor,pm.ChipsetCore,pm.ChipsetBit,pm.ChipsetFrequency,pm.ChipsetName,pm.CpuName,pm.FrontCamSensor,ppo.PoDate,
	                    pm.InitialApprovalBy,pm.PsApprovalBy,pm.BiApprovalBy,pm.CeoApprovalBy
        ...
        #endregion
        #region LC Approval Mnanager

        public List<string> RoleDescriptionList()
        {
            var query =
                string.Format(
                    @"select RoleDescription from [CellPhoneProject].[dbo].[CmnRoles] group by RoleDescription");
            var v = _dbEntities.Database.SqlQuery<string>(query).ToList();
            return v;
        }

        public void SaveLcApprovalManager(LcApprovalManagerModel model)
        {
            Mapper.CreateMap<LcApprovalManagerModel, LcApprovalManager>();
            var v = Mapper.Map<LcApprovalManager>(model);
            _dbEntities.LcApprovalManagers.AddOrUpdate(v);
            _dbEntities.SaveChanges();
        }

        public List<LcApprovalManagerModel> GetLcApprovalManagerModels()
        {
            var query = string.Format(@"select * from LcApprovalManagers");
            var v = _dbEntities.Database.SqlQuery<LcApprovalManagerModel>(query).ToList();
            return v;
        }

        public List<string> GetRoleByDescription(string desc)
        {
            var v = _dbEntities.CmnRoles.Where(x => x.RoleDescription == desc).Select(x => x.RoleName).ToList();
            return v;
        }

        public string GetDescriptionByRole(string role)
        {
            var v = _dbEntities.CmnRoles.Where(x => x.RoleName == role).Select(x => x.RoleDescription).FirstOrDefault();
            return v;
        }

        public LcApprovalManagerModel GetLcApprovalManagerByRole(string role)
        {
            var desc = _dbEntities.CmnRoles.Where(x => x.RoleName == role).Select(x => x.RoleDescription).ToList();
            var query = string.Format(@"select * from LcApprovalManagers where RoleDescription='{0}'",desc);
            var v = _dbEntities.Database.SqlQuery<LcApprovalManagerModel>(query).FirstOrDefault();
            return v;
        }

        public bool CheckInitiationApprovalByRoleDesc(string roleDesc,long permId)
        {
            var v = _dbEntities.LcOpeningApprovalDetails.Any(x => x.RoleDescription == roleDesc && x.LcOpeningPermissionId==permId);
            return v;
        }

        public List<LcOpeningPermissionModel> GetOpeningPermissionByRoleDesc(string roleDesc)
        {
            var query = string.Format(@" select * from LCOpeningPermission lop
 where not exists (select * from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.Id and RoleDescription='{0}')", roleDesc);
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionModel>(query).ToList();
            return v;
        }

        public List<LcOpeningPermissionOtherProductModel> GetOtherProdPermByRoleDesc(string roleDesc)
        {
            var query = string.Format(@" select * from LcOpeningPermissionOtherProducts lop
 where not exists (select * from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.Id and RoleDescription='{0}')", roleDesc);
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionOtherProductModel>(query).ToList();
            return v;
        }

        public string GetInitiationApprovedNameByLcId(long lcId)
        {
            var s="";
            var v = _dbEntities.LcOpeningApprovalDetails.Where(x => x.LcOpeningPermissionId == lcId).ToList();
            foreach (var i in v)
            {
                s = s + i.ApprovedByName + ",";
            }
            return s;
        }
        #endregion

        public List<ProjectImageModel> GetProjectImages()
        {
            List<ProjectImage> projectImages = _dbEntities.ProjectImages.ToList();
