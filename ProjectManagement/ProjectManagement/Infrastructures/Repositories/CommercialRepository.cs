...
namespace ProjectManagement.Infrastructures.Repositories
{
    public class CommercialRepository : ICommercialRepository
    {
        ...
        public long UpdateProjectShipment(ProjectOrderShipmentModel model, long userId, List<ProjectMasterModel> issueList1)
        {
            ...
            shipment.Remarks = model.Remarks;
                    ...
                    if (model.ApproximateFinishDateForReorder != null)
                    {
                        ...
                        if (projectMaster != null)
                        {
                            ...
                            if (order)
                            {
                                ...
                                master.BatteryCategory = model.ProjectPurchaseOrderFormModel.BatteryCategory;
                                //-----o------
        ...
        public decimal UpdateProject(ProjectMasterModel model, long userId)
        {
            try
            {
                ...
                if (projectMaster != null)
                {
                    ...
                    projectMaster.BatteryCategory = model.BatteryCategory;
...
order by Id desc").Take(100).ToList();
        ...
        public List<VmIncentivePolicy> GetPrimarySales(long monIds, long yearIds)
        {
            ...
            //string oradb = "Data Source=(DESCRIPTION="
            //                              + "(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.100.157)(PORT=1521))"
            //                              + "(CONNECT_DATA=(SERVICE_NAME=PROD)));"
            //                              + "User Id=appswg;Password=appswg#;";

            string oradb = "Data Source=(DESCRIPTION="
                                        + "(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.119.90)(PORT=1521))"
                                        + "(CONNECT_DATA=(SERVICE_NAME=PROD)));"
                                        + "User Id=appswg;Password=appswg#;";
            // OracleConnection con = OracleDbConnection.GetOldConnection();//150.46
        ...
        public List<VmIncentivePolicy> GetFeaturePhoneService(long monIds, long yearIds)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                and (sm.Model like '%Olvio%' or sm.Model like '%Axino%' or sm.Model like '%Excel%')
        ...
        public List<VmIncentivePolicy> GetFeaturePhoneSales(long monIds, long yearIds)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                from RBSYNERGY.[dbo].tblDealerDistributionDetails tdd where DATEPART(mm,tdd.DistributionDate)={0}
                and DATENAME(YEAR,tdd.DistributionDate)={1} and (tdd.Model like '%Olvio%' or tdd.Model like '%Axino%' or tdd.Model like '%Excel%')
        ...
        public string GetSaveIncentive(string month, string monNum, string year, string totalAmount, List<Incentive> results, List<CmIncentiveModel> results2)
        {
            ...
            {
                var mods = new SourcingIncentiveBreakdown();

                if (insResult.orders == "Orders")
                {
                    mods.IncentiveTypes = insResult.orders;
                    mods.IncentiveQtyOrPercent = insResult.parameter;
                    mods.IncentiveValue = insResult.parameter_Value;
                    mods.IncentiveTotalSum = insResult.orders_Value;

                    bool isExist = CheckSourcingIncentiveBreakdown(monNum, year, mods.IncentiveTypes);
                    if (!isExist)
                    {
                        ...
                        _dbEntities.SourcingIncentiveBreakdowns.Add(mods);
                        _dbEntities.SaveChanges();
                    }
                }
                if (insResult.primarySales == "Primary Sales Target")
                {
                    mods.IncentiveTypes = insResult.primarySales;
                    mods.IncentiveQtyOrPercent = insResult.primaryParameter;
                    mods.IncentiveValue = insResult.primaryParameter_value;
                    mods.IncentiveTotalSum = insResult.primaryParameter_Total_value;
                ...
                if (insResult.featureRatio == "Feature Phone Service Ratio")
                {
                    mods.IncentiveTypes = insResult.featureRatio;
                    mods.IncentiveQtyOrPercent = insResult.serviceToSalesForFeaturePhone;
                    mods.IncentiveValue = insResult.primaryParameterValueForFeaturePhone;
                    mods.IncentiveTotalSum = insResult.finalServiceToSalesRatioForFeature;
                ...
                if (insResult.smartRatio == "Smart/Tablet Phone Service Ratio")
                {
                    mods.IncentiveTypes = insResult.smartRatio;
                    mods.IncentiveQtyOrPercent = insResult.serviceToSalesForSmartPhone;
                    mods.IncentiveValue = insResult.primaryParameterValueForSmartPhone;
                    mods.IncentiveTotalSum = insResult.finalServiceToSalesRatioForSmart;
                ...
                if (insResult.totalIns == "Total Incentive Amount")
                {
                    mods.IncentiveTypes = insResult.totalIns;
                    mods.IncentiveTotalSum = insResult.totalIncentiveAmount;
        ...
        public bool CheckSourcingIncentiveBreakdown(string monNum, string year, string IncentiveTypes)
        {
            int MonNum = Convert.ToInt32(monNum);
            List<VmIncentivePolicy> getIncentiveReports = null;
            if (IncentiveTypes!=null)
            {
                ...
                {
                    string getIncentiveReportQuery = string.Format(@"
                 select MonNum,CONVERT(varchar(10),Year) from [CellPhoneProject].[dbo].[SourcingIncentiveBreakdown] where  Year={1} and  MonNum={0} and IncentiveTypes='{2}' ", MonNum, year, IncentiveTypes);
                    getIncentiveReports =
                       _dbEntities.Database.SqlQuery<VmIncentivePolicy>(getIncentiveReportQuery).ToList();
                }
            }
        ...
        public List<VmIncentivePolicy> GetIncentiveReport(string monId, long yearIds)
        {
            ...
            (	
	            ...
	            from CellPhoneProject.dbo.CmnUsers cm left join CellPhoneProject.dbo.Incentive ii on cm.EmployeeCode=ii.UserId
	            where cm.EmployeeCode=ii.UserId and ii.DepartmentName in ('Sourcing') and cm.IsActive=1 
        ...
        public List<VmIncentivePolicy> GetIncentiveReport2(string monId, long yearIds)
        {
            int MonNum = Convert.ToInt32(monId);
            ...
            (	
	            ...
	            ii.TotalPenalties,ii.Penalties,cast(ii.SpecialAmount as decimal(18,2)) as  SpecialAmount,ii.SpecialRemarks
	            from CellPhoneProject.dbo.CmnUsers cm left join CellPhoneProject.dbo.Incentive ii on cm.EmployeeCode=ii.UserId
	            where cm.EmployeeCode=ii.UserId and ii.DepartmentName in ('CM','CMHEAD') and cm.IsActive=1 
	            and ii.MonNum={0}  and ii.Year={1} 
            )A order by Percentage desc", MonNum, yearIds);
        ...
        public bool GetCheckDate(string monId, string yearId)
        {
            ...
            {
                string getIncentiveReportQuery = string.Format(@"
             select MonNum,CONVERT(varchar(10),Year) from [CellPhoneProject].[dbo].[Incentive] where DepartmentName in ('Sourcing') and Year={1} and  MonNum={0}", MonNum, yearId);
                getIncentiveReports =
                   _dbEntities.Database.SqlQuery<VmIncentivePolicy>(getIncentiveReportQuery).ToList();
   ...
   string status)
        ...
        public List<VmIncentivePolicy> GetPrimarySalesNew(long monIds, long yearIds)
        {
            ...
            string oradb = "Data Source=(DESCRIPTION="
                                            + "(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.150.49)(PORT=1521))"
                                            + "(CONNECT_DATA=(SERVICE_NAME=PROD)));"
                                            + "User Id=appswq;Password=appswq#;";
...
and YEAR(DATEADD(MONTH,CASE WHEN (select top 1 ProjectType from ProjectMasters where projectName='{0}')='Smart' THEN 4 ELSE 3 END,CONVERT(datetime2,'{1}',102)))=YEAR(DATEADD(MONTH,CASE WHEN pm.ProjectType='Smart' THEN 4 ELSE 3 END,ppf.PoDate))", projectName, poDate.ToString("yyyy-MM-dd"));
        ...
        public List<LcOpeningPermissionModel> GetLcPermissionList()
        {
            ...
            //    //InitiationApprovedBy = GetInitiationApprovedNameByLcId(v.Id)
            //}).ToList();
            var query =
                string.Format(
                    @"select * ,(select UserFullName from CmnUsers where CmnUserId=lc.AddedBy) as AddedByName,(select UserFullName from CmnUsers where CmnUserId=lc.ApprovedBy) as ApprovedByName,(select UserFullName from CmnUsers where CmnUserId=lc.SourcingApprovalBy) as SourcingApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.CeoApprovalBy) as CeoApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.FinanceApprovalBy) as FinanceApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.AccountsApprovalBy) as AccountsApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.BiApprovalBy) as BiApprovalByName from LCOpeningPermission lc WHERE lc.IsActive=1 or lc.IsActive is null");
            var lcPermissionModel = _dbEntities.Database.SqlQuery<LcOpeningPermissionModel>(query).ToList();
            //foreach (var i in lcPermissionModel)
            //{
            //    var v = _dbEntities.LcOpeningApprovalDetails.Where(x => x.LcOpeningPermissionId == i.Id).ToList();
            //    foreach (var j in v)
            //    {
            //        i.InitiationApprovedBy = i.InitiationApprovedBy + j.ApprovedByName + ",";
            //    }
            //}
            return lcPermissionModel;
        ...
        public List<LcOpeningPermissionModel> GetLcOpeningPermWithInitiationDone()
        {
            //STRING_AGG works from sql server 2017
            var query = string.Format(@" select lop.*,(select STRING_AGG(ApprovedByName,',') from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.id) as InitiationApprovedBy from LCOpeningPermission lop
 where exists (select * from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Project Manager')
 and exists (select * from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.Id and RoleDescription='ProductionPlanning')
 and exists (select * from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Hardware')
 and exists (select * from LcOpeningApprovalDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Production')");
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionModel>(query).ToList();
            return v;
        }

        public List<LcOpeningPermissionOtherProductModel> GetLcOpeningOtherPermWithInitiationDone()
        {
            //STRING_AGG works from sql server 2017
            var query = string.Format(@" select lop.*,(select STRING_AGG(ApprovedByName,',') from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.id) as InitiationApprovedBy from LcOpeningPermissionOtherProducts lop
 where exists (select * from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Project Manager')
 and exists (select * from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.Id and RoleDescription='ProductionPlanning')
 and exists (select * from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Hardware')
 and exists (select * from LcOpeningApprovalOtherProductDetails where LcOpeningPermissionId=lop.Id and RoleDescription='Production')");
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionOtherProductModel>(query).ToList();
            return v;
        }

        public bool CheckDupePo(string oraclePoNo)
        {
            var v = _dbEntities.LCOpeningPermissions.Any(x => x.OraclePoNo == oraclePoNo);
            return v;
        }

        public bool CheckDupePoOtherProduct(string oraclePoNo)
        {
            var v = _dbEntities.LcOpeningPermissionOtherProducts.Any(x => x.OraclePoNo == oraclePoNo);
            return v;
        }

        public List<LcOpeningPermissionOtherProductModel> GetLcPermissionOtherProductList()
        {
            var query = string.Format("select * ,(select UserFullName from CmnUsers where CmnUserId=lc.AddedBy) as AddedByName,(select UserFullName from CmnUsers where CmnUserId=lc.ApprovedBy) as ApprovedByName,(select UserFullName from CmnUsers where CmnUserId=lc.SourcingApprovalBy) as SourcingApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.CeoApprovalBy) as CeoApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.FinanceApprovalBy) as FinanceApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.AccountsApprovalBy) as AccountsApprovalByName,(select UserFullName from CmnUsers where CmnUserId=lc.BiApprovalBy) as BiApprovalByName from LcOpeningPermissionOtherProducts lc");
            ...
            //        .Select(x => x.UserFullName).FirstOrDefault();
            //    //initiation done by names
            //    var v = _dbEntities.LcOpeningApprovalOtherProductDetails.Where(x => x.LcOpeningPermissionId == l.Id).ToList();
            //    foreach (var j in v)
            //    {
            //        l.InitiationApprovedBy = l.InitiationApprovedBy + j.ApprovedByName + ",";
            //    }
            //}
            return list;
        }
        ...
        public long SaveLcOpeningOtherProduct(LcOpeningPermissionOtherProductModel m)
        {
            ...
            _dbEntities.SaveChanges();
            //add log
            Mapper.CreateMap<LcOpeningPermissionOtherProductModel, LcOpeningPermissionOtherProductsLog>();
            var log = Mapper.Map<LcOpeningPermissionOtherProductsLog>(m);
            log.LcId = v.Id;
            log.LogAddedBy = v.UpdatedBy ?? v.AddedBy;
            log.LogAddedDate = DateTime.Now;
            _dbEntities.LcOpeningPermissionOtherProductsLogs.Add(log);
            _dbEntities.SaveChanges();
            return v.Id;
        }
        ...
        public LcOpeningPermissionDetailModel SaveOrUpdateLcOpeningPermissionDetail(LcOpeningPermissionDetailModel model)
        {
            Mapper.CreateMap<LcOpeningPermissionDetailModel, LcOpeningPermissionDetail>();
            var v = Mapper.Map<LcOpeningPermissionDetail>(model);
            _dbEntities.LcOpeningPermissionDetails.AddOrUpdate(v);
            _dbEntities.SaveChanges();
            //save in log
            Mapper.CreateMap<LcOpeningPermissionDetailModel, LcOpeningPermissionDetailsLog>();
            var log = Mapper.Map<LcOpeningPermissionDetailsLog>(model);
            log.LcDetailId = v.Id;
            log.LogAddedDate = DateTime.Now;
            log.LogAddedBy = v.Updated ?? v.Added;
            _dbEntities.LcOpeningPermissionDetailsLogs.Add(log);
            _dbEntities.SaveChanges();
            //-----
            model.Id = v.Id;
            return model;
        ...
        public List<LcOpeningPermissionDetailModel> GetLcPermissionDetailByPermId(long permissionId)
        {
            string query = string.Format(@"SELECT * FROM LcOpeningPermissionDetails WHERE PermissionId={0}",
                permissionId);
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionDetailModel>(query).ToList();
            return v;
        }

        public LcOpeningPermissionDetailModel GetLcPermissionDetailById(long id)
        {
            string query = string.Format(@"SELECT * FROM LcOpeningPermissionDetails WHERE Id={0}",
                id);
            var v = _dbEntities.Database.SqlQuery<LcOpeningPermissionDetailModel>(query).FirstOrDefault();
            return v;
        }
        #endregion
        ...
        public void SaveBtrcSpec(ForeignModelInfoFromBtrcModel model)
        {
            var v = _dbEntities.ForeignModelInfoFromBtrcs.Any(x => x.Imei1 == model.Imei1);
            if (!v)
            {
                Mapper.CreateMap<ForeignModelInfoFromBtrcModel, ForeignModelInfoFromBtrc>();
                var save = Mapper.Map<ForeignModelInfoFromBtrc>(model);
                _dbEntities.ForeignModelInfoFromBtrcs.Add(save);
                _dbEntities.SaveChanges();
            }
        }

        public List<ForeignModelInfoFromBtrcModel> GetInfoFromBtrc()
        {
            var query = string.Format(@"SELECT * FROM ForeignModelInfoFromBtrcs");
            var v = _dbEntities.Database.SqlQuery<ForeignModelInfoFromBtrcModel>(query).ToList();
            return v;
        }
        #endregion
...
order by cast(UpdatedDate as Date)", projectId);
        ...
        public List<ProjectForecastingInfoModel> GetProjectForecastingSum()
        {
            var query = string.Format(@"select * from
            (
	            select Serial=1,Brand,FSegment,sum(January) January,sum(February) February,sum(March) March,sum(April) April,sum(May) May,
	            sum(June) June,sum(July) July,sum(August) August,sum(September) September,
	            sum(October) October,sum(November) November,sum(December) December,sum(TotalSum) as TotalSum
	            from
	            (
		            select Brand,FSegment,case when January is null then 0 else January end as January,case when February is null then 0 else February end as February,
		            case when March is null then 0 else March end as March,case when April is null then 0 else April end as April,
		            case when May is null then 0 else May end as May,case when June is null then 0 else June end as June,
		            case when July is null then 0 else July end as July,case when August is null then 0 else August end as August,
		            case when September is null then 0 else September end as September,case when October is null then 0 else October end as October,
		            case when November is null then 0 else November end as November,case when December is null then 0 else December end as December,
		            (January+February+March+April+May+June+July+August+September+October+November+December) as TotalSum
		            from [CellPhoneProject].[dbo].[ProjectForecastingInfo]  where NULLIF(FSegment, '') IS not NULL
		            group by FSegment,Brand,January,February,March,April,May,June,July,August,September,October,November,December
	            )A group by FSegment,Brand

	            union

	            select Serial=2,Brand,FSegment,sum(January) January,sum(February) February,sum(March) March,sum(April) April,sum(May) May,
	            sum(June) June,sum(July) July,sum(August) August,sum(September) September,
	            sum(October) October,sum(November) November,sum(December) December,sum(TotalSum) as TotalSum
	            from
	            (
		            select Brand='Total FP',FSegment,case when January is null then 0 else January end as January,case when February is null then 0 else February end as February,
		            case when March is null then 0 else March end as March,case when April is null then 0 else April end as April,
		            case when May is null then 0 else May end as May,case when June is null then 0 else June end as June,
		            case when July is null then 0 else July end as July,case when August is null then 0 else August end as August,
		            case when September is null then 0 else September end as September,case when October is null then 0 else October end as October,
		            case when November is null then 0 else November end as November,case when December is null then 0 else December end as December,
		             (January+February+March+April+May+June+July+August+September+October+November+December) as TotalSum
		            from [CellPhoneProject].[dbo].[ProjectForecastingInfo] where FSegment='FP'
		            group by FSegment,Brand,January,February,March,April,May,June,July,August,September,October,November,December
	            )A group by FSegment,Brand

	            union

	            select Serial=3,Brand,FSegment,sum(January) January,sum(February) February,sum(March) March,sum(April) April,sum(May) May,
	            sum(June) June,sum(July) July,sum(August) August,sum(September) September,
	            sum(October) October,sum(November) November,sum(December) December,sum(TotalSum) as TotalSum
	            from
	            (
		            select Brand='Total',FSegment='FP+SP',case when January is null then 0 else January end as January,case when February is null then 0 else February end as February,
		            case when March is null then 0 else March end as March,case when April is null then 0 else April end as April,
		            case when May is null then 0 else May end as May,case when June is null then 0 else June end as June,
		            case when July is null then 0 else July end as July,case when August is null then 0 else August end as August,
		            case when September is null then 0 else September end as September,case when October is null then 0 else October end as October,
            ...
            )B where NULLIF(FSegment, '') IS not NULL
            order by Serial asc
            ");
            var v = _dbEntities.Database.SqlQuery<ProjectForecastingInfoModel>(query).ToList();
            return v;
        }

        public List<ProjectMasterModel> GetAllProjectName()
        {
            //            var query = _dbEntities.Database.SqlQuery<ProjectMasterModel>(@"select distinct ProjectName from CellPhoneProject.dbo.ProjectMasters 
            //            where ProjectType in ('Smart','Feature') and IsActive=1
            //            order by projectName asc ").ToList();

            //            var query = _dbEntities.Database.SqlQuery<ProjectMasterModel>(@"select distinct ProjectName,ProjectMasterId from CellPhoneProject.dbo.ProjectMasters pm2
            //            where ProjectType in ('Smart','Feature') and IsActive=1
            //            and ProjectMasterId in (select top 1 pm.ProjectMasterId from CellPhoneProject.dbo.ProjectMasters pm 
            //            where pm.ProjectName=pm2.ProjectName  order by pm.ProjectMasterId,pm.OrderNuber desc)
            //            order by ProjectName asc").ToList();

            var query = _dbEntities.Database.SqlQuery<ProjectMasterModel>(@"
                select distinct ProjectModel as ProjectName,ProjectMasterId from CellPhoneProject.dbo.ProjectOrderQuantityDetails pm2
                where  pm2.IsActive=1
                and ProjectMasterId in (select top 1 pm.ProjectMasterId from CellPhoneProject.dbo.ProjectOrderQuantityDetails pm 
                where pm.ProjectModel=pm2.ProjectModel  and  pm.IsActive=1 order by pm.ProjectMasterId desc)
                order by ProjectModel asc").ToList();

            return query;
        }

        public string SaveProjectForecastingData(List<ProjectForecastingInfoModel> results)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var models = new ProjectForecastingInfo();
            if (results[0].ProjectMasterId != 0)
            {
                models.ProjectMasterId = results[0].ProjectMasterId;
            }
            else
            {
                models.ProjectMasterId = 0;

            }

            models.ProjectName = results[0].ProjectName;
            if (results[0].FSegment == "SP")
            {
                models.ProjectType = "Smart";
            }
            else if (results[0].FSegment == "FP")
            {
                models.ProjectType = "Feature";
            }
            models.LaunchDate = results[0].LaunchDate;
            if (results[0].Plc != "SELECT")
            {
                models.Plc = results[0].Plc;
            }
            if (results[0].Brand != "SELECT")
            {
                models.Brand = results[0].Brand;
            }
            if (results[0].FSegment != "SELECT")
            {
                models.FSegment = results[0].FSegment;
            }
            if (results[0].Display != null)
            {
                models.Display = results[0].Display;
            }
            else
            {
                models.Display = 0;
            }
            models.Battery = results[0].Battery;
            models.Ram = results[0].Ram;
            models.Rom = results[0].Rom;
            models.Fob = results[0].Fob;

            if (results[0].January != null)
            {
                models.January = results[0].January;
            }
            else
            {
                models.January = 0;
            }
            if (results[0].February != null)
            {
                models.February = results[0].February;
            }
            else
            {
                models.February = 0;
            }
            if (results[0].March != null)
            {
                models.March = results[0].March;
            }
            else
            {
                models.March = 0;
            }
            if (results[0].April != null)
            {
                models.April = results[0].April;
            }
            else
            {
                models.April = 0;
            }
            if (results[0].May != null)
            {
                models.May = results[0].May;
            }
            else
            {
                models.May = 0;
            }
            if (results[0].June != null)
            {
                models.June = results[0].June;
            }
            else
            {
                models.June = 0;
            }
            if (results[0].July != null)
            {
                models.July = results[0].July;
            }
            else
            {
                models.July = 0;
            }
            if (results[0].August != null)
            {
                models.August = results[0].August;
            }
            else
            {
                models.August = 0;
            }
            if (results[0].September != null)
            {
                models.September = results[0].September;
            }
            else
            {
                models.September = 0;
            }
            if (results[0].October != null)
            {
                models.October = results[0].October;
            }
            else
            {
                models.October = 0;
            }
            if (results[0].November != null)
            {
                models.November = results[0].November;
            }
            else
            {
                models.November = 0;
            }
            if (results[0].December != null)
            {
                models.December = results[0].December;
            }
            else
            {
                models.December = 0;
            ...
            _dbEntities.ProjectForecastingInfoes.Add(models);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<ProjectForecastingInfoModel> GetProjectInfoForForecasting(string proIds)
        {
            long ids;
            long.TryParse(proIds, out ids);

            var query = _dbEntities.Database.SqlQuery<ProjectForecastingInfoModel>(@"select pm.ProjectMasterId,ProjectName,case when ProjectType='Feature' then 'FP'  when ProjectType='Smart' then 'SP'
            end as FSegment,DisplaySize as Display,Battery,Ram,Rom,pm.BrandId,bdd.BrandName as Brand,cast(FORMAT(poq.MarketClearanceDate, 'yyyy-MM-dd') as DATE) as ReleaseDate
            from CellPhoneProject.dbo.ProjectMasters pm 
            left join  [CellPhoneProject].[dbo].[ProjectOrderQuantityDetails] poq on pm.ProjectMasterId=poq.ProjectMasterId
            left join  [CellPhoneProject].[dbo].[Brands] bdd on bdd.Id=pm.BrandId
            where pm.ProjectMasterId={0} ", ids).ToList();
            return query;
        }

        public string UpdateProjectForecastingData(List<ProjectForecastingInfoModel> results)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var ss = Convert.ToString(results[0].Id);
            long ids;
            long.TryParse(ss, out ids);


            var models = (from mm in _dbEntities.ProjectForecastingInfoes where mm.Id == ids && mm.IsActive == true select mm).FirstOrDefault();
            if (results[0].ProjectMasterId != 0)
            {
                models.ProjectMasterId = results[0].ProjectMasterId;
            }
            ...
            models.Updated = userId;
            models.UpdatedDate = DateTime.Now;
            //ss
            _dbEntities.ProjectForecastingInfoes.AddOrUpdate(models);
            _dbEntities.SaveChanges();

            //Log 
            var config = new MapperConfiguration(c => c.CreateMap<ProjectForecastingInfo, ProjectForecastingInfoLog>());
            var mapper = config.CreateMapper();
            var babt = mapper.Map<ProjectForecastingInfoLog>(models);
            babt.ForecastId = models.Id;
            babt.LogAdded = userId;
            babt.LogAddedDate = DateTime.Now;

            _dbEntities.ProjectForecastingInfoLogs.Add(babt);
            _dbEntities.SaveChanges();
            //end Log
        ...
        public string DeleteProjectForecastingData(List<ProjectForecastingInfoModel> results)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);
            ...
            var dbForecast = _dbEntities.ProjectForecastingInfoes.FirstOrDefault(i => i.Id == ids && i.IsActive == true);
            if (dbForecast != null)
            {
                dbForecast.IsActive = false;
                dbForecast.Updated = userId;
                dbForecast.UpdatedDate = DateTime.Now;
                _dbEntities.ProjectForecastingInfoes.AddOrUpdate(dbForecast);
                _dbEntities.SaveChanges();

                //Log
                var config = new MapperConfiguration(c => c.CreateMap<ProjectForecastingInfo, ProjectForecastingInfoLog>());
                var mapper = config.CreateMapper();
                var babt = mapper.Map<ProjectForecastingInfoLog>(dbForecast);
                babt.ForecastId = dbForecast.Id;
                babt.LogAdded = userId;
                babt.LogAddedDate = DateTime.Now;
        ...
        public List<ProjectForecastingInfoModel> GetProjectForecastingList()
        {
//            var query = _dbEntities.Database.SqlQuery<ProjectForecastingInfoModel>(@"select *,
//            (January+February+March+April+May+June+July+August+September+October+November+December) as TotalSum
//            from [CellPhoneProject].[dbo].[ProjectForecastingInfo] where IsActive=1 order by Id desc").ToList();
//            return query;

//            var query = _dbEntities.Database.SqlQuery<ProjectForecastingInfoModel>(@"select *, (OpenPo-(TotalOnhandQty+ProductionableQty+RepairPendingQty+Quantity)) as BDStock,OpenPo
//            from
//            (
//	            select pfi.*,(January+February+March+April+May+June+July+August+September+October+November+December) as TotalSum,
//	            (case when ffg.Quantity is null then 0 else ffg.Quantity end) as Quantity,case when pp.TotalOnhandQty is null then 0 else pp.TotalOnhandQty end as TotalOnhandQty,
//	            case when pp.ProductionableQty is null then 0 else pp.ProductionableQty end as ProductionableQty,case when pp.RepairPendingQty is null then 0 else pp.RepairPendingQty end as RepairPendingQty,
//	            case when pp.WIPQuantity is null then 0 else pp.WIPQuantity end as WIPQuantity,(case when pp.OrderQuantity is null then 0 else pp.OrderQuantity end) as OpenPo 
//	            from [CellPhoneProject].[dbo].[ProjectForecastingInfo] pfi 
//	            left join [CellPhoneProject].[dbo].[ProjectForecastingFinishGoodInfo] ffg on ffg.ProjectName=pfi.ProjectName
//	            left join 
//	            (
//		            select A.ProjectMasterId,A.OrderQuantity,A.ProjectModel,pff.TotalOnhandQty,pff.ProductionableQty,pff.RepairPendingQty,pff.WIPQuantity
//		            from
//		            (
//			            select pod.ProjectMasterId,pod.OrderQuantity,pod.ProjectModel
//			            from  [CellPhoneProject].[dbo].[ProjectOrderQuantityDetails] pod 
//			            where ProjectMasterId not in (select ProjectMasterId from [CellPhoneProject].[dbo].[ProjectOrderShipments])
//			            and OrderQuantity is not null 
//		            )A 
//		            left join [CellPhoneProject].[dbo].[ProjectForecastingMaterialQty] pff on pff.Model=A.ProjectModel
//	            ) pp on pfi.ProjectName=pp.ProjectModel
//	            where pfi.IsActive=1 
//            )A order by Id desc").ToList();


            var query = _dbEntities.Database.SqlQuery<ProjectForecastingInfoModel>(@"select distinct *, ((TotalOnhandQty+ProductionableQty+RepairPendingQty+Quantity)) as BDStock
            from
            (
	            select pfi.*,(January+February+March+April+May+June+July+August+September+October+November+December) as TotalSum,
	            (case when ffg.Quantity is null then 0 else ffg.Quantity end) as Quantity,case when pp.TotalOnhandQty is null then 0 else pp.TotalOnhandQty end as TotalOnhandQty,
	            ...
	            case when pp.WIPQuantity is null then 0 else pp.WIPQuantity end as WIPQuantity,(case when pp.OrderQuantity is null then 0 else pp.OrderQuantity end) as OpenPo 
	            from [CellPhoneProject].[dbo].[ProjectForecastingInfo] pfi 
	            left join [CellPhoneProject].[dbo].[ProjectForecastingFinishGoodInfo] ffg on UPPER(ffg.ProjectName)=UPPER(pfi.ProjectName)
	            left join 
	            (
		            select A.ProjectMasterId,A.OrderQuantity,A.ProjectModel,pff.TotalOnhandQty,pff.ProductionableQty,pff.RepairPendingQty,pff.WIPQuantity
		            from
		            (
			            select pod.ProjectMasterId,pod.OrderQuantity,pod.ProjectModel
			            from  [CellPhoneProject].[dbo].[ProjectOrderQuantityDetails] pod 
			            inner join   [CellPhoneProject].[dbo].ProjectMasters pm on pm.ProjectMasterId= pod.ProjectMasterId 
			            where pod.ProjectMasterId not in (select ProjectMasterId from [CellPhoneProject].[dbo].[ProjectOrderShipments])
			            and pod.OrderQuantity is not null
		            )A 
		            left join [CellPhoneProject].[dbo].[ProjectForecastingMaterialQty] pff on UPPER(pff.Model)=UPPER(A.ProjectModel)
	            ) pp on UPPER(pfi.ProjectName)=UPPER(pp.ProjectModel)
	            where pfi.IsActive=1 
            )A order by Id desc").ToList();
            foreach (var qq1 in query)
            {
                qq1.TTL = qq1.BDStock + qq1.OpenPo;
            }
        ...
        #region Commercial Incentive policy 2022
        public string SaveSpareArrangementAndSupport(Cm_SpareArrangementAndSupportModel models)
        {
            _dbEntities.Database.CommandTimeout = 6000;
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var datas = new Cm_SpareArrangementAndSupport();
            datas.EffectiveMonth = models.EffectiveMonth;
            datas.TotalHandsetValue = models.TotalHandsetValue;
            datas.TotalImportedSpareValue = models.TotalImportedSpareValue;
            var total = (datas.TotalHandsetValue + datas.TotalImportedSpareValue);
            var totalSum = (Convert.ToDecimal(total) * Convert.ToDecimal(0.0015)).ToString("#.##");
            datas.Amount = Convert.ToDecimal(totalSum);
            if (datas.Amount > 8000)
            {
                datas.FinalAmount = 8000;
            }
            else
            {
                datas.FinalAmount = datas.Amount;
            }
            datas.IsActive = true;
            datas.Added = userId;
            datas.AddedDate = DateTime.Now;
            _dbEntities.Cm_SpareArrangementAndSupport.Add(datas);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<Cm_SpareArrangementAndSupportModel> GetSpareArrangementAndSupport()
        {
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_SpareArrangementAndSupportModel>(@"SELECT top 100 
            TotalImportedSpareValue,TotalHandsetValue,DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years,
            Amount,FinalAmount FROM [CellPhoneProject].[dbo].[Cm_SpareArrangementAndSupport] where IsActive=1
            order by Id desc").ToList();

            return query;
        }

        public string SaveMaterialReorderAndSupport(Cm_MaterialReorderAndSupportModel models)
        {
            _dbEntities.Database.CommandTimeout = 6000;
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var datas = new Cm_MaterialReorderAndSupport();
            datas.OrderConfirmationDate = models.OrderConfirmationDate;
            datas.FactoryReceiveDate = models.FactoryReceiveDate;
            datas.Remarks = models.Remarks;
            datas.FixedDateDiff = 90;
            var dateDiff1 = Convert.ToDateTime(datas.FactoryReceiveDate).Date - Convert.ToDateTime(datas.OrderConfirmationDate).Date;
            var dateDiff2 = dateDiff1.Days + 1;
            datas.DateDifference = dateDiff2;
            datas.Amount = 5000;
            if (datas.FixedDateDiff < dateDiff2)
            {
                // datas.Amount = 0;
                datas.FinalAmount = 0;
            }
            else
            {
                ...
                datas.FinalAmount = 5000;
            }
            datas.IsActive = true;
            datas.Added = userId;
            datas.AddedDate = DateTime.Now;
            _dbEntities.Cm_MaterialReorderAndSupport.Add(datas);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<Cm_MaterialReorderAndSupportModel> GetMaterialReorderAndSupport()
        {
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_MaterialReorderAndSupportModel>(@"SELECT top 100 *,DATENAME(month, FactoryReceiveDate) as MonthNames,Year(FactoryReceiveDate) as Years,
            Amount,FinalAmount FROM [CellPhoneProject].[dbo].[Cm_MaterialReorderAndSupport] where IsActive=1
            order by Id desc").ToList();

            return query;
        }

        public string SaveReportAnalysis(Cm_ReportAnalysisModel models)
        {
            _dbEntities.Database.CommandTimeout = 6000;
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var datas = new Cm_ReportAnalysis();
            datas.Category = models.Category;
            datas.Unit = "Report Analysis Per month";
            datas.Remarks = models.Remarks;
            datas.Timeline = 7;
            datas.DocumentSubmitDate = Convert.ToDateTime(models.DocumentSubmitDate);
            var ssdate = Convert.ToDateTime(datas.DocumentSubmitDate);
            string strDate = ssdate.ToString("yyyy-MM-01");
            var dateDiff1 = Convert.ToDateTime(datas.DocumentSubmitDate).Date - Convert.ToDateTime(strDate).Date;
            var dateDiff2 = dateDiff1.Days + 1;

            datas.StartDate = Convert.ToDateTime(strDate);
            datas.DateDifference = dateDiff2;
            datas.Amount = 1000;
            if (datas.Timeline < dateDiff2)
            {
                //datas.Amount = 0;
                datas.FinalAmount = 0;
            }
            ...
            {
                datas.FinalAmount = 1000;
            }

            datas.IsActive = true;
            datas.Added = userId;
            datas.AddedDate = DateTime.Now;
            _dbEntities.Cm_ReportAnalysis.Add(datas);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<Cm_ReportAnalysisModel> GetReportAnalysis()
        {
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_ReportAnalysisModel>(@"SELECT top 100 * FROM [CellPhoneProject].[dbo].Cm_ReportAnalysis where IsActive=1
                order by Id desc").ToList();

            return query;
        }

        public string SaveNegotiationsAndDevelopment(Cm_NegotiationsAndDevelopmentModel models)
        {
            _dbEntities.Database.CommandTimeout = 6000;
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var datas = new Cm_NegotiationsAndDevelopment();
            datas.Category = models.Category;
            datas.Unit = "Per New task";
            datas.Remarks = models.Remarks;
            datas.EffectiveMonth = models.EffectiveMonth;

            if (datas.Category == "Vendor Development and contract establishment")
            {
                datas.Amount = 7000;
                datas.FinalAmount = 7000;
            }
            else
            {
                datas.SavingAmount = models.SavingAmount;
                var amOne = Convert.ToDecimal(datas.SavingAmount) * Convert.ToDecimal(0.1);
                datas.Amount = amOne;
                if (datas.Amount > 5000)
                {
                    datas.FinalAmount = 5000;
                }
                else if (datas.Amount < 5000)
                {
                    datas.FinalAmount = datas.Amount;
                }
            }
            ...
            datas.Added = userId;
            datas.AddedDate = DateTime.Now;
            _dbEntities.Cm_NegotiationsAndDevelopment.Add(datas);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<Cm_NegotiationsAndDevelopmentModel> GetNegotiationsAndDevelopment()
        {
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_NegotiationsAndDevelopmentModel>(@"SELECT top 100 *,DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years
           FROM [CellPhoneProject].[dbo].Cm_NegotiationsAndDevelopment where IsActive=1 order by Id desc").ToList();

            return query;
        }

        public string SavePenalties(Cm_PenaltiesModel models)
        {
            _dbEntities.Database.CommandTimeout = 6000;
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var datas = new Cm_Penalties();
            datas.Category = models.Category;
            datas.Unit = "Running Month";
            datas.Remarks = models.Remarks;
            datas.EffectiveMonth = models.EffectiveMonth;

            if (datas.Category == "Product import demurrage")
            {
                datas.AssumptionAmount = models.AssumptionAmount;
                datas.RealAmount = models.RealAmount;
                var totAm = datas.RealAmount - datas.AssumptionAmount;
                var totAm1 = Convert.ToDecimal(totAm) * Convert.ToDecimal(0.1);
                datas.Amount = -totAm1;
                if (totAm1 > 4000 && datas.AssumptionAmount < datas.RealAmount)
                {
                    datas.FinalAmount = -4000;
                }
                else if (totAm1 < 4000 && datas.AssumptionAmount < datas.RealAmount)
                {
                    datas.FinalAmount = -totAm1;
                }

            }
            else
            {
                datas.AssumptionPercentage = models.AssumptionPercentage;
                datas.RealPercentage = models.RealPercentage;
                var totAm = datas.RealPercentage - datas.AssumptionPercentage;

                //0.26% to 1.00%
                var condition11 = Convert.ToDecimal(0.26);
                var condition12 = Convert.ToDecimal(1.00);
                //1.01% to 3.00%
                var condition21 = Convert.ToDecimal(1.01);
                var condition22 = Convert.ToDecimal(3.00);
                //3.00% to Above
                var condition31 = Convert.ToDecimal(3.00);


                if (totAm >= condition11 && totAm <= condition12)
                {
                    datas.FinalAmount = -3000;
                    datas.Amount = datas.FinalAmount;
                }
                else if (totAm >= condition21 && totAm <= condition22)
                {
                    datas.FinalAmount = -4000;
                    datas.Amount = datas.FinalAmount;
                }
                else if (totAm > condition31)
                {
                    datas.FinalAmount = -5000;
                    datas.Amount = datas.FinalAmount;
                }
            ...
            datas.Added = userId;
            datas.AddedDate = DateTime.Now;
            _dbEntities.Cm_Penalties.Add(datas);
            _dbEntities.SaveChanges();

            return "OK";
        }

        public List<Cm_PenaltiesModel> GetPenalties()
        {
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_PenaltiesModel>(@"select *, (AssumptionAmount-RealAmount) as Diff
            from
            (
	            SELECT top 100 Category,Unit,EffectiveMonth,Remarks,
	            case when AssumptionAmount is null then AssumptionPercentage else AssumptionAmount end as AssumptionAmount,
	            case when RealAmount is null then RealPercentage else RealAmount end as RealAmount,
	            DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years,Amount,FinalAmount
	            FROM [CellPhoneProject].[dbo].Cm_Penalties where IsActive=1 order by Id desc
            )A").ToList();
            return query;
        }
        //incentive page
        public List<Cm_SpareArrangementAndSupportModel> GetSpareArrangementAndSupportIncentive(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);

            long yrs;
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_SpareArrangementAndSupportModel>(@"SELECT  
            TotalImportedSpareValue,TotalHandsetValue,DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years,
            Amount,FinalAmount FROM [CellPhoneProject].[dbo].[Cm_SpareArrangementAndSupport] where IsActive=1
            and Year(EffectiveMonth)={0} and  Month(EffectiveMonth)={1}
            order by Id desc", yrs, mons).ToList();

            return query;
        }

        public List<Cm_MaterialReorderAndSupportModel> GetMaterialReorderAndSupportIncentive(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_MaterialReorderAndSupportModel>(@"SELECT OrderConfirmationDate,FactoryReceiveDate,Remarks,FixedDateDiff,DateDifference,
                DATENAME(month, FactoryReceiveDate) as MonthNames,Year(FactoryReceiveDate) as Years,
                Amount,FinalAmount FROM [CellPhoneProject].[dbo].[Cm_MaterialReorderAndSupport] where IsActive=1
                and Year(FactoryReceiveDate)={0} and  Month(FactoryReceiveDate)={1}
                order by Id desc", yrs, mons).ToList();

            return query;
        }

        public List<Cm_ReportAnalysisModel> GetReportAnalysisIncentive(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_ReportAnalysisModel>(@"SELECT  * FROM [CellPhoneProject].[dbo].Cm_ReportAnalysis where IsActive=1
                 and Year(DocumentSubmitDate)={0} and  Month(DocumentSubmitDate)={1}  order by Id desc", yrs, mons).ToList();

            return query;
        }

        public List<Cm_NegotiationsAndDevelopmentModel> GetNegotiationsIncentive(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_NegotiationsAndDevelopmentModel>(@"
                SELECT *,DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years
                FROM [CellPhoneProject].[dbo].Cm_NegotiationsAndDevelopment where IsActive=1 
                and Year(EffectiveMonth)={0} and  Month(EffectiveMonth)={1}
                order by Id desc", yrs, mons).ToList();
        ...
        public List<Cm_PenaltiesModel> GetPenaltiesIncentive(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_PenaltiesModel>(@"
               select *, (AssumptionAmount-RealAmount) as Diff
                from
                (
	                SELECT top 100 Category,Unit,EffectiveMonth,Remarks,
	                case when AssumptionAmount is null then AssumptionPercentage else AssumptionAmount end as AssumptionAmount,
	                ...
	                DATENAME(month, EffectiveMonth) as MonthNames,Year(EffectiveMonth) as Years,Amount,FinalAmount
	                FROM [CellPhoneProject].[dbo].Cm_Penalties where IsActive=1 order by Id desc
                )A where Year(EffectiveMonth)={0} and Month(EffectiveMonth)={1} ", yrs, mons).ToList();

            return query;
        }

        public List<Cm_MaterialShipmentAndConsumableItemsModel> GetMaterialShipmentAndConsumableItems(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            #region com
            //            var query = _dbEntities.Database.SqlQuery<Cm_MaterialShipmentAndConsumableItemsModel>(@"
            //            select ProjectMasterId,ProjectName,ProjectType,OraclePoNo='',PrNo='',OrderNumber,ContatinerType,IsFinalShipment,cast('' as varchar(50)) as PrDate,cast(ProjectManagerClearanceDate as varchar(50)) as ProjectManagerClearanceDate,
            //            cast(WarehouseEntryDate as varchar(50)) as WarehouseEntryDate,DaysDiff,EffectiveTime,ShipmentType,
            //            cast(Amount as decimal) as Amount,cast(Reward as decimal) as Reward,cast(Penalties as decimal) as Penalties,cast(0 as decimal) as FinalAmount
            //
            //            from
            //            (	
            //		            select C.ProjectMasterId,C.ProjectName,C.ProjectType,OrderNumber,ShipmentType,ContatinerType,IsFinalShipment,C.ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,
            //		            case when Amount1 is null then 0 else Amount1 end as Amount,case when Reward is null then 0 else Reward end as Reward,case when Penalties is null then 0 else Penalties end as Penalties
            //		            from
            //		            (	
            //			            select ProjectMasterId,ProjectName,ProjectType,OrderNumber,ShipmentType,ContatinerType,ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,IsFinalShipment,
            //			            case when ShipmentType in ('Sea','Land') and ContatinerType='FCL' and ProjectType='Smart' and OrderNumber=1 then 2500
            //			            when ShipmentType in ('Sea','Land') and ContatinerType='LCL' and ProjectType='Smart' and OrderNumber=1 then 2500	
            //			            when ShipmentType='Air'  and ProjectType='Smart' and OrderNumber=1 then 2500
            //
            //			            when ShipmentType in ('Sea','Land') and ContatinerType='FCL' and  ProjectType='Feature' and OrderNumber=1 then 2000
            //			            when ShipmentType in ('Sea','Land') and ContatinerType='LCL' and ProjectType='Feature' and OrderNumber=1 then 2000
            //			            when ShipmentType='Air'  and ProjectType='Feature' and OrderNumber=1 then 2000
            //
            //			            when ShipmentType in ('Sea','Land') and ContatinerType='FCL' and OrderNumber!=1 then 1500
            //			            when ShipmentType in ('Sea','Land') and ContatinerType='LCL' and OrderNumber!=1 then 1500	
            //			            when ShipmentType='Air' and OrderNumber!=1 then 1500 end as Amount1,
            //			            case when DaysDiff<EffectiveTime and EffectiveTime-DaysDiff>0 then (EffectiveTime-DaysDiff)*200 else 0	end as Reward,
            //
            //			            case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as Penalties		
            //	
            //			            from
            //			            (
            //				            select A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,A.WarehouseEntryDate,rww.ProjectManagerClearanceDate,A.IsFinalShipment,
            //				            ((DATEDIFF(day, rww.ProjectManagerClearanceDate, WarehouseEntryDate))) as DaysDiff,
            //				            case when A.ShipmentType in ('Sea','Land') and A.ContatinerType='FCL' then 40
            //				            when A.ShipmentType in ('Sea','Land') and A.ContatinerType='LCL' then 45
            //				            when A.ShipmentType='Air' then 20 end as EffectiveTime
            //
            //				            from
            //				            (
            //					            select pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber as OrderNumber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
            //					            pos.WarehouseEntryDate
            //
            //					            FROM [CellPhoneProject].[dbo].[ProjectOrderShipments] pos					
            //					            inner join [CellPhoneProject].[dbo].ProjectMasters pm on pm.ProjectMasterId=pos.ProjectMasterId
            //					            where Month(pos.WarehouseEntryDate)={0} and Year(pos.WarehouseEntryDate)={1} and pos.IsFinalShipment='Yes' and pm.IsActive=1 
            //					            and pos.WarehouseEntryDate in (select top 1 pos1.WarehouseEntryDate
            //				                from [CellPhoneProject].[dbo].[ProjectOrderShipments] pos1 where pos1.ProjectMasterId=pos.ProjectMasterId and pos1.IsFinalShipment='Yes'  order by pos1.WarehouseEntryDate desc)
            //
            //					            group by pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
            //					            pos.WarehouseEntryDate
            //
            //				            )A 
            //				            inner join [CellPhoneProject].[dbo].RawMaterialInspection rww on rww.ProjectMasterId=A.ProjectMasterId
            //				            where  rww.ProjectManagerClearanceDate in (select top 1 rww1.ProjectManagerClearanceDate 
            //				            from [CellPhoneProject].[dbo].RawMaterialInspection rww1 where rww1.ProjectMasterId=rww.ProjectMasterId order by rww1.ProjectManagerClearanceDate desc)
            //				            group by A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,rww.ProjectManagerClearanceDate,A.IsFinalShipment,A.WarehouseEntryDate	
            //			            )B
            //		            )C
            //              )D
            //            union
            //
            //            select  ProjectMasterId,ProjectName,ProjectType,OraclePoNo,PrNo,
            //			OrderNumber,ContatinerType,IsFinalShipment,cast(PrDate as varchar(50)) as PrDate,cast('' as varchar(50)) as ProjectManagerClearanceDate,cast(WarehouseEntryDate as varchar(50)) as WarehouseEntryDate,
            //			DaysDiff,EffectiveTime,ShipmentType,
            //			cast(case when ShipmentType='Air' then 500 when ShipmentType in ('Sea','Land') then 500 else 0 end as decimal) as Amount,
            //
            //				cast(case when DaysDiff<EffectiveTime and EffectiveTime-DaysDiff>0 then (EffectiveTime-DaysDiff)*200 else 0 end as decimal)  as Reward,
            //				cast(case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as decimal) as Penalties,cast(0 as decimal) as FinalAmount
            //
            //			from
            //			( 
            //				select distinct OraclePoNo,PrNo,ProjectMasterId=0,case when Model is null then OtherProductLcForTheProject else Model end as ProjectName,ProductType as ProjectType,
            //				OrderNumber=0,ShipmentType,ContatinerType='',IsFinalShipment='',PrDate,ProjectManagerClearanceDate=null,WarehouseReceiveDate as WarehouseEntryDate,
            //				((DATEDIFF(day, Prdate, WarehouseReceiveDate))) as DaysDiff,
            //				case when ShipmentType in ('Sea','Land') then 115 when  ShipmentType='Air' then 80 end as EffectiveTime
            //				FROM [CellPhoneProject].[dbo].[LcOpeningPermissionOtherProducts] where Month(WarehouseReceiveDate)={0} and Year(WarehouseReceiveDate)={1} and IsFinal=1
            //				and PrDate is not null  and ProductType in ('Consumable Item','Machine','Machine Spare Parts','Tools')
            //			)A
            //            ", mons, yrs).ToList();
            ...
            //            select ProjectMasterId,ProjectName,ProjectType,OraclePoNo='',PrNo='',OrderNumber,ContatinerType,IsFinalShipment,cast('' as varchar(50)) as PrDate,cast(ProjectManagerClearanceDate as varchar(50)) as ProjectManagerClearanceDate,
            //            cast(WarehouseEntryDate as varchar(50)) as WarehouseEntryDate,DaysDiff,EffectiveTime,ShipmentType,
            //            cast(Amount as decimal) as Amount,cast(Reward as decimal) as Reward,cast(Penalties as decimal) as Penalties,cast(0 as decimal) as FinalAmount,FinishGoodCheck
            //
            //            from
            //            (	
            //		            select C.ProjectMasterId,C.ProjectName,C.ProjectType,OrderNumber,ShipmentType,ContatinerType,IsFinalShipment,C.ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,
            //		            case when Amount1 is null then 0 else Amount1 end as Amount,case when Reward is null then 0 else Reward end as Reward,case when Penalties is null then 0 else Penalties end as Penalties,FinishGoodCheck
            //		            from
            //		            (	
            //			            select ProjectMasterId,ProjectName,ProjectType,OrderNumber,ShipmentType,ContatinerType,ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,IsFinalShipment,
            //			            case when ShipmentType in ('Sea','Land') and ContatinerType='FCL' and ProjectType='Smart' and OrderNumber=1 then 2500
            ...
            //			            case when DaysDiff<EffectiveTime and EffectiveTime-DaysDiff>0 then (EffectiveTime-DaysDiff)*200 else 0	end as Reward,
            //
            //			            case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as Penalties,FinishGoodCheck		
            //	
            //			            from
            //			            (
            //				            select A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,A.WarehouseEntryDate,rww.ProjectManagerClearanceDate,A.IsFinalShipment,
            ...
            //				            case when A.ShipmentType in ('Sea','Land') and A.ContatinerType='FCL' then 40
            //				            when A.ShipmentType in ('Sea','Land') and A.ContatinerType='LCL' then 45
            //				            when A.ShipmentType='Air' then 20 end as EffectiveTime,FinishGoodCheck
            //
            //				            from
            //				            (
            //							     SELECT * FROM
            //							     (
            //									select pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber as OrderNumber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
            //									pos.WarehouseEntryDate,    case when pos.ProjectOrderShipmentId in (select sm.ProjectOrderShipmentId 
            //									FROM [CellPhoneProject].[dbo].[ShipmentFinishGoodModel] sm where sm.ProjectOrderShipmentId=pos.ProjectOrderShipmentId) then 'YES' end as FinishGoodCheck
            //
            //									FROM [CellPhoneProject].[dbo].[ProjectOrderShipments] pos					
            //									inner join [CellPhoneProject].[dbo].ProjectMasters pm on pm.ProjectMasterId=pos.ProjectMasterId
            //									where Month(pos.WarehouseEntryDate)={0} and Year(pos.WarehouseEntryDate)={1} and pm.IsActive=1 -- and pos.IsFinalShipment='Yes' 
            //									--and pos.WarehouseEntryDate in (select top 1 pos1.WarehouseEntryDate
            //									--from [CellPhoneProject].[dbo].[ProjectOrderShipments] pos1 where pos1.ProjectMasterId=pos.ProjectMasterId --and pos1.IsFinalShipment='Yes'  
            //									--order by pos1.WarehouseEntryDate desc)
            //
            //									group by pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
            //									pos.WarehouseEntryDate,pos.ProjectOrderShipmentId
            //							    )KK where FinishGoodCheck='YES'
            //				            )A 
            //				            inner join [CellPhoneProject].[dbo].RawMaterialInspection rww on rww.ProjectMasterId=A.ProjectMasterId
            //				            where  rww.ProjectManagerClearanceDate in (select top 1 rww1.ProjectManagerClearanceDate 
            //				            from [CellPhoneProject].[dbo].RawMaterialInspection rww1 where rww1.ProjectMasterId=rww.ProjectMasterId order by rww1.ProjectManagerClearanceDate asc)
            //				            group by A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,rww.ProjectManagerClearanceDate,A.IsFinalShipment,A.WarehouseEntryDate,FinishGoodCheck	
            //			            )B
            //		            )C
            //              )D 
            //            union
            ...
            //
            //				cast(case when DaysDiff<EffectiveTime and EffectiveTime-DaysDiff>0 then (EffectiveTime-DaysDiff)*200 else 0 end as decimal)  as Reward,
            //				cast(case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as decimal) as Penalties,cast(0 as decimal) as FinalAmount,FinishGoodCheck=''
            //
            //			from
            //			( 
            //				select distinct OraclePoNo,PrNo,ProjectMasterId=0,case when Model is null then OtherProductLcForTheProject else Model end as ProjectName,ProductType as ProjectType,
            ...
            var query = _dbEntities.Database.SqlQuery<Cm_MaterialShipmentAndConsumableItemsModel>(@"
                    select ProjectMasterId,ProjectName,ProjectType,OraclePoNo='',PrNo='',OrderNumber,ContatinerType,IsFinalShipment,cast('' as varchar(50)) as PrDate,cast(ProjectManagerClearanceDate as varchar(50)) as ProjectManagerClearanceDate,
            cast(WarehouseEntryDate as varchar(50)) as WarehouseEntryDate,DaysDiff,EffectiveTime,ShipmentType,
            cast(Amount as decimal) as Amount,cast(Reward as decimal) as Reward,cast(Penalties as decimal) as Penalties,cast(0 as decimal) as FinalAmount,FinishGoodCheck,Remarks

            from
            (	
		            select C.ProjectMasterId,C.ProjectName,C.ProjectType,OrderNumber,ShipmentType,ContatinerType,IsFinalShipment,C.ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,
		            case when Amount1 is null then 0 else Amount1 end as Amount,case when Reward is null then 0 else Reward end as Reward,case when Penalties is null then 0 else Penalties end as Penalties,FinishGoodCheck,C.Remarks
		            from
		            (	
			            select ProjectMasterId,ProjectName,ProjectType,OrderNumber,ShipmentType,ContatinerType,ProjectManagerClearanceDate,WarehouseEntryDate,DaysDiff,EffectiveTime,IsFinalShipment,
			            case when ShipmentType in ('Sea','Land') and ContatinerType='FCL' and ProjectType='Smart' and OrderNumber=1 then 2500
			            ...
			            case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as Penalties,FinishGoodCheck,Remarks	
	
			            from
			            (
				            select A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,A.WarehouseEntryDate,A.ProjectManagerClearanceDate,A.IsFinalShipment,
				            ((DATEDIFF(day, A.ProjectManagerClearanceDate, WarehouseEntryDate))) as DaysDiff,
				            case when A.ShipmentType in ('Sea','Land') and A.ContatinerType='FCL' then 40
				            when A.ShipmentType in ('Sea','Land') and A.ContatinerType='LCL' then 45
				            when A.ShipmentType='Air' then 20 end as EffectiveTime,FinishGoodCheck,A.Remarks
				            ...
				            (
							     ...
							     (
									select pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber as OrderNumber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
									pos.ProjectManagerClearanceDate,pos.WarehouseEntryDate,    case when pos.ProjectOrderShipmentId in (select sm.ProjectOrderShipmentId 
									FROM [CellPhoneProject].[dbo].[ShipmentFinishGoodModel] sm where sm.ProjectOrderShipmentId=pos.ProjectOrderShipmentId) then 'YES' end as FinishGoodCheck,pos.Remarks

									FROM [CellPhoneProject].[dbo].[ProjectOrderShipments] pos					
									inner join [CellPhoneProject].[dbo].ProjectMasters pm on pm.ProjectMasterId=pos.ProjectMasterId
									where Month(pos.WarehouseEntryDate)={0} and Year(pos.WarehouseEntryDate)={1} and pm.IsActive=1 

									group by pos.ProjectMasterId,pm.ProjectName,pm.ProjectType,pm.OrderNuber,pos.ShipmentType,pos.IsFinalShipment,pos.ContatinerType,
									pos.WarehouseEntryDate,pos.ProjectOrderShipmentId,pos.ProjectManagerClearanceDate,pos.Remarks
							    )KK where FinishGoodCheck='YES' and ProjectManagerClearanceDate is not null
				            )A 				     
				            group by A.ProjectMasterId,A.ProjectName,A.ProjectType,OrderNumber,A.ShipmentType,A.ContatinerType,A.ProjectManagerClearanceDate,A.IsFinalShipment,A.WarehouseEntryDate,FinishGoodCheck,Remarks
			...
			cast(case when ShipmentType='Air' then 500 when ShipmentType in ('Sea','Land') then 500 else 0 end as decimal) as Amount,
				...
				cast(case when DaysDiff<EffectiveTime and EffectiveTime-DaysDiff>0 then (EffectiveTime-DaysDiff)*200 else 0 end as decimal)  as Reward,
				cast(case when DaysDiff>EffectiveTime and EffectiveTime-DaysDiff<1 then (EffectiveTime-DaysDiff)*80 else 0 end as decimal) as Penalties,cast(0 as decimal) as FinalAmount,FinishGoodCheck='',Remarks=''
            ...
            //ddh  _dbEntities.
            foreach (var mods in query)
            {
                if (mods.PrDate != null)
                {
                    var tt1 = mods.PrDate.Split(' ');
                    mods.PrDate = tt1[0].Trim();
                }
                if (mods.ProjectManagerClearanceDate != null)
                {
                    var tt1 = mods.ProjectManagerClearanceDate.Split(' ');
                    mods.ProjectManagerClearanceDate = tt1[0].Trim();
                }
                if (mods.WarehouseEntryDate != null)
                {
                    var tt1 = mods.WarehouseEntryDate.Split(' ');
                    mods.WarehouseEntryDate = tt1[0].Trim();
                }
        ...
        public List<Cm_TotalPoLcAndTtModel> GetPoLcAnTt(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = new List<Cm_TotalPoLcAndTtModel>();
            //Po
            var query1 = _dbEntities.Database.SqlQuery<Cm_TotalPoLcAndTtModel>(@"
           select AllType,TotalPo,case when TotalPo=0 then 0 else Amount end as Amount,Reward,cast((case when TotalPo>0 and Reward>3000 then Amount+3000 when TotalPo>0 and Reward<=3000 then Amount+Reward 
            else 0 end) as decimal) as FinalAmount
            from
            (
	            select AllType='PO',TotalPo,cast(Amount as decimal) as Amount,cast((case when TotalPo>8 then (TotalPo-8) * 500 else 0 end) as decimal) as Reward
	            from
	            (
		            select count(*) as TotalPo,Amount=7000 
		            from
		            (
			            select ProjectLcId=0,ProjectPurchaseOrderFormId,ProjectMasterId,OracleLcNo=0,ProductType='',OpeningDate='',LcPassDate='',LcValue='',TtDate='',TtNumber='',TtValue='',PurchaseOrderNumber,PoDate,PoCategory
			            FROM [CellPhoneProject].[dbo].[ProjectPurchaseOrderForms] where  Month(PoDate)={0} and Year(PoDate)={1} 
		            )A 
	            )B
            )C
            ", mons, yrs).ToList();

            foreach (var mod1 in query1)
            {
                var models = new Cm_TotalPoLcAndTtModel();
                models.AllType = mod1.AllType;
                models.TotalsCount = mod1.TotalPo;
                models.Amount = mod1.Amount;
                models.Reward = mod1.Reward;
                models.FinalAmount = mod1.FinalAmount;
                query.Add(models);
            }
            _dbEntities.Database.CommandTimeout = 6000;
            //LC
            var query2 = _dbEntities.Database.SqlQuery<Cm_TotalPoLcAndTtModel>(@"
            select AllType,TotalLc,case when TotalLc=0 then 0 else Amount end as Amount,Reward,cast((case when TotalLc>0 and Reward>3000 then Amount+3000 when TotalLc>0 and Reward<=3000 then Amount+Reward else 0 end) as decimal) as FinalAmount
            from
            (
	            select AllType='LC',TotalLc,cast(Amount as decimal) as Amount,cast((case when TotalLc>18 then (TotalLc-18) * 500 else 0 end) as decimal) as Reward
	            from
	            (
		            select count(*) as TotalLc,Amount=7000 
		            from
		            (
			            select Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectId as ProjectMasterId,OracleLcNo,ProductType='',OpeningDate,LcPassDate,LcValue,TtDate='',TtNumber='',TtValue='',PurchaseOrderNumber='',PoDate='',PoCategory=''
			            from CellPhoneProject.dbo.LcOpeningPermissionDetails 
			            where Month(LcPassDate)={0} and Year(LcPassDate)={1}
			            union
			            select ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId,LcNo as OracleLcNo,ProductType='',OpeningDate,LcPassDate,LcValue,TtDate='',TtNumber='',TtValue='',PurchaseOrderNumber='',PoDate='',PoCategory=''
			            from CellPhoneProject.dbo.ProjectLcs 
			            where Month(LcPassDate)={0} and Year(LcPassDate)={1}
			            union
			            select Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId=0,OraclePoNo as OracleLcNo,ProductType,OpeningDate,LcPassDate='',LcValue=0,TtDate,TtNumber,TtValue,PurchaseOrderNumber='',PoDate='',PoCategory=''
			            from CellPhoneProject.dbo.[LcOpeningPermissionOtherProducts] 
			            where Month(TtDate)={0} and Year(TtDate)={1} and cast(TtValue as decimal)>10000
		            )A 
	            )B
            )C
            ", mons, yrs).ToList();

            foreach (var mod1 in query2)
            {
                var models = new Cm_TotalPoLcAndTtModel();
                models.AllType = mod1.AllType;
                models.TotalsCount = mod1.TotalLc;
                models.Amount = mod1.Amount;
                models.Reward = mod1.Reward;
                models.FinalAmount = mod1.FinalAmount;
                query.Add(models);
            ...
            //TT
            _dbEntities.Database.CommandTimeout = 6000;
            var query3 = _dbEntities.Database.SqlQuery<Cm_TotalPoLcAndTtModel>(@"
            select AllType,TotalTt,case when TotalTt=0 then 0 else Amount end as Amount,Reward,cast((case when TotalTt>0 and Reward>3000 then Amount+3000 when TotalTt>0 and  Reward<=3000 then Amount+Reward else 0 end) as decimal) as FinalAmount
            from
            (
	            select AllType='TT',TotalTt,cast(Amount as decimal) as Amount,cast((case when TotalTt>35 then (TotalTt-35) * 500 else 0 end) as decimal) as Reward
	            from
	            (
		            select count(*) as TotalTt,Amount=7000 
		            from
		            (
			            select Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId=0,OraclePoNo as OracleLcNo,ProductType,OpeningDate,LcPassDate='',LcValue='',TtDate,TtNumber,TtValue,PurchaseOrderNumber='',PoDate='',PoCategory=''
			            from CellPhoneProject.dbo.[LcOpeningPermissionOtherProducts] 
			            where Month(TtDate)={0} and Year(TtDate)={1} and cast(TtValue as decimal)<10000
		            )A 
	            )B
            ...
            foreach (var mod1 in query3)
            {
                var models = new Cm_TotalPoLcAndTtModel();
                models.AllType = mod1.AllType;
                models.TotalsCount = mod1.TotalTt;
                models.Amount = mod1.Amount;
                models.Reward = mod1.Reward;
                models.FinalAmount = mod1.FinalAmount;
                query.Add(models);
        ...
        public List<Cm_UserInfosForNewPolicyModel> GetCmUserListForNewPolicy(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            _dbEntities.Database.CommandTimeout = 6000;
            string getUserQuery = string.Format(@"SELECT cm.UserFullName,cm.UserName,cm.EmployeeCode,cm.RoleName,iss.EmployeeCode,iss.CarryAmount,iss.Share,iit.Amount,iit.UserId,cast(iit.ThisMonthAmount as decimal) as ThisMonthAmount,
                iit.AmountCarry,iit.DepartmentName,cast(iit.Percentage as decimal) as Percentage,cast(iit.TotalAmount as decimal) as TotalAmount,cast(iit.TotalIncentive as decimal) as TotalIncentive,
                iit.FixedIncentive,iit.AddedAmount,iit.Remarks,iit.AmountDeduction,iit.DeductionRemarks,cast(iit.SpecialAmount as decimal) as SpecialAmount,iit.SpecialRemarks FROM [CellPhoneProject].[dbo].[CmnUsers] cm
                left join [CellPhoneProject].[dbo].IncentiveShare iss on cm.EmployeeCode=iss.EmployeeCode
                left join [CellPhoneProject].[dbo].[Incentive] iit on iit.UserId=iss.EmployeeCode and iit.MonNum={0} and iit.Year={1}
                where cm.rolename in ('CM','CMHEAD') and cm.IsActive=1 and iss.Category=2
                order by iss.Share desc", mons, yrs);
            var getUserList =
                _dbEntities.Database.SqlQuery<Cm_UserInfosForNewPolicyModel>(
                    getUserQuery).ToList();
            return getUserList;
        }

        public string SaveCommercialMonthlyIncentiveNew(string month, string monNum, string year, string totalAmount, List<Cm_UserInfosForNewPolicyModel> results21, List<Cm_MaterialShipmentAndConsumableItemsModel> results2)
        {
            String userIdentity = System.Web.HttpContext.Current.User.Identity.Name;
            long userId = Convert.ToInt64(userIdentity == "" ? "0" : userIdentity);

            var mm1 = Convert.ToInt32(monNum);
            var yy1 = Convert.ToInt64(year);

            //
            _dbEntities.Database.CommandTimeout = 6000;
            var insQueryLcAndTt =
            (from mm in _dbEntities.Cm_TotalPoLcAndTt where mm.MonNum == mm1 && mm.Year == yy1 && mm.IsActive == true select mm).ToList();
            if (insQueryLcAndTt.Count == 0)
            {
                _dbEntities.Database.CommandTimeout = 6000;
                var qq = _dbEntities.Database.SqlQuery<Cm_TotalPoLcAndTtModel>(@"
                    select AllType='PO',ProjectLcId=0,ProjectPurchaseOrderFormId,ProjectMasterId,OracleLcNo='',ProductType='',cast('' as varchar(50)) as OpeningDate,cast('' as varchar(50)) as LcPassDate,LcValue='',cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber,cast(PoDate as varchar(50)) as PoDate,PoCategory
                    FROM [CellPhoneProject].[dbo].[ProjectPurchaseOrderForms] where  Month(PoDate)={0} and Year(PoDate)={1} 
                    union
                    select AllType='TT',Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId=0,OraclePoNo as OracleLcNo,ProductType,cast(OpeningDate as varchar(50)) as OpeningDate,cast('' as varchar(50)) as LcPassDate,LcValue='',cast(TtDate as varchar(50)) as TtDate,TtNumber,TtValue,PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.[LcOpeningPermissionOtherProducts] 
                    where Month(TtDate)={0} and Year(TtDate)={1} and cast(TtValue as decimal)<10000
                    union
                    select AllType='LC',Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectId as ProjectMasterId,OracleLcNo,ProductType='',cast(OpeningDate as varchar(50)) as OpeningDate,cast(LcPassDate as varchar(50)) as LcPassDate,cast(LcValue as varchar(50)) as LcValue,cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.LcOpeningPermissionDetails 
                    where Month(LcPassDate)={0} and Year(LcPassDate)={1}
                    union
                    select AllType='LC',ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId,LcNo as OracleLcNo,ProductType='',cast(OpeningDate as varchar(50)) as OpeningDate,cast(LcPassDate as varchar(50)) as LcPassDate,cast(LcValue as varchar(50)) as LcValue,cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.ProjectLcs 
                    where Month(LcPassDate)={0} and Year(LcPassDate)={1}
                    union
                    select AllType='LC',Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId=0,OraclePoNo as OracleLcNo,ProductType,cast(OpeningDate as varchar(50)) as OpeningDate,cast('' as varchar(50)) as LcPassDate,LcValue='',cast(TtDate as varchar(50)) as TtDate,TtNumber,TtValue,PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                ...
                foreach (var mdd in qq)
                {
                    var model = new Cm_TotalPoLcAndTt();
                    model.AllType = mdd.AllType;
                    model.ProjectLcId = mdd.ProjectLcId;
                    model.ProjectPurchaseOrderFormId = mdd.ProjectPurchaseOrderFormId;
                    model.ProjectMasterId = mdd.ProjectMasterId;
                    model.OracleLcNo = mdd.OracleLcNo;
                    model.ProductType = mdd.ProductType;
                    if (mdd.OpeningDate != "")
                    {
                        model.OpeningDate = Convert.ToDateTime(mdd.OpeningDate);
                    }
                    if (mdd.LcPassDate != "")
                    {
                        model.LcPassDate = Convert.ToDateTime(mdd.LcPassDate);
                    }
                    if (mdd.TtDate != "")
                    {
                        model.TtDate = Convert.ToDateTime(mdd.TtDate);
                    }
                    if (mdd.PoDate != "")
                    {
                        model.PoDate = Convert.ToDateTime(mdd.PoDate);
                    }
                    model.LcValue = mdd.LcValue;
                    model.TtNumber = mdd.TtNumber;
                    model.PurchaseOrderNumber = mdd.PurchaseOrderNumber;
                    model.PoCategory = mdd.PoCategory;
                    model.MonNum = mm1;
                    model.Year = yy1;
                    model.IsActive = true;
                    model.Added = userId;
                    model.AddedDate = DateTime.Now;
                    _dbEntities.Cm_TotalPoLcAndTt.Add(model);
                    _dbEntities.SaveChanges();
                }
            }
            //
            _dbEntities.Database.CommandTimeout = 6000;
            var insQueryCom =
              (from mm in _dbEntities.Cm_MaterialShipmentAndConsumableItems where mm.MonNum == mm1 && mm.Year == yy1 && mm.IsActive == true select mm).ToList();

            if (insQueryCom.Count == 0)
            {
                foreach (var insResult in results2)
                {
                    var model = new Cm_MaterialShipmentAndConsumableItems();
                    model.ProjectMasterId = Convert.ToInt64(insResult.ProjectMasterId);
                    model.ProjectName = insResult.ProjectName;
                    model.ProjectType = insResult.ProjectType;
                    model.OraclePoNo = insResult.OraclePoNo;
                    model.PrNo = insResult.PrNo;
                    model.OrderNumber = insResult.OrderNumber;
                    model.ContainerType = insResult.ContainerType;
                    model.IsFinalShipment = insResult.IsFinalShipment;
                    if (insResult.PrDate != "")
                    {
                        model.PrDate = Convert.ToDateTime(insResult.PrDate);
                    }
                    if (insResult.ProjectManagerClearanceDate != "")
                    {
                        model.ProjectManagerClearanceDate = Convert.ToDateTime(insResult.ProjectManagerClearanceDate);
                    }
                    if (insResult.WarehouseEntryDate != "")
                    {
                        model.WarehouseEntryDate = Convert.ToDateTime(insResult.WarehouseEntryDate);
                    }
                    //  model.ProjectManagerClearanceDate = Convert.ToDateTime(insResult.ProjectManagerClearanceDate);
                    //  model.WarehouseEntryDate = Convert.ToDateTime(insResult.WarehouseEntryDate);
                    model.DaysDiff = insResult.DaysDiff;
                    model.EffectiveTime = insResult.EffectiveTime;
                    model.ShipmentType = insResult.ShipmentType;
                    ...
                    {
                        model.ModifiedPenalties = 0;
                    }
                    else
                    {
                        model.ModifiedPenalties = model.Penalties;
                    }
                    model.Added = userId;
                    model.AddedDate = DateTime.Now;
                    _dbEntities.Cm_MaterialShipmentAndConsumableItems.Add(model);
                    _dbEntities.SaveChanges();
                }
            }
            _dbEntities.Database.CommandTimeout = 6000;
            var insQuery =
                (from mm in _dbEntities.Incentives where mm.MonNum == mm1 && mm.Year == yy1 && mm.Percentage==45 select mm).ToList();
            ...
            if (insQuery.Count == 0)
            {
                ...
                foreach (var insResult in results21)
                {
                    ...
                    if (insResult.RoleName == "CMHEAD")
                    {

                        model.TotalAmount = Convert.ToDecimal(totalAmount);
                        model.ThisMonthAmount = Convert.ToDecimal(totalAmount);

                        if (insResult.SpecialAmount != null)
                        {
                            model.TotalIncentive = Convert.ToDecimal(insResult.SpecialAmount);
                            model.SpecialAmount = insResult.SpecialAmount;
                            model.SpecialRemarks = insResult.SpecialRemarks;
                        }
                        else
                        {
                            var ttAm1 = Convert.ToDecimal(totalAmount);
                            ttAm21 = Convert.ToDecimal(ttAm1 * (insResult.Percentages / 100));
                            model.TotalIncentive = Convert.ToDecimal(ttAm21);
                    ...
                    {
                        var ttAm1 = Convert.ToDecimal(totalAmount);
                        var ttAm = ttAm1 - ttAm21;
                        model.TotalAmount = Convert.ToDecimal(ttAm);
                        model.ThisMonthAmount = Convert.ToDecimal(ttAm);

                        //var ttAm3 = Convert.ToDecimal(ttAm * (insResult.Percentages / 100));
                        var ttAm3 = Convert.ToDecimal(ttAm1 * (insResult.Percentages / 100));

                        //   model.TotalIncentive = Convert.ToDecimal(ttAm3);
                    ...
                    model.Percentage = Convert.ToDouble(insResult.Percentages);
                    model.DepartmentName = insResult.RoleName;
                    model.Remarks = insResult.Remarks;
                    model.Added = userId;
                    model.AddedDate = DateTime.Now;
        ...
        public List<Cm_UserInfosForNewPolicyModel> CheckIncentiveEntry(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            _dbEntities.Database.CommandTimeout = 6000;
            string getUserQuery = string.Format(@"select UserId from [CellPhoneProject].[dbo].[Incentive] where MonNum={0}
              and Year={1} and DepartmentName in ('CM')  and Percentage=45", mons, yrs);
            var getUserList =
                _dbEntities.Database.SqlQuery<Cm_UserInfosForNewPolicyModel>(
                    getUserQuery).ToList();
            return getUserList;
        ...
        public List<Cm_TotalPoLcAndTtModel> GetTotalPoLcAndTt(string months, string years)
        {
            long mons;
            long.TryParse(months, out mons);
            ...
            long.TryParse(years, out yrs);
            _dbEntities.Database.CommandTimeout = 6000;
            var query = _dbEntities.Database.SqlQuery<Cm_TotalPoLcAndTtModel>(@"
                    select AllType='PO',ProjectLcId=0,ProjectPurchaseOrderFormId,ppo.ProjectMasterId,OracleLcNo='',pm.ProjectName as ProductType,cast('' as varchar(50)) as OpeningDate,cast('' as varchar(50)) as LcPassDate,LcValue='',cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber,cast(PoDate as varchar(50)) as PoDate,PoCategory
                    FROM [CellPhoneProject].[dbo].[ProjectPurchaseOrderForms] ppo
                    inner join CellPhoneProject.dbo.ProjectMasters pm on pm.ProjectMasterId=ppo.ProjectMasterId
                    where  Month(PoDate)={0} and Year(PoDate)={1} 
                    union
                    select AllType='TT',Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId=0,OraclePoNo as OracleLcNo,ProductType,cast(OpeningDate as varchar(50)) as OpeningDate,cast('' as varchar(50)) as LcPassDate,LcValue='',cast(TtDate as varchar(50)) as TtDate,TtNumber,TtValue,PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.[LcOpeningPermissionOtherProducts] 
                    where Month(TtDate)={0} and Year(TtDate)={1} and cast(TtValue as decimal)<10000
                    union
                    select AllType='LC',Id as ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectId as ProjectMasterId,OracleLcNo,pm.ProjectName as ProductType,cast(OpeningDate as varchar(50)) as OpeningDate,cast(LcPassDate as varchar(50)) as LcPassDate,cast(LcValue as varchar(50)) as LcValue,cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.LcOpeningPermissionDetails lcc
                   inner join CellPhoneProject.dbo.ProjectMasters pm on pm.ProjectMasterId=lcc.ProjectId
                    where Month(LcPassDate)={0} and Year(LcPassDate)={1}
                    union
                    select AllType='LC',ProjectLcId,ProjectPurchaseOrderFormId=0,ProjectMasterId,LcNo as OracleLcNo,ProductType='',cast(OpeningDate as varchar(50)) as OpeningDate,cast(LcPassDate as varchar(50)) as LcPassDate,cast(LcValue as varchar(50)) as LcValue,cast('' as varchar(50)) as TtDate,TtNumber='',TtValue='',PurchaseOrderNumber='',cast('' as varchar(50)) as PoDate,PoCategory=''
                    from CellPhoneProject.dbo.ProjectLcs 
            ...
            foreach (var mods in query)
            {
                if (mods.OpeningDate != null)
                {
                    var tt1 = mods.OpeningDate.Split(' ');
                    mods.OpeningDate = tt1[0].Trim();
                }
                if (mods.LcPassDate != null)
                {
                    var tt1 = mods.LcPassDate.Split(' ');
                    mods.LcPassDate = tt1[0].Trim();
                }
                if (mods.TtDate != null)
                {
                    var tt1 = mods.TtDate.Split(' ');
                    mods.TtDate = tt1[0].Trim();
                }
                if (mods.PoDate != null)
                {
                    var tt1 = mods.PoDate.Split(' ');
                    mods.PoDate = tt1[0].Trim();
                }
