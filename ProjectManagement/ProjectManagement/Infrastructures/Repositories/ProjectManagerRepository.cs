  ...
  string.Format(@"select ProjectMasterId as ProjectMasterID
        ...
        public List<SwQcHeadAssignsFromPmModel> GetProjectVersionName(string projectId, int swVersionNo, long testPhaseIds)
        {
            ...
            if (proName != null && swVersionNo == 0 && testPhaseIds == 0)
            {
                ...
                CellPhoneProject.dbo.SwQcHeadAssignsFromPm where ProjectName={0} and TestPhaseID  not in (10,47)
                union 
                ...
                from CellPhoneProject.dbo.SwQcHeadAssignsFromPm where ProjectName={0} and TestPhaseID  not in (10,47)
                order by SoftwareVersionNo asc", proName.Trim()).ToList();
            }
            else if ((proName != null && swVersionNo != 0 && testPhaseIds != 5) || testPhaseIds != 47)
            {
                //                query = _dbEntities.Database.SqlQuery<SwQcHeadAssignsFromPmModel>(@"            
                //               select distinct ProjectName as SoftVersionName,SoftwareVersionNo=1,ProjectName as SoftwareVersionName from
                //                CellPhoneProject.dbo.SwQcHeadAssignsFromPm where ProjectName={0}  
                ...
                case when TestPhaseID=5 then 'SW_V_'+ cast(SoftwareVersionNo as varchar)+'_Demo' else 'SW_V_'+ cast(SoftwareVersionNo as varchar) end as SoftwareVersionName,  case when  TestPhaseID not in (5,47) then 0 else TestPhaseID end as TestPhaseID
                from CellPhoneProject.dbo.SwQcHeadAssignsFromPm where ProjectName={0}   and SoftwareVersionNo={1} and TestPhaseID  not in (10,47)
                order by SoftwareVersionNo asc", proName.Trim(), swVersionNo).ToList();
            }
        ...
        public SwQcHeadAssignsFromPm GetAllVersionNameForPm(long swVerNo, long proId, long testPhases)
        {
            ...
            if (testPhases != 5)
            {
                ...
                where ProjectName={1} and SoftwareVersionNo={0} and TestPhaseID !=5 and  TestPhaseID !=10 and  TestPhaseID !=47", swVerNo, proNames, testPhases).FirstOrDefault();
            }
        ...
        public SwQcHeadAssignsFromPm GetVersionNameForPm(long swVerNo, long proId)
        {
            ...
            where ProjectName={1} and SoftwareVersionNo={0} and TestPhaseID !=5  and  TestPhaseID !=47", swVerNo, proNames).FirstOrDefault();
                    ...
                    try
                    {
                        using (var package = new ExcelPackage(file.InputStream))
                        {
                            ...
                            for (int rowIterator = 3; rowIterator <= noOfRow; rowIterator++)
                            {
                                ...
                                {
                                    swData = (from c in dbeEntities.SwQcIssueDetails
                                              where c.IssueSerial == serial && c.ProjectName == projectName && c.SoftwareVersionNo == softVersion && c.TestPhaseID != 5 && c.TestPhaseID != 47
                                              select c).FirstOrDefault();
                                }
	...
	where bpm.Model={2} and bom.BOMType={1} ", proIds, bomsTypes, projectNames).ToList();
        ...
        {
            var roleName = _dbeEntities.Database.SqlQuery<CmnUserModel>(@"select RoleName,UserName from [CellPhoneProject].[dbo].[CmnUsers]
                where CmnUserId={0} ", userId).FirstOrDefault();
        ...
        public List<PmQcAssignModel> GetPmAssignedProjectForUx()
        {

            var query = _dbeEntities.Database.SqlQuery<PmQcAssignModel>(@"
                select distinct        
                ...
                pm.ProcessorClock,pm.Chipset,pm.FrontCamera,pm.BackCamera,pm.Ram,pm.Rom,pm.Battery,pm.SourcingType,pm.ProjectType, 
                case when sm.ProjectManagerSampleNo is null then 0 else sm.ProjectManagerSampleNo end as ProjectManagerSampleNo,
                sm.SwQcHeadAssignId,tp.TestPhaseName,sm.SoftwareVersionName,
                sm.SoftwareVersionNo,sm.PmToQcHeadAssignComment,sm.PmToQcHeadAssignTime
                ...
                where sm.Status in ('NEW') and tp.TestPhaseName='User Experience Test'  order by sm.SwQcHeadAssignId desc
            ").ToList();
            return query;
        }

        public List<UxDetailsInfoModel> GetUxOptions(string swQcHeadIds)
        {
            long swQcs;
            long.TryParse(swQcHeadIds, out swQcs);

            var query = _dbeEntities.Database.SqlQuery<UxDetailsInfoModel>(@"SELECT uxo.Id as OptionId1,uxo.Options,uxo.StandardPoint,udi.*
                FROM [CellPhoneProject].[dbo].[UxOptions] uxo
                left join  [CellPhoneProject].[dbo].[UxDetailsInfo] udi on udi.OptionId=uxo.Id and udi.SwQcHeadAssignId={0}
                where uxo.IsActive=1 order by uxo.Id asc            
            ", swQcs).ToList();
            return query;
        }

        public List<UxStandardModel> GetUxStandard(string swQcHeadIds)
        {
            long swQcs;
            long.TryParse(swQcHeadIds, out swQcs);

            var query = _dbeEntities.Database.SqlQuery<UxStandardModel>(@"SELECT  uss.[Id],[StandardRange],[Descriptions],usii.StandardId,usii.SwQcHeadAssignId    
            FROM [CellPhoneProject].[dbo].[UxStandards] uss
            left join [CellPhoneProject].[dbo].UxStandardDetailsInfo usii on usii.StandardId=uss.Id and usii.SwQcHeadAssignId={0}
            where uss.IsActive=1          
            ", swQcs).ToList();
            return query;
        }

        public List<UxStandardModel> GetUxStandardDetails(string swQcHeadIds)
        {
            long swQcs;
            long.TryParse(swQcHeadIds, out swQcs);

            var query = _dbeEntities.Database.SqlQuery<UxStandardModel>(@"SELECT  usii.[Id],usii.StandardId,usii.SwQcHeadAssignId,usii.FinalComments,usii.Attachments 
                FROM [CellPhoneProject].[dbo].UxStandardDetailsInfo usii  
                where usii.IsActive=1  and usii.SwQcHeadAssignId={0} 
            ", swQcs).ToList();
            return query;
        }

        public string SaveUserExperienceData(VmUserExperienceTest models)
        {
            String useridentity = HttpContext.Current.User.Identity.Name;
            var users = Convert.ToInt64(useridentity == "" ? "0" : useridentity);

            var queryUxDetails =
                (from uxx in _dbeEntities.UxDetailsInfoes
                 where uxx.SwQcHeadAssignId == models.SwQcHeadAssignId
                 select uxx).FirstOrDefault();

            var queryUxStandard = (from ux in _dbeEntities.UxStandardDetailsInfoes
                                   where ux.SwQcHeadAssignId == models.SwQcHeadAssignId
                                   select ux).FirstOrDefault();

            var proMasters = (from uxx in _dbeEntities.ProjectMasters
                              where uxx.ProjectMasterId == models.ProjectMasterId
                              select uxx).FirstOrDefault();

            #region User Details Info
            if (queryUxDetails == null)
            {

                if (models.UxDetailsInfoModels.Any())
                {

                    foreach (var model1 in models.UxDetailsInfoModels)
                    {
                        var uxDetails = new UxDetailsInfo();
                        uxDetails.SwQcHeadAssignId = models.SwQcHeadAssignId;
                        uxDetails.OptionId = model1.OptionId1;
                        uxDetails.ProjectMasterId = models.ProjectMasterId;
                        uxDetails.ProjectName = models.ProjectName;
                        uxDetails.Orders = "Order " + Convert.ToString(proMasters.OrderNuber);
                        uxDetails.ObtainedPoint = model1.ObtainedPoint;
                        uxDetails.Remarks = model1.Remarks;
                        if (model1.Minor1 == true)
                        {
                            uxDetails.Minor = "Tick";
                        }
                        if (model1.Minor2 == true)
                        {
                            uxDetails.Minor = "Cross";
                        }
                        if (model1.Minor1 == false && model1.Minor2 == false)
                        {
                            uxDetails.Minor = null;
                        }
                        //
                        if (model1.Major1 == true)
                        {
                            uxDetails.Major = "Tick";
                        }
                        if (model1.Major2 == true)
                        {
                            uxDetails.Major = "Cross";
                        }
                        if (model1.Major1 == false && model1.Major2 == false)
                        {
                            uxDetails.Major = null;
                        }
                        //
                        if (model1.Critical1 == true)
                        {
                            uxDetails.Critical = "Tick";
                        }
                        if (model1.Critical2 == true)
                        {
                            uxDetails.Critical = "Cross";
                        }
                        if (model1.Critical1 == false && model1.Critical2 == false)
                        {
                            uxDetails.Critical = null;
                        }
                        //
                        uxDetails.IsActive = true;
                        uxDetails.Added = users;
                        uxDetails.AddedDate = DateTime.Now;

                        _dbeEntities.UxDetailsInfoes.Add(uxDetails);
                        _dbeEntities.SaveChanges();
                    }
                }
            ...
            else
            {
                ...
                if (models.UxDetailsInfoModels.Any())
                {
                    foreach (var model1 in models.UxDetailsInfoModels)
                    {
                        var queryUxDetails2 = (from uxx in _dbeEntities.UxDetailsInfoes
                                               where uxx.Id == model1.Id
                                               select uxx).FirstOrDefault();

                        queryUxDetails2.ObtainedPoint = model1.ObtainedPoint;
                        queryUxDetails2.Remarks = model1.Remarks;
                        if (model1.Minor1 == true)
                        {
                            queryUxDetails2.Minor = "Tick";
                        }
                        if (model1.Minor2 == true)
                        {
                            queryUxDetails2.Minor = "Cross";
                        }
                        if (model1.Minor1 == false && model1.Minor2 == false)
                        {
                            queryUxDetails2.Minor = null;
                        }
                        //
                        if (model1.Major1 == true)
                        {
                            queryUxDetails2.Major = "Tick";
                        }
                        if (model1.Major2 == true)
                        {
                            queryUxDetails2.Major = "Cross";
                        }
                        if (model1.Major1 == false && model1.Major2 == false)
                        {
                            queryUxDetails2.Major = null;
                        }
                        //
                        if (model1.Critical1 == true)
                        {
                            queryUxDetails2.Critical = "Tick";
                        }
                        if (model1.Critical2 == true)
                        {
                            queryUxDetails2.Critical = "Cross";
                        }
                        if (model1.Critical1 == false && model1.Critical2 == false)
                        {
                            queryUxDetails2.Critical = null;
                        }
                        //

                        queryUxDetails2.Updated = users;
                        queryUxDetails2.UpdatedDate = DateTime.Now;

                        _dbeEntities.UxDetailsInfoes.AddOrUpdate(queryUxDetails2);
                        _dbeEntities.SaveChanges();
           ...
           var queryUxStandard2 = (from ux in _dbeEntities.UxStandardDetailsInfoes
                               where ux.SwQcHeadAssignId == models.SwQcHeadAssignId
                               select ux).FirstOrDefault();
            #region Ux Standard
            if (queryUxStandard2 == null)
            {
                if (models.UxStandardModels.Any())
                {

                    var qq1 = _dbeEntities.UxDetailsInfoes.Where(t => t.SwQcHeadAssignId == models.SwQcHeadAssignId && t.IsActive == true).Select(t => t.ObtainedPoint ?? 0).Sum();
                    var qq2 = (from qq in _dbeEntities.UxStandards where qq.IsActive == true select qq).ToList();
                    var uxStan = new UxStandardDetailsInfo();
                    if (qq1 != null)
                    {
                        foreach (var uxStn in qq2)
                        {
                            if (qq1 >= uxStn.StartRange && qq1 <= uxStn.EndRange)
                            {

                                uxStan.StandardId = uxStn.Id;
                            }
                        }
                    }
                    uxStan.SwQcHeadAssignId = models.SwQcHeadAssignId;
                    uxStan.ProjectMasterId = models.ProjectMasterId;
                    uxStan.IsActive = true;
                    uxStan.Added = users;
                    uxStan.AddedDate = DateTime.Now;
                    _dbeEntities.UxStandardDetailsInfoes.Add(uxStan);
                    _dbeEntities.SaveChanges();
                ...
                if (models.UxStandardModels.Any())
                {
                    ...
                    if (qq1 != null)
                    {
                        foreach (var uxStn in qq2)
                        {
                            if (qq1 >= uxStn.StartRange && qq1 <= uxStn.EndRange)
                            {
                                queryUxStandard2.StandardId = uxStn.Id;
                            }
                        }
                    }
                    queryUxStandard2.Updated = users;
                    queryUxStandard2.UpdatedDate = DateTime.Now;
                    _dbeEntities.UxStandardDetailsInfoes.AddOrUpdate(queryUxStandard2);
                    _dbeEntities.SaveChanges();
           ...
           var queryUxStandard3 = (from ux in _dbeEntities.UxStandardDetailsInfoes
                               where ux.SwQcHeadAssignId == models.SwQcHeadAssignId
                               select ux).FirstOrDefault();


            if (models.UxStandardModels1.Any())
            {
                foreach (var model3 in models.UxStandardModels1)
                {
                    ...
                    if (qq1 != null)
                    {
                        foreach (var uxStn in qq2)
                        {
                            if (qq1 >= uxStn.StartRange && qq1 <= uxStn.EndRange)
                            {
                                queryUxStandard3.StandardId = uxStn.Id;
                            }
                        }
                    }
                    queryUxStandard3.SwQcHeadAssignId = models.SwQcHeadAssignId;
                    queryUxStandard3.ProjectMasterId = models.ProjectMasterId;
                    queryUxStandard3.IsActive = true;
                    queryUxStandard3.FinalComments = model3.FinalComments;

                    if (models.Attachments != null && models.Attachments != "")
                    {

                        if (model3.Attachments != null && model3.Attachments != "")
                        {
                            queryUxStandard3.Attachments = queryUxStandard3.Attachments == null ? models.Attachments : queryUxStandard3.Attachments + "|" + models.Attachments;

                        }
                        else
                        {
                            queryUxStandard3.Attachments = models.Attachments;
                        }
                    }
                    queryUxStandard3.Updated = users;
                    queryUxStandard3.UpdatedDate = DateTime.Now;
                    _dbeEntities.UxStandardDetailsInfoes.AddOrUpdate(queryUxStandard3);
                    _dbeEntities.SaveChanges();
                }
            }
            else
            {
                ...
                if (models.FinalComments != null && models.FinalComments != "")
                {
                    queryUxStandard3.FinalComments = models.FinalComments;
                }
                if (models.Attachments != null && models.Attachments != "")
                {
                    queryUxStandard3.Attachments = queryUxStandard3.Attachments == null ? models.Attachments : queryUxStandard3.Attachments + "|" + models.Attachments;
        ...
        public string GetUxInforForMail(long swQcIds)
        {
            String useridentity = HttpContext.Current.User.Identity.Name;
            var users = Convert.ToInt64(useridentity == "" ? "0" : useridentity);

            var qq4 = (from cm in _dbeEntities.CmnUsers where cm.CmnUserId == users select cm).FirstOrDefault();
            var qq5 = (from cm in _dbeEntities.SwQcHeadAssignsFromPms where cm.SwQcHeadAssignId == swQcIds select cm).FirstOrDefault();

            var query1 = _dbeEntities.Database.SqlQuery<UxDetailsInfoModel>(@"SELECT udi.*,uss.Id,uss.Options,uss.StandardPoint
                FROM [CellPhoneProject].[dbo].[UxDetailsInfo] udi
                inner join CellPhoneProject.dbo.UxOptions uss on uss.Id=udi.OptionId  
                where udi.SwQcHeadAssignId={0} and uss.IsActive=1", swQcIds).ToList();

            var query2 = (from sww in _dbeEntities.SwQcHeadAssignsFromPms where sww.SwQcHeadAssignId == swQcIds select sww).FirstOrDefault();
            ...
            body += "Dear Sir/Madam,";
            body += "<br/><br/>This is to inform you that a Ux feedback for <b>" + query2.ProjectName + ",</b> Software Version Name: <b>" + qq5.SoftwareVersionName + "</b>, Ver.No. <b>" + qq5.SoftwareVersionNo + "</b>, <b>Orders " + qq5.OrderNumber + "</b> has been<br/> generated by <b>" + qq4.UserFullName
                + "</b>. Details are given below: <br/><br/>";

            // body += "<br/><br/> <b>Bidding Name :</b>" + bidStart.BidName + "<br/><b>Openning Remarks :</b>" + bidStart.OpeningRemarks + "<br/><br/>";


            body += " <table border=" + 1 + " cellpadding=" + 10 + " cellspacing=" + 2 + " width = " + 800 + ">" +
                "<tr bgcolor='#FFE699' color='black'><td><b>Options</b></td><td><b> Standard</b> </td><td><b> Obtained</b> </td><td><b>Remarks</b></td>" +
                    "<td><b>Minor</b></td><td><b>Major</b></td><td><b>Critical</b></td></tr>";
            foreach (var bii in query1)
            {
                // body += "<tr><td>" + bii.Options + "</td><td> " + bii.StandardPoint + "</td><td> " + bii.ObtainedPoint + "</td><td> " + bii.Remarks + "</td><td style='font-weight:bold'> " + bii.Minor + "</td><td> " + bii.Major + "</td><td> " + bii.Critical + "</td> </tr>";
                body += "<tr><td>" + bii.Options + "</td><td> " + bii.StandardPoint + "</td><td> " + bii.ObtainedPoint + "</td><td> " + bii.Remarks + "</td>";
                if (bii.Minor == "Tick")
                {
                    body += "<td style='font-weight:bold;color:green'> " + bii.Minor + "</td>";
                }
                else if (bii.Minor == "Cross")
                {
                    body += "<td style='font-weight:bold;color:red'> " + bii.Minor + "</td>";
                }
                else
                {
                    body += "<td style='font-weight:bold;'> " + bii.Minor + "</td>";
                }
                if (bii.Major == "Tick")
                {
                    body += "<td style='font-weight:bold;color:green'> " + bii.Major + "</td>";
                }
                else if (bii.Major == "Cross")
                {
                    body += "<td style='font-weight:bold;color:red'> " + bii.Major + "</td>";
                }
                else
                {
                    body += "<td style='font-weight:bold;'> " + bii.Major + "</td>";
                }
                if (bii.Critical == "Tick")
                {
                    body += "<td style='font-weight:bold;color:green'> " + bii.Critical + "</td>";
                }
                else if (bii.Critical == "Cross")
                {
                    body += "<td style='font-weight:bold;color:red'> " + bii.Critical + "</td>";
                }
                else
                {
                    body += "<td style='font-weight:bold;'> " + bii.Critical + "</td>";
                }
                body += "</tr>";

            }
            var standardSum = query1.Sum(item => item.StandardPoint);
            var obtainedSum = query1.Sum(item => item.ObtainedPoint);

            body += "<tr><td style='font-weight:bold;color:black'>Sum :</td><td style='font-weight:bold;color:black'> " + standardSum + "</td><td style='font-weight:bold;color:black'> " + obtainedSum + "</td><td></td><td style='font-weight:bold'></td><td> </td><td></td> </tr>";

            body += "</table>";

            var query3 = _dbeEntities.Database.SqlQuery<UxStandardModel>(@"select ux.Descriptions,ux.StandardRange,uxsi.FinalComments,uxsi.Attachments
              from [CellPhoneProject].[dbo].[UxStandardDetailsInfo] uxsi 
              left join [CellPhoneProject].[dbo].[UxStandards] ux on ux.Id=uxsi.StandardId
              where uxsi.SwQcHeadAssignId={0} and uxsi.IsActive=1", swQcIds).FirstOrDefault();


            body += "<br/><br/><b style='color:black;font-weight:bold;'><u>STANDARDS:</u></b><br/><br/>";

            body += " <table border=" + 1 + " cellpadding=" + 10 + " cellspacing=" + 2 + " width = " + 800 + ">" +
               "<tr bgcolor='#FFE699' color='black'><td><b>Descriptions</b></td><td><b> Standard Range</b> </td><td><b> Final Comments</b> </td></tr>";
            if (query3!=null)
            {
                body += "<tr><td>" + query3.Descriptions + "</td><td> " + query3.StandardRange + "</td><td> " + query3.FinalComments + "</td>";
                body += "</tr>";
            }
            body += "</table><br/><br/>";

            body += "Best Regards<br/>";
            body += "<b>" + qq4.UserFullName + "</b>";
            body += "<br/><br/>";

            //live
            ids3.Add("mobile.qc@waltonbd.com");
            ids3.Add("mobile_pm@waltonbd.com");
            ids3.Add("rnd_hw@waltonbd.com");
            //end live

            var mailMessage = new MailMessage { From = new MailAddress("wpms.noreply@waltonbd.com") };

            if (query3.Attachments != null && query3.Attachments != "")
            {
              
                var fileManager = new FileManager();

                var urls = query3.Attachments.Split('|').ToList();
               
                for (int i = 0; i < urls.Count; i++)
                {
                    var dd = fileManager.GetFileName(urls[i]);
                    const string webServerFilePath3 = @"~/Content/UploadImage";
                    var kk = Path.Combine(HttpContext.Current.Server.MapPath(webServerFilePath3), dd);
                    var attachment = new Attachment(kk);
                    mailMessage.Attachments.Add(attachment);
                }
            }

            //foreach (var to in ids3)
            //{
            //    mailMessage.To.Add(to);
            //}

            mailMessage.To.Add("mobile.qc@waltonbd.com");
            mailMessage.To.Add("mobile_pm@waltonbd.com");
            mailMessage.To.Add("rnd_hw@waltonbd.com");

            mailMessage.Subject = "Feedback from UX Team Process for " + query2.ProjectName + ".";
            mailMessage.Body = body;
            mailMessage.IsBodyHtml = true;
            mailMessage.Bcc.Add("u.salma@waltonbd.com");
            var smtp = new SmtpClient
            {
                UseDefaultCredentials = false,
                Host = "192.168.150.201",
                Port = 25,
                Credentials = new System.Net.NetworkCredential("wpms.noreply", "walton123")
            };
        ...
        public string UserExperienceAttachmentDelete(long swQcIds)
        {
            String useridentity = HttpContext.Current.User.Identity.Name;
            var users = Convert.ToInt64(useridentity == "" ? "0" : useridentity);

            var queryUxDetails =
                (from uxx in _dbeEntities.UxStandardDetailsInfoes
                 where uxx.SwQcHeadAssignId == swQcIds && uxx.IsActive==true
                 select uxx).FirstOrDefault();

            queryUxDetails.Attachments = null;
            queryUxDetails.Updated = users;
            queryUxDetails.UpdatedDate = DateTime.Now;
            _dbeEntities.UxStandardDetailsInfoes.AddOrUpdate(queryUxDetails);
            _dbeEntities.SaveChanges();
