...
{
    [Authorize(Roles = "PMHEAD,PM,SA,CM,MM,CPSD,ASPM,ASPMHEAD,CPSDHEAD,ACCNT,INV,INVHEAD,HW,HWHEAD,QC,QCHEAD")]
    public class ProjectManagerController : Controller
    {
        ...
        public ActionResult ProjectForwardToSoftWareView(long projectId = 0)
        {
            ...
            if (assigendProjectPmInfo.CmnUserModel.RoleName == "ASPMHEAD" || assigendProjectPmInfo.CmnUserModel.RoleName == "ASPM" || assigendProjectPmInfo.CmnUserModel.UserName == "45237")
            {
        ...
        public static DataTable GetFirstRowForPmExcel(string projectId, string softVersionName, long testIds, int swVersionNo)
        {
            ...
            //var cn =
            //    new SqlConnection(
            //        "Data Source=192.168.2.123;initial catalog=CellPhoneProject;persist security info=True;user id=sa;password=r&d!@#456;MultipleActiveResultSets=True;App=EntityFramework");
            //var cn =
            //  new SqlConnection(
            //      "Data Source=192.168.100.97;initial catalog=CellPhoneProject;persist security info=True;user id=sa;password=Adios82amigo;MultipleActiveResultSets=True;App=EntityFramework");
...
//                order by IssueSerial asc", projectId.Trim(), softVersionName);
       ...
       Microsoft.Office.Interop.Excel.XlVAlign.xlVAlignCenter;
        ...
        // public static DataTable GetFirstRow(string projectId, string softVersionName)
        // {
        //     // DataTable mdl = new DataTable();
        //     DataTable totalhistry = new DataTable();
        ...
        //     if (projectId == softVersionName)
        //     {
        //         sql = String.Format(@" select * from CellPhoneProject.dbo.SwQcIssueDetails where ProjectName='{0}' and IsApprovedForChina=1 order by SoftwareVersionNo asc ", projectId, softVersionName);
        //     }
        //     else
        //     {
        //         sql = String.Format(@" select * from CellPhoneProject.dbo.SwQcIssueDetails where ProjectName='{0}' and IsApprovedForChina=1  and SoftwareVersionName='{1}' order by SoftwareVersionNo asc", projectId, softVersionName);
        //     }
        ...
        //     return totalhistry;
        // }
        // public void GetExcel(Excel.DataTable ds, string projectId)
        // {
        //     var vms = new VmSwQcSpecificationModified();
        ...
        //                     var qcF = excelWorkSheet.Cells[2, 8].Value2;
        //                     var supF = excelWorkSheet.Cells[2, 12].Value2;
        ...
        public DataTable GetFirstRow(DateTime sdate, DateTime edate, string modelname)
        {
            ...
            if (btrcmodelinfo != null)
            {
                //var cn =
                //   new SqlConnection(
                //       "Data Source=192.168.100.97;initial catalog=RBSYNERGY;persist security info=True;user id=sa;password=Adios82amigo;MultipleActiveResultSets=True;App=EntityFramework");
                var cn = new SqlConnection(_connectionStringCellphone);
        ...
        public DataTable GetIMEIRecords(string modelname, string stdate, string eddate)
        {
            ...
            if (btrcmodelinfo != null)
            {
                var cn =
                   new SqlConnection(
                       "Data Source=192.168.200.102;initial catalog=WCMS;persist security info=True;user id=sa;password=Admin@19;MultipleActiveResultSets=True;App=EntityFramework");
        ...
        public ActionResult UserExperienceEntryPhase(string swQcHeadIds, string proName, string proIds)
        {
            FileManager fileManager = new FileManager();

            var vmModel = new VmUserExperienceTest();
            ViewBag.ProNames = proName;
            ViewBag.SwQcHeadAssignId = swQcHeadIds;
            ViewBag.ProIds = proIds;
            vmModel.SwQcHeadAssignId = Convert.ToInt64(swQcHeadIds);
            vmModel.ProjectMasterId = Convert.ToInt64(proIds);
            vmModel.ProjectName = proName;

            vmModel.UxDetailsInfoModels = _projectManagerRepository.GetUxOptions(swQcHeadIds);
            vmModel.UxStandardModels = _projectManagerRepository.GetUxStandard(swQcHeadIds);
            vmModel.UxStandardModels1 = _projectManagerRepository.GetUxStandardDetails(swQcHeadIds);

            if (vmModel.UxStandardModels1.Any())
            {
                foreach (var model in vmModel.UxStandardModels1)
                {
                    if (model.Attachments != null)
                    {
                        var urls = model.Attachments.Split('|').ToList();
                        for (int i = 0; i < urls.Count; i++)
                        {
        ...
        public ActionResult UserExperienceTestList()
        {
            var vmModel = new VmUserExperienceTest();

            vmModel.PmQcAssignModels = _projectManagerRepository.GetPmAssignedProjectForUx();

            return View(vmModel);
        }
        [HttpPost]
        public ActionResult UserExperienceEntryPhase(VmUserExperienceTest models)
        {
            var _dbEntities = new CellPhoneProjectEntities();
            long userId = Convert.ToInt64(System.Web.HttpContext.Current.User.Identity.Name);
            ...
            for (int i = 0; i < uploadedFiles.Count; i++)
            {
                ...
                HttpPostedFileBase File1 = userPostedFile;

                if (File1.ContentLength > 0 && File1.FileName != "")
                {
                    var res = manager.Upload(models.SwQcHeadAssignId,"UxExList", "UxEx", File1);
                    Console.Write("res  :" + res);

                    models.Attachments = models.Attachments == null? res: models.Attachments + "|" + res;
                }
            }
            var savedata = _projectManagerRepository.SaveUserExperienceData(models);

            return RedirectToAction("UserExperienceEntryPhase", new { swQcHeadIds = models.SwQcHeadAssignId, proName = models.ProjectName, proIds=models.ProjectMasterId });
            //return View(models);
        }

        [HttpPost]
        public JsonResult UserExperienceTestMail(string swQcHeadAssignId)
        {
            long swQcIds;
            long.TryParse(swQcHeadAssignId, out swQcIds);
            var saveDatas = "";
            if (swQcIds != 0)
            {
                saveDatas = _projectManagerRepository.GetUxInforForMail(swQcIds);
            }
            return new JsonResult { Data = saveDatas, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }
        
        [HttpPost]
        public JsonResult UserExperienceAttachmentDelete(string swQcHeadAssignId, string projectName, string projectMasterId)
        {
            long swQcIds;
            long.TryParse(swQcHeadAssignId, out swQcIds);
            var saveDatas = "";
            if (swQcIds != 0)
            {
                saveDatas = _projectManagerRepository.UserExperienceAttachmentDelete(swQcIds);
            }
            return new JsonResult { Data = saveDatas, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }
        #endregion
