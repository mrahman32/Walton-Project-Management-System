...
namespace ProjectManagement.Controllers
{
    ...
    [Authorize(Roles = "CM, CMBTRC, SA, CMHEAD,SPRHEAD,PM,PMHEAD, ACCNT,ACCNTHEAD,FIN,FINHEAD,CEO,PS, MM,AUDHEAD,AUD,BIHEAD,BI")]
    ...
    public class CommercialController : Controller
    {
                ...
                if (model.ProjectPurchaseOrderFormModel.ProjectPurchaseOrderFormId > 0)
                {
                    ...
                    if (isUpdated)
                    {
                        ...
                        if (oldPoInfo.Quantity != model.ProjectPurchaseOrderFormModel.Quantity)
                        {
                            ...
                            SendSmsViaRobi("01755612002", message);
                            //===Email notification===
        ...
        public ActionResult ProjectEvents(string searchString, string fromDate, string toDate)
        {
            ...
            ToDate = !toDateConverted ? DateTime.MaxValue : ToDate.AddMonths(1).AddDays(-1);
        ...
        public JsonResult MonthlyIncentive(string Month, string MonNum, string Year, string TotalAmount, string objArr, string objArr1)
        {
            ...
            List<CmIncentiveModel> results2 = JsonConvert.DeserializeObject<List<CmIncentiveModel>>(objArr1);
        ...
        public JsonResult CheckDupePo(string oraclePoNo)
        {
            var v = _repository.CheckDupePo(oraclePoNo);
            return Json(v);
        }

        public JsonResult CheckDupePoOtherProduct(string oraclePoNo)
        {
            var v = _repository.CheckDupePoOtherProduct(oraclePoNo);
            return Json(v);
        }
        ...
        public ActionResult LcOpeningPermissionForOtherProduct(LcOpeningOtherProductViewModel m)
        {
            ...
            if (m.LcOpeningPermissionOtherProductModel.Id == 0)
            {
                mailSendFromPms.SendMail(new List<string>(new[] { "MM", "CM", "CMHEAD", "CEO", "ACCNTHEAD", "FINHEAD", "BIHEAD" }),
                    ...
                    "New LC opening permission for " + m.LcOpeningPermissionOtherProductModel.ProductType + " (Oracle PO " +
                    m.LcOpeningPermissionOtherProductModel.OraclePoNo + ") opened",
                    "This is to inform you that, LC permission for " +
                    m.LcOpeningPermissionOtherProductModel.ProductType + "(Oracle PO no" +
                    m.LcOpeningPermissionOtherProductModel.OraclePoNo + ") opened by <b>" + userInfo.UserFullName +
                    "</b>.<br/>"
            ...
            else
            {
                mailSendFromPms.SendMail(new List<string>(new[] { "MM", "CM", "CMHEAD", "CEO", "ACCNTHEAD", "FINHEAD", "BIHEAD" }),
                    new List<string>(new[] { "PS" }),
                    "LC opening permission for " + m.LcOpeningPermissionOtherProductModel.ProductType + " (Oracle PO " +
                    m.LcOpeningPermissionOtherProductModel.OraclePoNo + ") has been updated",
                    "This is to inform you that, LC permission for " +
                    m.LcOpeningPermissionOtherProductModel.ProductType + "(Oracle PO no " +
                    m.LcOpeningPermissionOtherProductModel.OraclePoNo + ") updated by <b>" + userInfo.UserFullName +
                    "</b>.<br/>"
                    + "Product : " + m.LcOpeningPermissionOtherProductModel.Product + "<br/>"
        ...
        public ActionResult CreateLcDetailsById(long lcPermissionId = 0,long id=0)
        {
            var v = new VmProjectLc();
            if (lcPermissionId > 0)
            {
                v.LcOpeningPermissionModel = _repository.GetLcPermissionDetailsById(lcPermissionId);
                v.LcOpeningPermissionDetailModels = _repository.GetLcPermissionDetailByPermId(lcPermissionId);
                return View(v);
            }
            if (id > 0)
            {
                v.LcOpeningPermissionDetailModel = _repository.GetLcPermissionDetailById(id);
                if (v.LcOpeningPermissionDetailModel.PermissionId != null)
                    v.LcOpeningPermissionModel = _repository.GetLcPermissionDetailsById((long)v.LcOpeningPermissionDetailModel.PermissionId);
                if (v.LcOpeningPermissionDetailModel.PermissionId != null)
                    v.LcOpeningPermissionDetailModels = _repository.GetLcPermissionDetailByPermId((long)v.LcOpeningPermissionDetailModel.PermissionId);
                return View(v);
            }
            return RedirectToAction("LcPermissionList");
        }
        ...
        [HttpPost]
        public ActionResult CreateLcDetailsById(VmProjectLc lc)
        {
            long userId = Convert.ToInt64(User.Identity.Name);
            var userInfo = _hardwareRepository.GetUserInfoByUserId(userId);
            lc.LcOpeningPermissionDetailModel.PermissionId = lc.LcOpeningPermissionModel.Id;
            lc.LcOpeningPermissionDetailModel.ProjectId = lc.LcOpeningPermissionModel.ProjectMasterId;
            lc.LcOpeningPermissionDetailModel.OraclePoNo = lc.LcOpeningPermissionModel.OraclePoNo;
            if (lc.LcOpeningPermissionDetailModel.Id == 0)
            {
                lc.LcOpeningPermissionDetailModel.Added = userId;
                lc.LcOpeningPermissionDetailModel.AddedDate = DateTime.Now;
                //===email notification====
                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(new List<string>(new[] { "CM", "CMHEAD", "CEO", "ACCNTHEAD", "FINHEAD", "BIHEAD" }), new List<string>(new[] { "MM", "PS" }), "New LC created for " + lc.LcOpeningPermissionModel.Model + "(Oracle PO " + lc.LcOpeningPermissionModel.OraclePoNo + ")",
            "This is to inform you that, LC permission for " + lc.LcOpeningPermissionModel.Model + "(Oracle PO " + lc.LcOpeningPermissionModel.OraclePoNo + ") opened by <b>" + userInfo.UserFullName + "</b>.<br/>"
            + "LC Value : "+ lc.LcOpeningPermissionDetailModel.Currency + " " + lc.LcOpeningPermissionDetailModel.LcValue);
                //=========================
            }
            if (lc.LcOpeningPermissionDetailModel.Id > 0)
            {
                lc.LcOpeningPermissionDetailModel.Updated = userId;
                lc.LcOpeningPermissionDetailModel.UpdatedDate = DateTime.Now;
                //===email notification====
                var mailSendFromPms = new MailSendFromPms();
                mailSendFromPms.SendMail(new List<string>(new[] { "CM", "CMHEAD", "CEO", "ACCNTHEAD", "FINHEAD", "BIHEAD" }), new List<string>(new[] { "MM", "PS" }), "LC data for " + lc.LcOpeningPermissionModel.Model + "(Oracle PO " + lc.LcOpeningPermissionModel.OraclePoNo + ")",
            "This is to inform you that, LC data for " + lc.LcOpeningPermissionModel.Model + "(Oracle PO " + lc.LcOpeningPermissionModel.OraclePoNo + ") updated by <b>" + userInfo.UserFullName + "</b>.<br/>"
            + "LC Value : " + lc.LcOpeningPermissionDetailModel.Currency + " " + lc.LcOpeningPermissionDetailModel.LcValue);
                //=========================
            }
            _repository.SaveOrUpdateLcOpeningPermissionDetail(lc.LcOpeningPermissionDetailModel);
            return RedirectToAction("CreateLcDetailsById", new { lcPermissionId = lc.LcOpeningPermissionModel.Id });
        }
        ...
        public ActionResult SpecFromBtrcExcelUpload()
        {
            ViewBag.Specs = _repository.GetInfoFromBtrc();
            return View();
        }

        [HttpPost]
        public ActionResult UploadBtrcSpec(ForeignModelInfoFromBtrcModel model)
        {
            long userId = Convert.ToInt64(System.Web.HttpContext.Current.User.Identity.Name == "" ? "0" : System.Web.HttpContext.Current.User.Identity.Name);
            if (Request != null)
            {
                //file = Request.Files["IDH_BOM_upload"];
                var extension = model.BtrcFile != null ? Path.GetExtension(model.BtrcFile.FileName) : null;
                if (extension != null)
                {
                    var fileExt = extension.Substring(1);
                    if ((model.BtrcFile != null) && (model.BtrcFile.ContentLength > 0) && !string.IsNullOrEmpty(model.BtrcFile.FileName) && (fileExt == "xlsx" || fileExt == "xls"))
                    {
                        using (var pack = new ExcelPackage(model.BtrcFile.InputStream))
                        {
                            var currentSheet = pack.Workbook.Worksheets;
                            var workSheet = currentSheet.First();
                            var noOfRow = workSheet.Dimension.End.Row;
                            ...
                            for (var i = 2; i <= noOfRow; i++)
                            {
                                var spec = new ForeignModelInfoFromBtrcModel();
                                //excel data
                                spec.Brand = workSheet.Cells[i, 1].Value.ToString();
                                spec.ModelName = workSheet.Cells[i, 2].Value.ToString();
                                spec.MarketingName = workSheet.Cells[i, 3].Value==null?null:workSheet.Cells[i, 3].Value.ToString();
                                spec.Color = workSheet.Cells[i, 4].Value == null ? null : workSheet.Cells[i, 4].Value.ToString();
                                spec.DeviceType = workSheet.Cells[i, 5].Value == null ? null : workSheet.Cells[i, 5].Value.ToString();
                                spec.AppRef = workSheet.Cells[i, 6].Value == null ? null : workSheet.Cells[i, 6].Value.ToString();
                                spec.CountryOfOrigin = workSheet.Cells[i, 7].Value == null ? null : workSheet.Cells[i, 7].Value.ToString();
                                spec.ManfAsPerIMEITac = workSheet.Cells[i, 8].Value == null ? null : workSheet.Cells[i, 8].Value.ToString();
                                spec.NoOfSim = workSheet.Cells[i, 9].Value == null ? null : workSheet.Cells[i, 9].Value.ToString();
                                spec.BatteryCapacity = workSheet.Cells[i, 10].Value == null ? null : workSheet.Cells[i, 10].Value.ToString();
                                spec.BatteryCapacityTested = workSheet.Cells[i, 11].Value == null ? null : workSheet.Cells[i, 11].Value.ToString();
                                spec.ChargerType = workSheet.Cells[i, 12].Value == null ? null : workSheet.Cells[i, 12].Value.ToString();
                                spec.ChargerOutput = workSheet.Cells[i, 13].Value == null ? null : workSheet.Cells[i, 13].Value.ToString();
                                spec.Processor = workSheet.Cells[i, 14].Value == null ? null : workSheet.Cells[i, 14].Value.ToString();
                                spec.Ram = workSheet.Cells[i, 15].Value == null ? null : workSheet.Cells[i, 15].Value.ToString();
                                spec.Rom = workSheet.Cells[i, 16].Value == null ? null : workSheet.Cells[i, 16].Value.ToString();
                                spec.Nfc = workSheet.Cells[i, 17].Value == null ? null : workSheet.Cells[i, 17].Value.ToString();
                                spec.Bluetooth = workSheet.Cells[i, 18].Value == null ? null : workSheet.Cells[i, 18].Value.ToString();
                                spec.Wlan = workSheet.Cells[i, 19].Value == null ? null : workSheet.Cells[i, 19].Value.ToString();
                                spec.DataSpeed = workSheet.Cells[i, 20].Value == null ? null : workSheet.Cells[i, 20].Value.ToString();
                                spec.SarValue = workSheet.Cells[i, 21].Value == null ? null : workSheet.Cells[i, 21].Value.ToString();
                                spec.RearCamera = workSheet.Cells[i, 22].Value == null ? null : workSheet.Cells[i, 22].Value.ToString();
                                spec.FrontCamera = workSheet.Cells[i, 23].Value == null ? null : workSheet.Cells[i, 23].Value.ToString();
                                spec.CameraResolution = workSheet.Cells[i, 24].Value == null ? null : workSheet.Cells[i, 24].Value.ToString();
                                spec.RadioInterface = workSheet.Cells[i, 25].Value == null ? null : workSheet.Cells[i, 25].Value.ToString();
                                spec.SpectrumBands2G = workSheet.Cells[i, 26].Value == null ? null : workSheet.Cells[i, 26].Value.ToString();
                                spec.SpectrumBands3G = workSheet.Cells[i, 27].Value == null ? null : workSheet.Cells[i, 27].Value.ToString();
                                spec.SpectrumBands4G = workSheet.Cells[i, 28].Value == null ? null : workSheet.Cells[i, 28].Value.ToString();
                                spec.Motherboard = workSheet.Cells[i, 29].Value == null ? null : workSheet.Cells[i, 29].Value.ToString();
                                spec.ChipsetModel = workSheet.Cells[i, 30].Value == null ? null : workSheet.Cells[i, 30].Value.ToString();
                                spec.OperatingSystem = workSheet.Cells[i, 31].Value == null ? null : workSheet.Cells[i, 31].Value.ToString();
                                spec.ImportQtyAir = workSheet.Cells[i, 32].Value == null ? null : workSheet.Cells[i, 32].Value.ToString();
                                spec.ImportQtySea = workSheet.Cells[i, 33].Value == null ? null : workSheet.Cells[i, 33].Value.ToString();
                                spec.TotalQuantity = workSheet.Cells[i, 34].Value == null ? null : workSheet.Cells[i, 34].Value.ToString();
                                spec.ProductType = workSheet.Cells[i, 35].Value == null ? null : workSheet.Cells[i, 35].Value.ToString();
                                spec.UnitPrice = workSheet.Cells[i, 36].Value == null ? null : workSheet.Cells[i, 36].Value.ToString();
                                spec.UnitPriceBdt = workSheet.Cells[i, 37].Value == null ? null : workSheet.Cells[i, 37].Value.ToString();
                                spec.MarketingPeriod = workSheet.Cells[i, 38].Value == null ? null : workSheet.Cells[i, 38].Value.ToString();
                                spec.ImeiTac1 = workSheet.Cells[i, 39].Value == null ? null : workSheet.Cells[i, 39].Value.ToString();
                                spec.ImeiTac2 = workSheet.Cells[i, 40].Value == null ? null : workSheet.Cells[i, 40].Value.ToString();
                                spec.ImeiTac3 = workSheet.Cells[i, 41].Value == null ? null : workSheet.Cells[i, 41].Value.ToString();
                                spec.ImeiTac4 = workSheet.Cells[i, 42].Value == null ? null : workSheet.Cells[i, 42].Value.ToString();
                                spec.Imei1 = workSheet.Cells[i, 43].Value == null ? null : workSheet.Cells[i, 43].Value.ToString();
                                spec.Imei2 = workSheet.Cells[i, 44].Value == null ? null : workSheet.Cells[i, 44].Value.ToString();
                                spec.Imei3 = workSheet.Cells[i, 45].Value == null ? null : workSheet.Cells[i, 45].Value.ToString();
                                spec.Imei4 = workSheet.Cells[i, 46].Value == null ? null : workSheet.Cells[i, 46].Value.ToString();
                                spec.DisplaySize = workSheet.Cells[i, 47].Value == null ? null : workSheet.Cells[i, 47].Value.ToString();
                                spec.NocDate = workSheet.Cells[i, 48].Value == null ? null : workSheet.Cells[i, 48].Value.ToString();
                                spec.AddedBy = userId;
                                spec.AddedDate = DateTime.Now;
                                _repository.SaveBtrcSpec(spec);
                            }
                        }
                    }
                }
            }
            return RedirectToAction("SpecFromBtrcExcelUpload");
        }

        #region Forecast
        public ActionResult ProjectForecasting()
        {
            var vmModel = new VmProjectForecasting();

            vmModel.ProjectMasterModels = _repository.GetAllProjectName();

            var items1 = new List<SelectListItem> { new SelectListItem { Value = "", Text = "SELECT PROJECT" } };
            items1 = vmModel.ProjectMasterModels.Select(model => new SelectListItem { Text = model.ProjectName, Value = model.ProjectMasterId.ToString(CultureInfo.InvariantCulture) }).ToList();
            ViewBag.Projects = items1;
            vmModel.ProjectForecastingInfoModels = _repository.GetProjectForecastingList();
            vmModel.ProjectForecastingInfoModels2 = _repository.GetProjectForecastingSum();
            return View(vmModel);
        }
        [HttpPost]
        public JsonResult SaveProjectForecastingData(string objArr)
        {
            List<ProjectForecastingInfoModel> results = JsonConvert.DeserializeObject<List<ProjectForecastingInfoModel>>(objArr);
            Console.Write("result :" + results);

            var saveDate = "";
            if (results.Count != 0)
            {
                saveDate = _repository.SaveProjectForecastingData(results);
            }
           // return RedirectToAction("ProjectForecasting");
            return Json(new { data = saveDate }, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult UpdateProjectForecastingData(string objArr)
        {
            List<ProjectForecastingInfoModel> results = JsonConvert.DeserializeObject<List<ProjectForecastingInfoModel>>(objArr);
            Console.Write("result :" + results);
            ...
            if (results.Count != 0)
            {
                saveDate = _repository.UpdateProjectForecastingData(results);
            }
            // return RedirectToAction("ProjectForecasting");
            return Json(new { data = saveDate }, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult DeleteProjectForecastingData(string objArr)
        {
            List<ProjectForecastingInfoModel> results = JsonConvert.DeserializeObject<List<ProjectForecastingInfoModel>>(objArr);
            Console.Write("result :" + results);
            ...
            if (results.Count != 0)
            {
                saveDate = _repository.DeleteProjectForecastingData(results);
            }
            // return RedirectToAction("ProjectForecasting");
            return Json(new { data = saveDate }, JsonRequestBehavior.AllowGet);
        }
        public JsonResult GetProjectInfoForForecasting(string proIds)
        {
            List<ProjectForecastingInfoModel> itemsPmPoIncentive = new List<ProjectForecastingInfoModel>();
            var PmPoIncentiveModels = _repository.GetProjectInfoForForecasting(proIds);
            foreach (var pmPoIncentiveModel in PmPoIncentiveModels)
            {
                ProjectForecastingInfoModel items = new ProjectForecastingInfoModel();
                items.ProjectName = pmPoIncentiveModel.ProjectName;
                items.ProjectMasterId = pmPoIncentiveModel.ProjectMasterId;
                items.Brand = pmPoIncentiveModel.Brand;
                items.Rom = pmPoIncentiveModel.Rom;
                items.Ram = pmPoIncentiveModel.Ram;
                items.Battery = pmPoIncentiveModel.Battery;
                items.Display = pmPoIncentiveModel.Display;
                items.FSegment = pmPoIncentiveModel.FSegment;
                items.ReleaseDate = pmPoIncentiveModel.ReleaseDate;
              
                itemsPmPoIncentive.Add(items);
            }
            ViewBag.GetProjectInfoForForecasting = itemsPmPoIncentive;
        ...
        //[HttpPost]
        //public ActionResult ProjectForecasting(string projectName, string launchDate, string plcVal, string brandName, string fSegmentVal,
        //    string display, string battery, string ramVal, string romVal, string fobVal, string january, string february, string march, string april,
        //    string may, string june, string july, string august, string september, string october, string november, string december)
        //{
        //    var saveDate = "";

        //    if (projectName!="" && launchDate!="")
        //    {
        //        saveDate = _repository.SaveProjectForecastingData();
        //    }
        ...
        public ActionResult SpareArrangementAndSupport()
        {
            var vmModel = new VmCommercialNewIncentivePolicy();

            vmModel.Cm_SpareArrangementAndSupportModels = _repository.GetSpareArrangementAndSupport();
            
            return View(vmModel);
        }
        [HttpPost]
        public ActionResult SpareArrangementAndSupport(VmCommercialNewIncentivePolicy model)
        {
            var savedatas = "";

            if (model.Cm_SpareArrangementAndSupportModel!=null)
            {
                savedatas = _repository.SaveSpareArrangementAndSupport(model.Cm_SpareArrangementAndSupportModel);
            }

            return RedirectToAction("SpareArrangementAndSupport");
        }
        public ActionResult MaterialReorderAndSupport()
        {
            var vmModel = new VmCommercialNewIncentivePolicy();

            vmModel.Cm_MaterialReorderAndSupportModels = _repository.GetMaterialReorderAndSupport();

            return View(vmModel);
        }
        [HttpPost]
        public ActionResult MaterialReorderAndSupport(VmCommercialNewIncentivePolicy model)
        {
            var savedatas = "";

            if (model.Cm_MaterialReorderAndSupportModel != null)
            {
                savedatas = _repository.SaveMaterialReorderAndSupport(model.Cm_MaterialReorderAndSupportModel);
            }

            return RedirectToAction("MaterialReorderAndSupport");
        }

        public ActionResult ReportAnalysis()
        {
            var vmModel = new VmCommercialNewIncentivePolicy();
            List<SelectListItem> selectListForCategory = new List<SelectListItem>();
            selectListForCategory.Add(new SelectListItem() { Text = "Select Category", Value = "0" });
            selectListForCategory.Add(new SelectListItem() { Text = "Items landed cost", Value = "Items landed cost" });
            selectListForCategory.Add(new SelectListItem() { Text = "TTI Percentage", Value = "TTI Percentage" });

            ViewBag.Categories = selectListForCategory;

            vmModel.Cm_ReportAnalysisModels = _repository.GetReportAnalysis();

            return View(vmModel);
        }
        [HttpPost]
        public ActionResult ReportAnalysis(VmCommercialNewIncentivePolicy model)
        {
            var savedatas = "";

            if (model.Cm_ReportAnalysisModel != null)
            {
                savedatas = _repository.SaveReportAnalysis(model.Cm_ReportAnalysisModel);
            }

            return RedirectToAction("ReportAnalysis");
        }

        public ActionResult NegotiationsAndDevelopment()
        {
            var vmModel = new VmCommercialNewIncentivePolicy();
            List<SelectListItem> selectListForCategory = new List<SelectListItem>();
            selectListForCategory.Add(new SelectListItem() { Text = "Select Category", Value = "0" });
            selectListForCategory.Add(new SelectListItem() { Text = "Vendor Development and contract establishment", Value = "Vendor Development and contract establishment" });
            selectListForCategory.Add(new SelectListItem() { Text = "Development", Value = "Development" });
            selectListForCategory.Add(new SelectListItem() { Text = "New Innovation", Value = "New Innovation" });

            ViewBag.Categories = selectListForCategory;

            vmModel.Cm_NegotiationsAndDevelopmentModels = _repository.GetNegotiationsAndDevelopment();

            return View(vmModel);
        }
        [HttpPost]
        public ActionResult NegotiationsAndDevelopment(VmCommercialNewIncentivePolicy model)
        {
            var savedatas = "";

            if (model.Cm_NegotiationsAndDevelopmentModel != null)
            {
                savedatas = _repository.SaveNegotiationsAndDevelopment(model.Cm_NegotiationsAndDevelopmentModel);
            }

            return RedirectToAction("NegotiationsAndDevelopment");
        }
        public ActionResult Penalties()
        {
            ...
            List<SelectListItem> selectListForCategory = new List<SelectListItem>();
            selectListForCategory.Add(new SelectListItem() { Text = "Select Category", Value = "0" });
            selectListForCategory.Add(new SelectListItem() { Text = "Product import demurrage", Value = "Product import demurrage" });
            selectListForCategory.Add(new SelectListItem() { Text = "Percentage Increased of TTI", Value = "Percentage Increased of TTI" });

            ViewBag.Categories = selectListForCategory;

            vmModel.Cm_PenaltiesModels = _repository.GetPenalties();

            return View(vmModel);
        }
        [HttpPost]
        public ActionResult Penalties(VmCommercialNewIncentivePolicy model)
        {
            var savedatas = "";

            if (model.Cm_PenaltiesModel != null)
            {
                savedatas = _repository.SavePenalties(model.Cm_PenaltiesModel);
            }

            return RedirectToAction("Penalties");
        }

        public ActionResult CommercialIncentiveGenerate(string months, string years)
        {
            var model = new VmCommercialNewIncentivePolicy();
            //
            List<SelectListItem> selectListItemsMonth = new List<SelectListItem>();
            selectListItemsMonth.Add(new SelectListItem() { Text = "SELECT MONTH", Value = "0" });
            selectListItemsMonth.Add(new SelectListItem() { Text = "January", Value = "1" });
            ...
            selectListItemsYear.Add(new SelectListItem() { Text = "2029", Value = "2029" });
            selectListItemsYear.Add(new SelectListItem() { Text = "2030", Value = "2030" });
            selectListItemsYear.Add(new SelectListItem() { Text = "2031", Value = "2031" });
            selectListItemsYear.Add(new SelectListItem() { Text = "2032", Value = "2032" });
            selectListItemsYear.Add(new SelectListItem() { Text = "2033", Value = "2033" });
            ...
            model.Year = years;
            //
            model.Cm_SpareArrangementAndSupportModels = _repository.GetSpareArrangementAndSupportIncentive(months,years);
            model.Cm_MaterialReorderAndSupportModels = _repository.GetMaterialReorderAndSupportIncentive(months, years);
            model.Cm_ReportAnalysisModels = _repository.GetReportAnalysisIncentive(months, years);
            model.Cm_NegotiationsAndDevelopmentModels = _repository.GetNegotiationsIncentive(months, years);
            model.Cm_PenaltiesModels = _repository.GetPenaltiesIncentive(months, years);
            model.Cm_MaterialShipmentAndConsumableItemsModels = _repository.GetMaterialShipmentAndConsumableItems(months, years);

            Decimal tAmount = 0;
            Decimal tReward = 0;
            Decimal tPenalties = 0;


            if (model.Cm_MaterialShipmentAndConsumableItemsModels.Any())
            {
                var modelss = new Cm_MaterialShipmentAndConsumableItemsModel();
                foreach (var mods in model.Cm_MaterialShipmentAndConsumableItemsModels)
                {

                    tAmount += Convert.ToDecimal(mods.Amount);
                    tReward += Convert.ToDecimal(mods.Reward);
                    if (mods.Remarks != null && mods.Remarks != "")
                    {
                        tPenalties += 0;
                    }
                    else
                    {
                        tPenalties += Convert.ToDecimal(mods.Penalties);
                    }
                }
               
                modelss.ProjectName = "Total Amount:";
                modelss.Amount = tAmount;
                modelss.Reward = tReward;
                if (modelss.Remarks != null && modelss.Remarks != "")
                {
                    modelss.Penalties = 0;
                }
                else
                {
                    modelss.Penalties = -tPenalties;
                }
                
                modelss.FinalAmount = modelss.Amount;
                if (modelss.Reward > 6000)
                {
                    Decimal fAmm = 0;
                    modelss.Reward = 6000;
                    fAmm = Convert.ToDecimal(modelss.FinalAmount) + Convert.ToDecimal(modelss.Reward);
                    modelss.FinalAmount = fAmm;
                }
                else if (modelss.Reward <= 6000)
                {
                    Decimal fAmm = 0;
                    fAmm = Convert.ToDecimal(modelss.FinalAmount) + Convert.ToDecimal(modelss.Reward);
                    modelss.FinalAmount = fAmm;
                ...
                if (-modelss.Penalties < -2000)
                {
                    Decimal fAmm = 0;
                    modelss.Penalties = -2000;
                    fAmm = Convert.ToDecimal(modelss.FinalAmount) + Convert.ToDecimal(modelss.Penalties);
                    modelss.FinalAmount = fAmm;
                }
                else if (-modelss.Penalties >= -2000)
                {
                    Decimal fAmm = 0;
                    modelss.Penalties = -modelss.Penalties;
                    fAmm = Convert.ToDecimal(modelss.FinalAmount) + Convert.ToDecimal(modelss.Penalties);
                    modelss.FinalAmount = fAmm;
                }

                model.Cm_MaterialShipmentAndConsumableItemsModels.Add(modelss);
                //
            }

            model.Cm_TotalPoLcAndTtModels = _repository.GetPoLcAnTt(months, years);
            model.Cm_UserInfosForNewPolicyModels = _repository.GetCmUserListForNewPolicy(months, years);
            model.Cm_UserInfosForNewPolicyModels2 = _repository.CheckIncentiveEntry(months, years);

            return View(model);
        }
        public ActionResult CommercialIncentiveSheet()
        {
            return View();
        }
        public ActionResult CommercialIncentiveDetailsReport(string monId, string month, string yearId)
        {
            long yearIds;
            long.TryParse(yearId, out yearIds);

            ViewBag.GetReports = _repository.GetIncentiveReport2(monId, yearIds);
            ViewBag.GetReports1 = _repository.GetIncentiveReport1(monId, yearIds);
            ViewBag.GetPreparedUser = _repository.GetPreparedUserName();
            return View();
        }
        ...
        [HttpPost]
        public JsonResult SaveCommercialMonthlyIncentiveNew(string Month, string MonNum, string Year, string TotalAmount, string objArr21, string objArr)
        {
            List<Cm_UserInfosForNewPolicyModel> results = JsonConvert.DeserializeObject<List<Cm_UserInfosForNewPolicyModel>>(objArr21);
            Console.Write("result21 :" + results);

            List<Cm_MaterialShipmentAndConsumableItemsModel> results2 = JsonConvert.DeserializeObject<List<Cm_MaterialShipmentAndConsumableItemsModel>>(objArr);
            Console.Write("result2 :" + results2);

            var saveIncentive21 ="";
            if (results.Count!=0)
            {
                saveIncentive21 = _repository.SaveCommercialMonthlyIncentiveNew(Month, MonNum, Year, TotalAmount, results, results2);
            }

            return new JsonResult { Data = saveIncentive21, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }

        public ActionResult CommercialIncentiveDescription(string months, string years)
        {
            var model = new VmCommercialNewIncentivePolicy();
            List<SelectListItem> selectListItemsMonth = new List<SelectListItem>();
            selectListItemsMonth.Add(new SelectListItem() { Text = "SELECT MONTH", Value = "0" });
            ...
            model.Cm_TotalPoLcAndTtModels = _repository.GetTotalPoLcAndTt(months,years);
            ...
            //GetIncentiveReport2
            ViewBag.GetReports = _repository.GetIncentiveReport2(months, yearIds);
            return View(model);
