...
{
    [Authorize(
        Roles = "CM, CMHEAD, CMBTRC, SA, PM, PMHEAD, HWHEAD, HW, QC, QCHEAD, CMBTRC, MM, BRND, SPRHEAD, SPR, MKTHEAD, MKT, PS, PRD, WAR, WARHEAD,CPSD,CPSDHEAD,ASPM,ASPMHEAD,PRC,PRCHEAD,BDIQC,BDIQCHEAD,INV,INVHEAD,ACCNT,ACCNTHEAD,FIN,FINHEAD,CEO,AUD,AUDHEAD,SALES,SALESHEAD,COO,CBO,BIHEAD,PDP,BI")]
    public class CommonController : Controller
    {
        ...
        public JsonResult UpdateProjectVariant(string variantName, string ramVendor, string romVendor, long variantQuantity = 0, long variantId = 0,long projectId = 0)
        {
            long userId;
            long.TryParse(HttpContext.User.Identity.Name, out userId);
            ...
            var model = new ProjectOrderQuantityDetailModel
            {
                ...
                OrderQuantity = variantQuantity,
                IsActive = true,
                AddedBy = prevVariantInfo.AddedBy,
                AddedDate = prevVariantInfo.AddedDate,
                UpdatedBy = userId,
                UpdatedDate = DateTime.Now,
                RamVendor = ramVendor,
                RomVendor = romVendor
        ...
        public JsonResult DeleteVariant(long id = 0)
        {
            long userId;
            long.TryParse(HttpContext.User.Identity.Name, out userId);
            var variantInfo = _commonRepository.GetOrderQuantityDetailById(id);
            var project = _commonRepository.GetProjectInfoByProjectId((long)variantInfo.ProjectMasterId);
            var v = _commonRepository.DeleteVariant(id);
            //===email notification====
            MailSendFromPms mailSendFromPms = new MailSendFromPms();
            var usr = _hardwareRepository.GetUserInfoByUserId(userId);
            mailSendFromPms.SendMail(new List<long>(new[] { userId }), new List<string>(new[] { "PS", "PM", "PMHEAD", "CMHEAD", "CM" }), "Variant quantity has been deleted",
            "This is to inform you that, variant <b>" + variantInfo.ProjectModel + " </b> has been deleted by <b>" + usr.UserFullName + ".  VariantQuantity <b>" + variantInfo.OrderQuantity + "</b>. Please assign this quantity to proper variant.<br/><br/><br/><br/>"
                       + "Project : <b>" + project.ProjectName
                       + ", Order No : <b>" + project.OrderNuber
                       + "<br/>Total Order Quantity - " + project.OrderQuantity);
            //==========
        ...
        public ActionResult LcOpeningApproval()
        {
            long userId = Convert.ToInt64(User.Identity.Name);
            var userInfo = _hardwareRepository.GetUserInfoByUserId(userId);
            List<LcOpeningPermissionModel> lcOpeningPermissionModels;
            //lc initialtion part
            var roleDesc = _commonRepository.GetDescriptionByRole(userInfo.RoleName);
            if (roleDesc == "Project Manager" || roleDesc == "Production" || roleDesc == "ProductionPlanning" || roleDesc == "Hardware")
            {
                lcOpeningPermissionModels = _commonRepository.GetOpeningPermissionByRoleDesc(roleDesc);
                ViewBag.OtherProductPermission = _commonRepository.GetOtherProdPermByRoleDesc(roleDesc);
                return View(lcOpeningPermissionModels);
            }
            //---for commercial show all--
            if (User.IsInRole("CM"))
            {
                lcOpeningPermissionModels = _commercialRepository.GetLcPermissionList();
                ViewBag.OtherProductPermission = _commercialRepository.GetLcPermissionOtherProductList();
                return View(lcOpeningPermissionModels);
            }
            //---Approval part starts---
            lcOpeningPermissionModels = _commercialRepository.GetLcPermissionList();
            //lcOpeningPermissionModels = _commercialRepository.GetLcOpeningPermWithInitiationDone();
            List<LcOpeningPermissionOtherProductModel> otherLcOpening = _commercialRepository.GetLcPermissionOtherProductList();
            //List<LcOpeningPermissionOtherProductModel> otherLcOpening = _commercialRepository.GetLcOpeningOtherPermWithInitiationDone();
            ViewBag.OtherProductPermission = otherLcOpening;
            if (User.IsInRole("MM"))
            {
                lcOpeningPermissionModels = lcOpeningPermissionModels.Where(x => x.ApprovedBy == null && x.BiApprovalBy != null && x.TotalAmount > 10000).ToList();
                ViewBag.OtherProductPermission = otherLcOpening.Where(x => x.ApprovedBy == null && x.BiApprovalBy != null && x.TotalAmount > 10000).ToList();
            }
            if (User.IsInRole("PS"))//not required
            {
            ...
            {
                lcOpeningPermissionModels = lcOpeningPermissionModels.Where(x => x.BiApprovalBy == null && x.ApprovedBy == null).ToList();
                ...
                ViewBag.OtherProductPermission = otherLcOpening.Where(x => x.BiApprovalBy == null && x.ApprovedBy == null).ToList();
        ...
        public JsonResult ApproveLc(string remarks, long id = 0)
        {
            ...
            if (User.IsInRole("PM") || User.IsInRole("PMHEAD") || User.IsInRole("PRD") || User.IsInRole("PRDHEAD") ||
                User.IsInRole("PDP") || User.IsInRole("HWHEAD") || User.IsInRole("HW"))
            {
                var app = new LcOpeningApprovalDetail();
                app.Role = addedByDetails.RoleName;
                app.ApproveDate = DateTime.Now;
                app.ApprovedBy = userId;
                app.ApprovedByName = addedByDetails.UserFullName+" ("+addedByDetails.EmployeeCode+")";
                app.AddedBy = userId;
                app.AddedDate = DateTime.Now;
                app.Remarks = remarks;
                app.LcOpeningPermissionId = id;
                _commonRepository.SaveLcOpeningApprovalDetail(app);
            }
            //===email notification====
            MailSendFromPms mailSendFromPms = new MailSendFromPms();
            var project = _commonRepository.GetProjectInfoByProjectId(model.ProjectMasterId??0);
            mailSendFromPms.SendMail(new List<string>(new[] { "CM","CMHEAD","ACCNT","ACCNTHEAD","FIN","FINHEAD","CEO","MM","PS","BIHEAD","PM","PMHEAD","PRD","PRDHEAD","PDP","HWHEAD","HW" }), new List<string>(new[] { "" }), "LC Opening Approved for "+project.ProjectName+"(Order "+project.OrderNuber+")",
            "This is to inform you that, LC opening approved by <b>" + addedByDetails.UserFullName + "</b>.<br/>"
                       + "Project : " + project.ProjectName + ",<br/>"
                       + "Oracle PO : " + model.OraclePoNo + ",<br/>"
        ...
        public JsonResult ApproveOtherLc(string remarks, long id = 0)
        {
            ...
            if (User.IsInRole("PM") || User.IsInRole("PMHEAD") || User.IsInRole("PRD") || User.IsInRole("PRDHEAD") ||
                User.IsInRole("PDP") || User.IsInRole("HWHEAD") || User.IsInRole("HW"))
            {
                var app = new LcOpeningApprovalOtherProductDetail();
                app.Role = addedByDetails.RoleName;
                app.ApproveDate = DateTime.Now;
                app.ApprovedBy = userId;
                app.ApprovedByName = addedByDetails.UserFullName + " (" + addedByDetails.EmployeeCode + ")";
                ...
                app.Remarks = remarks;
                app.LcOpeningPermissionId = id;
                _commonRepository.SaveLcOpeningApprovalOtherProductDetail(app);
            }
        ...
        public ActionResult DownloadOtherProductLcPermissionFile(long id = 0)
        {
            var model = _commonRepository.GetOpeningPermissionOtherFileById(id);
            var manager = new FileManager();
            var path = manager.GetPoFeedbackFile(model.FilePath);
            var apppath = HttpContext.Server.MapPath(path);//HttpRuntime.AppDomainAppPath;
            string fileName = Path.GetFileName(path);
        ...
        public ActionResult WpmsAllProjects(string ProjectName, string Orders, string ProStatus, string InitialApproval)
        {
           ...
           //ViewBag.Projects = items1;
            ...
            items5.Add(new SelectListItem() { Text = "Action Not taken By CBO", Value = "Action Not taken By CBO" });
        ...
        #endregion
        #region LC Approval Manager

        public ActionResult LcApprovalManager()
        {
            ViewBag.RoleDesc = _commonRepository.RoleDescriptionList();
            ViewBag.Apps = _commonRepository.GetLcApprovalManagerModels();
            return View();
        }

        [HttpPost]
        public ActionResult LcApprovalManager(LcApprovalManagerModel mod)
        {
            long userId = Convert.ToInt64(User.Identity.Name);
            mod.AddedBy = userId;
            mod.AddedDate = DateTime.Now;
            _commonRepository.SaveLcApprovalManager(mod);
            return RedirectToAction("LcApprovalManager");
        }
        #endregion

        public ActionResult RelevantModelPicker()
        {
            var variants = _commonRepository.RelevantModelPickPendingProjectsForLc();
            var variantList = new List<SelectListItem>();
            foreach (var v in variants)
            {
                variantList.Add(new SelectListItem
                {
                    Value = v.ProjectMasterId.ToString(),
                    Text = v.Model
                });
            }
            ViewBag.variants = variantList;
            return View();
        }

        public JsonResult GetProjectInfoById(long projectId)
        {
            var master = _commonRepository.GetProjectInfoByProjectId(projectId);
            var jigsPrice = _commonRepository.GetJigsPriceByProjectName(master.ProjectName);
            var accessoriesPrice = _commonRepository.GetAccessoriesPricesByProjectId(projectId);//Accessories Price
            return Json(new{master,accessoriesPrice,jigsPrice});
        }

        public ActionResult ProjectImages()
        {
            var data = _commonRepository.GetProjectImages();
            return View(data);
        }
    }
