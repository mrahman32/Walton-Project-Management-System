...
namespace ProjectManagement.Controllers
{
    ...
    public class SoftwareController : Controller
    {
        ...
        public static DataTable GetFirstRow(string projectId, string softVersionName, long testIds, int swVersionNo)
        {
            ...
            //var cn =new SqlConnection("Data Source=192.168.100.97;initial catalog=CellPhoneProject;persist security info=True;user id=sa;password=Adios82amigo;MultipleActiveResultSets=True;App=EntityFramework");
            //var cn =
            //    new SqlConnection(
            ...
            else
            {
                ...
                if (testIds != 5 || testIds !=47)
                {
                    sql = String.Format(@" select (select top 1 'Fixed in Demo_SW_V_'+cast(si1.SoftwareVersionNo as varchar(10))+'_Demo' from
                    CellPhoneProject.dbo.SwQcIssueDetails si1 where si1.ProjectName='{0}' 
                    and si1.TestPhaseID=5 and si1.WaltonQcStatus='FIXED' and si1.IssueSerial=sii.IssueSerial
                     order by si1.SwQcIssueId asc) as Demo2, * from CellPhoneProject.dbo.SwQcIssueDetails sii where ProjectName='{0}' and IsApprovedForChina=1  and SoftwareVersionName='{1}' and SoftwareVersionNo='{2}' and TestPhaseID not in (5,47) order by SoftwareVersionNo,IssueSerial asc", projectId.Trim(), softVersionName.Trim().Trim(), swVersionNo);
