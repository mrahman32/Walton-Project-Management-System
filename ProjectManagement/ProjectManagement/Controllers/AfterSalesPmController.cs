...
{
    [Authorize(Roles = "ASPM,ASPMHEAD,ACCNT,HW,HWHEAD")]
    public class AfterSalesPmController : Controller
    {
       ...
       public JsonResult SaveSupplierDetails(String IssueVerificationId, String Details)
       {
        ...
        public ActionResult AftersalesPmHarwareIssue()
        {
            ...
            vmModel.AftersalesPmHwIssueVerificationModels = _afterSales.GetHwIssueVerificationList();
        ...
        [HttpPost]
        public JsonResult SoftwareVersionNameLoadForAll1(string swVersionNo, string projectId)
        {
            var _dbEntities = new CellPhoneProjectEntities();
            long userId = Convert.ToInt64(System.Web.HttpContext.Current.User.Identity.Name);
            var user = _dbEntities.CmnUsers.FirstOrDefault(i => i.CmnUserId == userId);
            ...
            var getVersionName = _afterSales.GetAllVersionNameForPm1(swVerNo, proId);
        ...
        [HttpPost]
        public JsonResult SaveAftersalesPmHarwareIssue(string projectName, string projectId, string swVersionNo, string versionName, string moduleName,
            string issueFrequency, string issueType, string issueDetails, string testingPath, string resultFound, string data1, string issueOccRatio,
            string numberOfHSsChecked, string remarks, string numberOfHSsReturn)
        {
            long ids;
            long.TryParse(projectId, out ids);

            var saveData = "0";

            if (ids != 0)
            {
                saveData = _afterSales.SaveAftersalesPmHarwareIssue(ids, projectName,  swVersionNo,  versionName,  moduleName,
                issueFrequency,  issueType,  issueDetails,  testingPath,  resultFound,  data1,  issueOccRatio,numberOfHSsChecked,  remarks,  numberOfHSsReturn);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult AftersalesPmHarwareIssueDetails(string issueIds)
        {
            long issueId;
            long.TryParse(issueIds, out issueId);

            var vmModel = new VmIssueVerification();

           // vmModel.AftersalesPmHwIssueVerificationModels = _afterSales.GetPerHwIssue(issueId);
            vmModel.AftersalesPm_HwIssuePlaceRequirements = _afterSales.GetPlaceRequirementDetails(issueId);
            vmModel.AftersalesPm_HwIssueVerificationLogModels = _afterSales.GetLogDetailsPerIssue(issueId);
            vmModel.AftersalesPm_HwIssueReportShareModels = _afterSales.GetReportShareDetailsForPerModel(issueId);
            return View(vmModel);
        }
        [HttpPost]
        public JsonResult SaveForwardData(string Id, string ForwardReason)
        {
            long ids;
            ...
            if (ids != 0)
            {
                saveData = _afterSales.SaveForwardData(ids, ForwardReason);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult SaveAcceptedData(string Id)
        {
            long ids;
            long.TryParse(Id, out ids);
            ...
            if (ids != 0)
            {
                saveData = _afterSales.SaveAcceptedData(ids);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult SaveReportAcceptedData(string Id)
        {
            long ids;
            long.TryParse(Id, out ids);
            ...
            if (ids != 0)
            {
                saveData = _afterSales.SaveReportAcceptedData(ids);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }
        //SaveReportForwardData
        [HttpPost]
        public JsonResult SaveReportForwardData(string Id, string ForwardReason)
        {
            long ids;
            long.TryParse(Id, out ids);
            ...
            if (ids != 0)
            {
                saveData = _afterSales.SaveReportForwardData(ids, ForwardReason);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult SaveClosedIssueData(string Id)
        {
            long ids;
            long.TryParse(Id, out ids);
            ...
            if (ids != 0)
            {
                saveData = _afterSales.SaveClosedIssueData(ids);
            }
            return Json(new { data = saveData }, JsonRequestBehavior.AllowGet);
        }
        #endregion
